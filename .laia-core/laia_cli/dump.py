"""
Dump command for laia CLI.

Outputs a compact, plain-text summary of the user's LAIA setup
...
from laia_cli.config import get_laia_home, get_env_path, get_project_root, load_config
...
def _redact(value: str) -> str:
    ...
    ``laia dump`` formats empty values as blank, not as ``"(not set)"``).
    """
...
def _gateway_status() -> str:
    ...
    try:
        from laia_cli.gateway import get_gateway_runtime_snapshot
...
def _count_skills(laia_home: Path) -> int:
...
def _cron_summary(laia_home: Path) -> str:
    """Return cron jobs summary."""
    jobs_file = laia_home / "cron" / "jobs.json"
...
def run_dump(args):
    ...
    lines.append(f"laia_home:      {display_laia_home()}")
    ...
    toolsets = config.get("toolsets", ["laia-cli"])
    ...
    lines.append(f"  cron_jobs:          {_cron_summary(laia_home)}")
    lines.append(f"  skills:             {_count_skills(laia_home)}")
