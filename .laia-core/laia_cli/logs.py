"""``laia logs`` — view and filter LAIA log files.
...
Usage examples::
    ...
    laia logs                    # last 50 lines of agent.log
    laia logs -f                 # follow agent.log in real time
    laia logs errors             # last 50 lines of errors.log
    laia logs gateway -n 100    # last 100 lines of gateway.log
    laia logs --level WARNING    # only WARNING+ lines
    laia logs --session abc123   # filter by session ID substring
    laia logs --component tools  # only tool-related lines
    laia logs --since 1h         # lines from the last hour
    laia logs --since 30m -f     # follow, starting 30 min ago
"""
...
def tail_log(
    ...
    *,
    ...
    log_path = get_laia_home() / "logs" / filename
    if not log_path.exists():
        print(f"Log file not found: {log_path}")
        print(f"(Logs are created when LAIA runs — try 'laia chat' first)")
        sys.exit(1)
    ...
    if follow:
        print(f"--- {display_laia_home()}/logs/{filename}{filter_desc} (Ctrl+C to stop) ---")
    else:
        print(f"--- {display_laia_home()}/logs/{filename}{filter_desc} (last {num_lines}) ---")
...
def list_logs() -> None:
    ...
    if not log_dir.exists():
        print(f"No logs directory at {display_laia_home()}/logs/")
        return

    print(f"Log files in {display_laia_home()}/logs/:\n")
    found = False
    ...
    if not found:
        print("  (no log files yet — run 'laia chat' to generate logs)")
