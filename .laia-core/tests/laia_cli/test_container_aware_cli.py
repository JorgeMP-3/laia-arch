...
def container_env(tmp_path, monkeypatch):
    """Set up a fake LAIA_HOME with .container-mode file."""
    laia_home = tmp_path / ".laia"
    laia_home.mkdir()
    monkeypatch.setenv("LAIA_HOME", str(laia_home))
    monkeypatch.delenv("LAIA_DEV", raising=False)

    container_mode = laia_home / ".container-mode"
    container_mode.write_text(
        "# Written by NixOS activation script. Do not edit manually.\n"
        "backend=podman\n"
        "container_name=laia-agent\n"
        "exec_user=laia\n"
        "laia_bin=/data/current-package/bin/laia\n"
    )
    return laia_home
...
def test_get_container_exec_info_returns_metadata(container_env):
    ...
    with patch("laia_constants.is_container", return_value=False):
        info = get_container_exec_info()

    assert info is not None
    assert info["backend"] == "podman"
    assert info["container_name"] == "laia-agent"
    assert info["exec_user"] == "laia"
    assert info["laia_bin"] == "/data/current-package/bin/laia"


def test_get_container_exec_info_none_inside_container(container_env):
    """Returns None when we're already inside a container."""
    with patch("laia_constants.is_container", return_value=True):
        info = get_container_exec_info()
...
def test_get_container_exec_info_skipped_when_laia_dev(container_env, monkeypatch):
    """Returns None when LAIA_DEV=1 is set (dev mode bypass)."""
    monkeypatch.setenv("LAIA_DEV", "1")
...
def test_get_container_exec_info_not_skipped_when_laia_dev_zero(container_env, monkeypatch):
    """LAIA_DEV=0 does NOT trigger bypass — only '1' does."""
    monkeypatch.setenv("LAIA_DEV", "0")
...
def test_get_container_exec_info_defaults():
    ...
    with tempfile.TemporaryDirectory() as tmpdir:
        laia_home = Path(tmpdir) / ".laia"
        laia_home.mkdir()
        (laia_home / ".container-mode").write_text(
            "# minimal file with no keys\n"
        )

        with patch("laia_constants.is_container", return_value=False), \
             patch("laia_cli.config.get_laia_home", return_value=laia_home), \
             patch.dict(os.environ, {}, clear=False):
            os.environ.pop("LAIA_DEV", None)
            info = get_container_exec_info()
...
def test_get_container_exec_info_docker_backend(container_env):
    ...
    (container_env / ".container-mode").write_text(
        "backend=docker\n"
        "container_name=laia-custom\n"
        "exec_user=myuser\n"
        "laia_bin=/opt/laia/bin/laia\n"
    )
    ...
    assert info["backend"] == "docker"
    assert info["container_name"] == "laia-custom"
    assert info["exec_user"] == "myuser"
    assert info["laia_bin"] == "/opt/laia/bin/laia"
...
def test_exec_in_container_calls_execvp(docker_container_info):
    ...
    from laia_cli.main import _exec_in_container
    ...
    assert "-it" in cmd
    idx_u = cmd.index("-u")
    assert cmd[idx_u + 1] == "laia"
    e_indices = [i for i, v in enumerate(cmd) if v == "-e"]
    e_values = [cmd[i + 1] for i in e_indices]
    assert "TERM=xterm-256color" in e_values
    assert "LANG=en_US.UTF-8" in e_values
    assert "laia-agent" in cmd
    assert "/data/current-package/bin/laia" in cmd
    assert "chat" in cmd
