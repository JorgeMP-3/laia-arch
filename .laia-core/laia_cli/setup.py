"""
Interactive setup wizard for LAIA Agent.
...
Config files are stored in ~/.laia/ for easy access.
"""
...
from laia_cli.nous_subscription import get_nous_subscription_features
...
_DOCS_BASE = "https://laia-agent.nousresearch.com/docs"
...
# display_laia_home imported lazily at call sites (stale-module safety during laia update)
...
from laia_cli.cli_output import (  # noqa: E402
    print_error,
    print_info,
    print_success,
    print_warning,
...
def print_noninteractive_setup_guidance(reason: str | None = None) -> None:
    ...
    print()
    print(color("⚕ LAIA Setup — Non-interactive mode", Colors.CYAN, Colors.BOLD))
    print()
    if reason:
        print_info(reason)
    print_info("The interactive wizard cannot be used here.")
    print()
    print_info("Configure LAIA using environment variables or config commands:")
    print_info("  laia config set model.provider custom")
    print_info("  laia config set model.base_url http://localhost:8080/v1")
    print_info("  laia config set model.default your-model-name")
    print()
    print_info("Or set OPENROUTER_API_KEY / OPENAI_API_KEY in your environment.")
    print_info("Run 'laia setup' in an interactive terminal to use the full wizard.")
    print()
...
def _curses_prompt_choice(question: str, choices: list, default: int = 0, description: str | None = None) -> int:
    ...
    from laia_cli.curses_ui import curses_radiolist
...
def prompt_checklist(title: str, items: list, pre_selected: list = None) -> list:
    ...
    from laia_cli.curses_ui import curses_checklist
...
def _prompt_api_key(var: dict):
    ...
    if value:
        ...
        print_warning("  Skipped (configure later with 'laia setup')")


def _print_setup_summary(config: dict, laia_home):
    ...
    if _vision_backends:
        ...
        tool_status.append(("Vision (image analysis)", False, "run 'laia setup' to configure"))
    ...
    if subscription_features.image_gen.managed_by_nous:
        ...
        try:
            ...
            from laia_cli.plugins import _ensure_plugins_discovered
    ...
    if subscription_features.tts.managed_by_nous:
        ...
        if neutts_ok:
            ...
            tool_status.append(("Text-to-Speech (NeuTTS — not installed)", False, "run 'laia setup tts'"))
        ...
        if kittentts_ok:
            ...
            tool_status.append(("Text-to-Speech (KittenTTS — not installed)", False, "run 'laia setup tts'"))
    else:
    ...
    if subscription_features.modal.managed_by_nous:
        ...
        if subscription_features.modal.direct_override:
            ...
            tool_status.append(("Modal Execution", False, "run 'laia setup terminal'"))
    ...
    # Spotify (OAuth via laia auth spotify — check auth.json, not env vars)
    try:
        from laia_cli.auth import get_provider_auth_state
    ...
    if disabled_tools:
        print_warning(
            "Some tools are disabled. Run 'laia setup tools' to configure them,"
        )
    ...
    print(
        f"   {color('Data:', Colors.YELLOW)}      {laia_home}/cron/, sessions/, logs/"
    )
    print()
    ...
    print()
    print(f"   {color('laia setup', Colors.GREEN)}          Re-run the full wizard")
    print(f"   {color('laia setup model', Colors.GREEN)}    Change model/provider")
    print(f"   {color('laia setup terminal', Colors.GREEN)} Change terminal backend")
    print(f"   {color('laia setup gateway', Colors.GREEN)}  Configure messaging")
    print(f"   {color('laia setup tools', Colors.GREEN)}    Configure tool providers")
    print()
    print(f"   {color('laia config', Colors.GREEN)}         View current settings")
    print(
        f"   {color('laia config edit', Colors.GREEN)}    Open config in your editor"
    )
    print(f"   {color('laia config set <key> <value>', Colors.GREEN)}")
    ...
    print()
    print(f"   {color('laia', Colors.GREEN)}              Start chatting")
    print(f"   {color('laia gateway', Colors.GREEN)}      Start messaging gateway")
    print(f"   {color('laia doctor', Colors.GREEN)}       Check for issues")
    print()
...
# between `laia tools` and `laia setup tools`).
...
def setup_model_provider(config: dict, *, quick: bool = False):
    ...
    Delegates to ``cmd_model()`` (the same flow used by ``laia model``)
    ...
    provider added to ``laia model`` is automatically available here.
    ...
    # Delegate to the shared laia model flow — handles provider picker,
    # credential prompting, model selection, and config persistence.
    from laia_cli.main import select_provider_and_model
    try:
        ...
        print_info("You can try again later with: laia model")
    ...
    if not quick and _supports_same_provider_pool_setup(selected_provider):
        try:
            ...
            from laia_cli.auth_commands import auth_add_command
            ...
            print_info(
                "LAIA can keep multiple credentials for one provider and rotate between"
            )
            print_info(
    ...
    if _vision_needs_setup:
        ...
        print_info("doesn't currently provide one LAIA can auto-use for vision,")
        ...
        if _vision_idx == 0:  # OpenRouter
            ...
            print_info("Skipped — add later with 'laia setup' or configure AUXILIARY_VISION_* settings")
...
def _setup_tts_provider(config: dict):
    ...
    if selected == "nous-openai":
        ...
        if get_env_value("VOICE_TOOLS_OPENAI_KEY") or get_env_value("OPENAI_API_KEY"):
            print_warning(
                "Direct OpenAI credentials are still configured and may take precedence until removed from ~/.laia/.env."
            )
        ...
        if not existing:
            ...
            if api_key:
                ...
                print_warning(
                    ...
                    f"laia setup model or {_dhh()}/.env to use xAI TTS. "
...
def setup_tts(config: dict):
    """Standalone TTS setup (for 'laia setup tts')."""
    _setup_tts_provider(config)
...
def setup_terminal_backend(config: dict):
    ...
    print_info("Choose where LAIA runs shell commands and code.")
    ...
    if selected_backend == "local":
        ...
        print_info("  When using LAIA via Telegram/Discord, this is where")
        print_info(
        ...
        print_info("Requires the optional SDK: pip install 'laia-agent[vercel]'")
        ...
        try:
            ...
            if result.returncode == 0:
                ...
                print_warning("Install failed — run manually: pip install 'laia-agent[vercel]'")
                if result.stderr:
                    print_info(f"  Error: {result.stderr.strip().splitlines()[-1]}")
...
def _apply_default_agent_settings(config: dict):
    ...
    save_env_value("LAIA_MAX_ITERATIONS", "90")
    ...
    print_info("  Run `laia setup agent` later to customize.")
...
def setup_agent_settings(config: dict):
    ...
    # ── Max Iterations ──
    current_max = get_env_value("LAIA_MAX_ITERATIONS") or str(
    ...
    try:
        ...
        if max_iter > 0:
            save_env_value("LAIA_MAX_ITERATIONS", str(max_iter))
...
def _setup_telegram():
    ...
    print()
    print_info("📬 Home Channel: where LAIA delivers cron job results,")
...
def _setup_slack():
    ...
    if existing:
        ...
        if not prompt_yes_no("Reconfigure Slack?", False):
            ...
            if prompt_yes_no(
                ...
                "list? (recommended after `laia update`)",
                True,
            ):
    ...
    print()
    print_info("   Full guide: https://laia-agent.nousresearch.com/docs/user-guide/messaging/slack/")
    print()
...
def _write_slack_manifest_and_instruct():
    """Generate the Slack manifest, write it under LAIA_HOME, and print
    ...
    """
    try:
        from laia_cli.slack_cli import _build_full_manifest
        from laia_constants import get_laia_home

        manifest = _build_full_manifest(
            bot_name="LAIA",
            bot_description="Your LAIA agent on Slack",
        )
        target = Path(get_laia_home()) / "slack-manifest.json"
        ...
        print_info(
            "   Re-run `laia slack manifest --write` anytime to refresh after "
            "LAIA adds new commands."
        )
...
def _setup_matrix():
    ...
    if token or get_env_value("MATRIX_PASSWORD"):
        ...
        print()
        print_info("📬 Home Room: where LAIA delivers cron job results and notifications.")
...
def _setup_mattermost():
    ...
    print()
    print_info("📬 Home Channel: where LAIA delivers cron job results and notifications.")
...
def _setup_whatsapp():
    ...
    print_info("Requires Node.js. Run 'laia whatsapp' for guided setup.")
    print()
    if prompt_yes_no("Enable WhatsApp now?", True):
        save_env_value("WHATSAPP_ENABLED", "true")
        print_success("WhatsApp enabled")
        print_info("Run 'laia whatsapp' to choose your mode (separate bot number")
        print_info("or personal self-chat) and pair via QR code.")


def _setup_weixin():
    """Configure Weixin (personal WeChat) via iLink Bot API QR login."""
    from laia_cli.gateway import _setup_weixin as _gateway_setup_weixin
    _gateway_setup_weixin()


def _setup_signal():
    """Configure Signal via gateway setup."""
    from laia_cli.gateway import _setup_signal as _gateway_setup_signal
    _gateway_setup_signal()


def _setup_email():
    """Configure Email via gateway setup."""
    from laia_cli.gateway import _setup_email as _gateway_setup_email
    _gateway_setup_email()


def _setup_sms():
    """Configure SMS (Twilio) via gateway setup."""
    from laia_cli.gateway import _setup_sms as _gateway_setup_sms
    _gateway_setup_sms()


def _setup_dingtalk():
    """Configure DingTalk via gateway setup."""
    from laia_cli.gateway import _setup_dingtalk as _gateway_setup_dingtalk
    _gateway_setup_dingtalk()


def _setup_feishu():
    """Configure Feishu / Lark via gateway setup."""
    from laia_cli.gateway import _setup_feishu as _gateway_setup_feishu
    _gateway_setup_feishu()


def _setup_yuanbao():
    """Configure Yuanbao via gateway setup."""
    from laia_cli.gateway import _setup_yuanbao as _gateway_setup_yuanbao
    _gateway_setup_yuanbao()


def _setup_wecom():
    """Configure WeCom (Enterprise WeChat) via gateway setup."""
    from laia_cli.gateway import _setup_wecom as _gateway_setup_wecom
    _gateway_setup_wecom()


def _setup_wecom_callback():
    """Configure WeCom Callback (self-built app) via gateway setup."""
    from laia_cli.gateway import _setup_wecom_callback as _gw_setup
    _gw_setup()
...
def _setup_bluebubbles():
    ...
    print_info("Connects LAIA to iMessage via BlueBubbles — a free, open-source")
...
def _setup_qqbot():
    ...
    from laia_cli.gateway import _setup_qqbot as _gateway_setup_qqbot
...
def _setup_webhooks():
    ...
    print()
    print_info("   Full guide: https://laia-agent.nousresearch.com/docs/user-guide/messaging/webhooks/")
    print()
    ...
    print_info("   https://laia-agent.nousresearch.com/docs/user-guide/messaging/webhooks/#configuring-routes")
    print()
    print_info("   Open config in your editor:  laia config edit")
...
def setup_gateway(config: dict):
    ...
    print_info("Connect to messaging platforms to chat with LAIA from anywhere.")
    ...
    if not selected:
        print_info("No platforms selected. Run 'laia setup gateway' later to configure.")
        return
    ...
    if any_messaging:
        ...
        if missing_home:
            ...
            for plat in missing_home:
                print_info(
                    f"     laia config set {plat.upper()}_HOME_CHANNEL <channel_id>"
                )
        ...
        if supports_systemd and has_legacy_laia_units():
            print_legacy_unit_warning()
            print()
        ...
        if service_running:
            ...
            if prompt_yes_no(
                ...
                print_info("  Or run in foreground:  laia gateway")
        else:
            from laia_constants import is_container
            if is_container():
                print_info("Start the gateway to bring your bots online:")
                print_info("   laia gateway run          # Run as container main process")
                print_info("")
                ...
                print_info("Start the gateway to bring your bots online:")
                print_info("   laia gateway              # Run in foreground")
...
def setup_tools(config: dict, first_install: bool = False):
    ...
    Both `laia setup tools` and `laia tools` use the same flow:
...
def _model_section_has_credentials(config: dict) -> bool:
    ...
    Sources of truth:
      * ``PROVIDER_REGISTRY`` in ``laia_cli.auth`` — lists every supported
    ...
    """
    try:
        from laia_cli.auth import get_active_provider
...
def _get_section_config_summary(config: dict, section_key: str) -> Optional[str]:
    ...
    ``get_env_value`` is the module-level import from laia_cli.config
...
def _load_openclaw_migration_module():
    """Load the openclaw_to_laia migration script as a module.
    ...
    spec = importlib.util.spec_from_file_location(
        "openclaw_to_laia", _OPENCLAW_SCRIPT
    )
...
# Config values may have different semantics between OpenClaw and LAIA.
# Instruction/context files (.md) can contain incompatible setup procedures.
_HIGH_IMPACT_KIND_KEYWORDS = {
    "gateway": "⚠ Gateway/messaging — this will configure LAIA to use your OpenClaw messaging channels",
    "telegram": "⚠ Telegram — this will point LAIA at your OpenClaw Telegram bot",
    "slack": "⚠ Slack — this will point LAIA at your OpenClaw Slack workspace",
    "discord": "⚠ Discord — this will point LAIA at your OpenClaw Discord bot",
    "whatsapp": "⚠ WhatsApp — this will point LAIA at your OpenClaw WhatsApp connection",
    "config": "⚠ Config values — OpenClaw settings may not map 1:1 to LAIA equivalents",
...
def _print_migration_preview(report: dict):
    ...
    if conflict_items:
        print(color("  Would overwrite (conflicts with existing LAIA config):", Colors.YELLOW))
    ...
    if warnings_shown:
        ...
        print()
        print(color("  Note: OpenClaw config values may have different semantics in LAIA.", Colors.YELLOW))
        print(color("  For example, OpenClaw's tool_call_execution: \"auto\" ≠ LAIA's yolo mode.", Colors.YELLOW))
        print(color("  Instruction files (.md) from OpenClaw may contain incompatible procedures.", Colors.YELLOW))
        print()


def _offer_openclaw_migration(laia_home: Path) -> bool:
    ...
    print_info("LAIA can preview what would be imported before making any changes.")
    print()

    if not prompt_yes_no("Would you like to see what can be imported?", default=True):
        print_info(
            "Skipping migration. You can run it later with: laia claw migrate --dry-run"
        )
        return False
    ...
    if not prompt_yes_no("Proceed with migration?", default=False):
        print_info(
            "Migration cancelled. You can run it later with: laia claw migrate"
        )
        print_info(
    ...
    # Execute the migration — overwrite=False so existing LAIA configs are
    ...
    try:
        migrator = mod.Migrator(
            ...
            execute=True,
            workspace_target=None,
            overwrite=False,  # preserve existing LAIA config
            migrate_secrets=True,
            output_dir=None,
            selected_options=selected,
            preset_name="full",
    ...
    if conflicts:
        print_info(f"Skipped {conflicts} item(s) that already exist in LAIA (use laia claw migrate --overwrite to force).")
    if skipped:
...
def run_setup_wizard(args):
    ...
    Supports full, quick, and section-specific setup:
      laia setup           — full or quick (auto-detected)
      laia setup model     — just model/provider
      laia setup tts       — just text-to-speech
      laia setup terminal  — just terminal backend
      laia setup gateway   — just messaging platforms
      laia setup tools     — just tool configuration
      laia setup agent     — just agent settings
    """
    ...
    if section:
        for key, label, func in SETUP_SECTIONS:
            if key == section:
                ...
                print(color(f"│     ⚕ LAIA Setup — {label:<34s} │", Colors.MAGENTA))
                print(
                    color(
                        "└─────────────────────────────────────────────────────────┘",
                        Colors.MAGENTA,
    ...
    print(
        color(
            "│             ⚕ LAIA Agent Setup Wizard                │", Colors.MAGENTA
        )
    )
    print(
        color(
    ...
    print(
        color(
            "│  Let's configure your LAIA Agent installation.       │", Colors.MAGENTA
        )
    )
    print(
        color(
    ...
    if is_existing:
        ...
        if quick_requested:
            _run_quick_setup(config, laia_home)
            return

        print()
        print_header("Reconfigure")
        print_success("You already have LAIA configured.")
        print_info("Running the full wizard — each prompt shows your current value.")
        print_info("Press Enter to keep it, or type a new value to change it.")
        print_info("")
        print_info("Tip: jump straight to a section with 'laia setup model|terminal|")
        ...
        migration_ran = _offer_openclaw_migration(laia_home)
        if migration_ran:
            config = load_config()

        setup_mode = prompt_choice("How would you like to set up LAIA?", [
            "Quick setup — provider, model & messaging (recommended)",
            "Full setup — configure everything",
        ], 0)

        if setup_mode == 0:
            _run_first_time_quick_setup(config, laia_home, is_existing)
            return
    ...
    print_info(f"Data folder:  {laia_home}")
    print_info(f"Install dir:  {PROJECT_ROOT}")
    print()
    print_info("You can edit these files directly or use 'laia config edit'")
    ...
    save_config(config)
    _print_setup_summary(config, laia_home)

    _offer_launch_chat()


def _resolve_laia_chat_argv() -> Optional[list[str]]:
    """Resolve argv for launching ``laia chat`` in a fresh process."""
    laia_bin = shutil.which("laia")
    if laia_bin:
        return [laia_bin, "chat"]

    try:
        if importlib.util.find_spec("laia_cli") is not None:
            return [sys.executable, "-m", "laia_cli.main", "chat"]
    except Exception:
        pass
...
def _offer_launch_chat():
    ...
    print()
    if not prompt_yes_no("Launch laia chat now?", True):
        return

    chat_argv = _resolve_laia_chat_argv()
    if not chat_argv:
        print_info("Could not relaunch LAIA automatically. Run 'laia chat' manually.")
        return

    os.execvp(chat_argv[0], chat_argv)


def _run_first_time_quick_setup(config: dict, laia_home, is_existing: bool):
    ...
    gateway_choice = prompt_choice(
        ...
        [
            ...
            "Skip — set up later with 'laia setup gateway'",
        ],
        0,
    )
    ...
    print()
    print_info("  Configure all settings:    laia setup")
    if gateway_choice != 0:
        print_info("  Connect Telegram/Discord:  laia setup gateway")
    print()
...
def _run_quick_setup(config: dict, laia_home):
    ...
    if not has_anything_missing:
        ...
        print()
        print_info("Run 'laia setup' and choose 'Full Setup' to reconfigure,")
    ...
    if missing_messaging:
        print()
        print_header("Messaging Platforms")
        print_info("Connect LAIA to messaging apps to chat from anywhere.")
        print_info("You can configure these later with 'laia setup gateway'.")
