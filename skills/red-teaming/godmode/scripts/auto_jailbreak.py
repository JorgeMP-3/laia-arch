...
Usage in execute_code:
    exec(open(os.path.expanduser(
        os.path.join(os.environ.get("LAIA_HOME", os.path.expanduser("~/.laia")), "skills/red-teaming/godmode/scripts/auto_jailbreak.py")
    )).read())
...
try:
    ...
    _SKILL_DIR = Path(os.getenv("LAIA_HOME", Path.home() / ".laia")) / "skills" / "red-teaming" / "godmode"
...
LAIA_HOME = Path(os.getenv("LAIA_HOME", Path.home() / ".laia"))
CONFIG_PATH = LAIA_HOME / "config.yaml"
PREFILL_PATH = LAIA_HOME / "prefill.json"
...
MODEL_STRATEGIES = {
    ...
    # Nous/LAIA models — already uncensored, just needs clean prompt
    "laia": {
        "order": ["prefill_only"],
        "system_templates": {},
    },
...
def _detect_model_family(model: str) -> str:
    ...
    if "grok" in model_lower or "x-ai" in model_lower:
        return "grok"
    if "laia" in model_lower or "nous" in model_lower:
        return "laia"
...
def _get_current_model() -> tuple:
    """Read current model and provider from LAIA config.yaml.
...
def _write_prefill(prefill_messages: list):
    """Write prefill messages to ~/.laia/prefill.json."""
    ...
    if winning_strategy:
        ...
        if not dry_run:
            ...
            if verbose:
                ...
                print()
                print("[DONE] Jailbreak locked in. Restart LAIA for changes to take effect.")
        else:
            if verbose:
...
def undo_jailbreak(verbose=True):
    ...
    if verbose:
        print("[UNDO] Jailbreak removed. Restart LAIA for changes to take effect.")
