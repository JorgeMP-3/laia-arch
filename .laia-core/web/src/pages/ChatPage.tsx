/**
 * ChatPage — embeds `laia --tui` inside the dashboard.
 *
 ...
 *          ▼                                                              .
 *     FastAPI pty_ws  (laia_cli/web_server.py)                          .
 *          ▼                                                              .
...
export default function ChatPage({ isActive = true }: { isActive?: boolean }) {
  ...
  const [banner, setBanner] = useState<string | null>(() =>
    typeof window !== "undefined" && !window.__LAIA_SESSION_TOKEN__
      ? "Session token unavailable. Open this page through `laia dashboard`, not directly."
      : null,
  );
  ...
  useEffect(() => {
    ...
    const token = window.__LAIA_SESSION_TOKEN__;
    ...
    if (useWebgl) {
      try {
        ...
        console.warn(
          "[laia-chat] WebGL renderer unavailable; falling back to default",
          err,
        );
      }
    }
    ...
    <div className="flex min-h-0 flex-1 flex-col gap-2 normal-case">
      ...
      <div className="flex min-h-0 flex-1 flex-col gap-2 lg:flex-row lg:gap-3">
        ...
        >
          <div
            ref={hostRef}
            className="laia-chat-xterm-host min-h-0 min-w-0 flex-1"
          />
