...
class TestWriteDenyExactPaths:
    ...
    def test_laia_env(self):
        # ``.env`` under the active LAIA_HOME (profile-aware, not just
        # ``~/.laia``) must be write-denied. The hermetic test conftest
        # points LAIA_HOME at a tempdir — resolve via get_laia_home()
        # to match the denylist.
        from laia_constants import get_laia_home
        path = str(get_laia_home() / ".env")
        assert _is_write_denied(path) is True
...
class TestWriteAllowed:
    ...
    def test_laia_config_not_env(self):
        path = os.path.join(str(Path.home()), ".laia", "config.yaml")
