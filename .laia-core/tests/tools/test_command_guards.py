...
def _clean_state():
    ...
    saved = {}
    for k in ("LAIA_INTERACTIVE", "LAIA_GATEWAY_SESSION", "LAIA_EXEC_ASK", "LAIA_YOLO_MODE"):
        if k in os.environ:
...
class TestTirithWarnSafe:
    ...
    def test_warn_session_approved(self, mock_tirith):
        os.environ["LAIA_INTERACTIVE"] = "1"
        session_key = os.getenv("LAIA_SESSION_KEY", "default")
    ...
    def test_warn_non_interactive_auto_allow(self, mock_tirith):
        # No LAIA_INTERACTIVE or LAIA_GATEWAY_SESSION set
        result = check_all_command_guards("curl https://bit.ly/abc", "local")
        assert result["approved"] is True
