...
as ``kind: backend``) or ``~/.laia/plugins/image_gen/<name>/`` (user, opt-in
via ``plugins.enabled``).
...
class ImageGenProvider(abc.ABC):
    ...
    def display_name(self) -> str:
        """Human-readable label shown in ``laia tools``. Defaults to ``name.title()``."""
    ...
    def list_models(self) -> List[Dict[str, Any]]:
        """Return catalog entries for ``laia tools`` model picker.
    ...
    def get_setup_schema(self) -> Dict[str, Any]:
        """Return provider metadata for the ``laia tools`` picker.
...
def _images_cache_dir() -> Path:
    """Return ``$LAIA_HOME/cache/images/``, creating parents as needed."""
    from laia_constants import get_laia_home

    path = get_laia_home() / "cache" / "images"
...
def save_b64_image(
    ...
    *,
    ...
    """Decode base64 image data and write it under ``$LAIA_HOME/cache/images/``.
