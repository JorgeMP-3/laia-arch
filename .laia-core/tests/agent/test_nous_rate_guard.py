...
def rate_guard_env(tmp_path, monkeypatch):
    ...
    laia_home = str(tmp_path / ".laia")
    os.makedirs(laia_home, exist_ok=True)
    monkeypatch.setenv("LAIA_HOME", laia_home)
...
class TestIsGenuineNousRateLimit:
    ...
    Nous Portal multiplexes upstreams (DeepSeek, Kimi, MiMo, LAIA).
