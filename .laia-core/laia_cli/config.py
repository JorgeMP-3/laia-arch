"""
Configuration management for LAIA Agent.

Config files are stored in ~/.laia/ for easy access:
- ~/.laia/config.yaml  - All settings (model, toolsets, terminal, etc.)
- ~/.laia/.env         - API keys and secrets

This module provides:
- laia config          - Show current configuration
- laia config edit     - Open config in editor
- laia config set      - Set a specific value
- laia config wizard   - Re-run setup wizard
"""
...
_EXTRA_ENV_KEYS = frozenset({
    ...
    # Activation is via plugins.enabled (opt-in through `laia plugins enable
    # observability/langfuse` or `laia tools → Langfuse`); credentials gate
    # the plugin at runtime.
    "LAIA_LANGFUSE_ENV",
    "LAIA_LANGFUSE_RELEASE",
    "LAIA_LANGFUSE_SAMPLE_RATE",
    "LAIA_LANGFUSE_MAX_CHARS",
    "LAIA_LANGFUSE_DEBUG",
    "LANGFUSE_PUBLIC_KEY",
    "LANGFUSE_SECRET_KEY",
    "LANGFUSE_BASE_URL",
...
from laia_cli.default_soul import DEFAULT_SOUL_MD
...
def get_managed_system() -> Optional[str]:
    ...
    raw = os.getenv("LAIA_MANAGED", "").strip()
    if raw:
    ...
    managed_marker = get_laia_home() / ".managed"
...
def is_managed() -> bool:
    """Check if LAIA is running in package-manager-managed mode.

    Two signals: the LAIA_MANAGED env var (set by the systemd service),
    or a .managed marker file in LAIA_HOME (set by the NixOS activation
...
def get_managed_update_command() -> Optional[str]:
    ...
    if managed_system == "Homebrew":
        return "brew upgrade laia-agent"
...
def recommended_update_command() -> str:
    ...
    return get_managed_update_command() or "laia update"


def format_managed_message(action: str = "modify this LAIA installation") -> str:
    """Build a user-facing error for managed installs."""
    managed_system = get_managed_system() or "a package manager"
    raw = os.getenv("LAIA_MANAGED", "").strip().lower()

    if managed_system == "NixOS":
        env_hint = "true" if raw in _MANAGED_TRUE_VALUES else raw or "true"
        return (
            f"Cannot {action}: this LAIA installation is managed by NixOS "
            f"(LAIA_MANAGED={env_hint}).\n"
            "Edit services.laia-agent.settings in your configuration.nix and run:\n"
    ...
    if managed_system == "Homebrew":
        ...
        return (
            f"Cannot {action}: this LAIA installation is managed by Homebrew "
            f"(LAIA_MANAGED={env_hint}).\n"
            "Use:\n"
            "  brew upgrade laia-agent"
        )

    return (
        f"Cannot {action}: this LAIA installation is managed by {managed_system}.\n"
        "Use your package manager to upgrade or reinstall LAIA."
    )
...
def get_container_exec_info() -> Optional[dict]:
    """Read container mode metadata from LAIA_HOME/.container-mode.

    Returns a dict with keys: backend, container_name, exec_user, laia_bin
    or None if container mode is not active, we're already inside the
    container, or LAIA_DEV=1 is set.

    The .container-mode file is written by the NixOS activation script when
    container.enable = true. It tells the host CLI to exec into the container
    instead of running locally.
    """
    if os.environ.get("LAIA_DEV") == "1":
        return None
    ...
    container_mode_file = get_laia_home() / ".container-mode"
    ...
    container_name = info.get("container_name", "laia-agent")
    exec_user = info.get("exec_user", "laia")
    laia_bin = info.get("laia_bin", "/data/current-package/bin/laia")
...
# Re-export from laia_constants — canonical definition lives there.
from laia_constants import get_laia_home  # noqa: F811,E402
from utils import atomic_replace

def get_config_path() -> Path:
    """Get the main config file path."""
    return get_laia_home() / "config.yaml"

def get_env_path() -> Path:
    """Get the .env file path (for API keys)."""
    return get_laia_home() / ".env"
...
def _secure_dir(path):
    ...
    permissions (0750) so interactive users in the laia group can
    share state with the gateway service.

    The mode can be overridden via the LAIA_HOME_MODE environment variable
    (e.g. LAIA_HOME_MODE=0701) for deployments where a web server (nginx,
    caddy, etc.) needs to traverse LAIA_HOME to reach a served subdirectory.
    ...
    if is_managed():
        return
    try:
        mode_str = os.environ.get("LAIA_HOME_MODE", "").strip()
...
def _is_container() -> bool:
    ...
    When LAIA runs in a container with volume-mounted config files, forcing
    ...
    """
    # Explicit opt-out
    if os.environ.get("LAIA_CONTAINER") or os.environ.get("LAIA_SKIP_CHMOD"):
        return True
...
def _secure_file(path):
    ...
    permissions.  Set LAIA_SKIP_CHMOD=1 to force-skip on other systems.
    """
...
def _ensure_default_soul_md(home: Path) -> None:
    """Seed a default SOUL.md into LAIA_HOME if the user doesn't have one yet."""
...
def ensure_laia_home():
    """Ensure ~/.laia directory structure exists with secure permissions.
    ...
    if is_managed():
        ...
        try:
            _ensure_laia_home_managed(home)
        finally:
            os.umask(old_umask)
    else:
...
def _ensure_laia_home_managed(home: Path):
    """Managed-mode variant: verify dirs exist (activation creates them), seed SOUL.md."""
    if not home.is_dir():
        raise RuntimeError(
            f"LAIA_HOME {home} does not exist. "
...
DEFAULT_CONFIG = {
    ...
    "agent": {
        ...
        # the LAIA-level retry loop that wraps the whole call.  Lower
    ...
    "terminal": {
        ...
        # skipped. When empty, LAIA auto-sources ``~/.profile``,
        # ``~/.bash_profile``, and ``~/.bashrc`` (in that order) if the
        # snapshot shell is bash (this is the ``auto_source_bashrc``
        # behaviour — disable with that key if you want strict login-only
        # semantics).
        "shell_init_files": [],
        # When true (default), LAIA sources the user's shell rc files
        ...
        # docker_env lets you specify exact key-value pairs — useful when LAIA
        ...
        #  "/home/user/.laia/cache/documents:/output"]
        ...
        # bundled LAIA image, which drops to the `laia` user via gosu).
    ...
    "browser": {
        ...
        "camofox": {
            # When true, LAIA sends a stable profile-scoped userId to Camofox
    ...
    "checkpoints": {
        ...
        # ~/.laia/checkpoints/ (one per cd'd working directory). Field
        # reports put the typical offender at 1000+ repos / ~12 GB. When
        # auto_prune is on, laia sweeps at startup (at most once per
        # min_interval_hours) and deletes:
        #   * orphan repos: LAIA_WORKDIR no longer exists on disk
    ...
    # single read_file page exceeds these limits, LAIA truncates the
    ...
    "display": {
        ...
        # When true, `laia --tui` auto-resumes the most recent human-
        # facing session on launch instead of forging a fresh one.
        # Mirrors `laia -c` muscle memory.  Default off so existing
        # users aren't surprised.  LAIA_TUI_RESUME=<id> always wins.
        ...
        # e.g. `model · 68% · ~/projects/laia`. Per-platform overrides go under
    ...
    # a plugin in plugins/context_engine/<name>/ or ~/.laia/plugins/.
    "context": {
        "engine": "compressor",
    },
    ...
    # always goes to ~/.laia/skills/.
    "skills": {
        "external_dirs": [],   # e.g. ["~/.agents/skills", "/shared/team-skills"]
        # Substitute ${LAIA_SKILL_DIR} and ${LAIA_SESSION_ID} in SKILL.md
    ...
    # cron daemon.
    #
    # See `laia curator status` for the last run summary.
    "curator": {
        "enabled": True,
        ...
        # to use LAIA' main auxiliary client resolution.
        "auxiliary": {
            "provider": None,
            "model": None,
        },
    ...
    # This section is only needed for laia-specific overrides; everything else
    ...
    "whatsapp": {
        ...
        # Default (None) uses the built-in "⚕ *LAIA Agent*" header.
    ...
    # stored approval from ~/.laia/shell-hooks-allowlist.json.
    # See `website/docs/user-guide/features/hooks.md` for schema + examples.
    "hooks": {},

    # Auto-accept shell-hook registrations without a TTY prompt.  Also
    # toggleable per-invocation via --accept-hooks or LAIA_ACCEPT_HOOKS=1.
    ...
    "cron": {
        ...
        # Also overridable via LAIA_CRON_MAX_PARALLEL env var.
    ...
    "code_execution": {
        ...
        #     laia-agent's own python (sys.executable). Maximum isolation
    ...
    # Logging — controls file logging to ~/.laia/logs/.
    ...
    # update model picker lists without shipping a laia-agent release.
    # The default URL is served by the docs site GitHub Pages deploy.
    "model_catalog": {
        "enabled": True,
        "url": "https://laia-agent.nousresearch.com/docs/api/model-catalog.json",
        # Disk cache TTL in hours.  Beyond this, the CLI refetches on the
        # next /model or `laia model` invocation; network failures
    ...
    # Session storage — controls automatic cleanup of ~/.laia/state.db.
    ...
    "sessions": {
        ...
        # default of ``laia sessions prune``.
        "retention_days": 90,
    ...
    # ``laia update`` behaviour.
    "updates": {
        # Run a full ``laia backup``-style zip of LAIA_HOME before every
        # ``laia update``.  Backups land in ``<LAIA_HOME>/backups/`` and
        # can be restored with ``laia import <path>``.  Off by default —
        # on large LAIA_HOME directories the zip can add minutes to every
...
OPTIONAL_ENV_VARS = {
    ...
    "AZURE_FOUNDRY_BASE_URL": {
        "description": "Azure Foundry base URL (set via 'laia model' for endpoint-specific config)",
    ...
    "TOOL_GATEWAY_USER_TOKEN": {
        "description": "Explicit Nous Subscriber access token for tool-gateway requests (optional; otherwise read from the LAIA auth store)",
    ...
    "MATRIX_USER_ID": {
        "description": "Matrix user ID (e.g. @laia:example.org)",
    ...
    "MATRIX_DEVICE_ID": {
        "description": "Stable Matrix device ID for E2EE persistence across restarts (e.g. LAIA_BOT)",
    ...
    "API_SERVER_MODEL_NAME": {
        "description": "Model name advertised on /v1/models. Defaults to the profile name (or 'laia-agent' for the default profile). Useful for multi-user setups with OpenWebUI.",
    ...
    "GATEWAY_PROXY_URL": {
        "description": "URL of a remote LAIA API server to forward messages to (proxy mode). When set, the gateway handles platform I/O only — all agent work is delegated to the remote server. Use for Docker E2EE containers that relay to a host agent. Also configurable via gateway.proxy_url in config.yaml.",
        "prompt": "Remote LAIA API server URL (e.g. http://192.168.1.100:8642)",
        "url": None,
        "password": False,
        "category": "messaging",
        "advanced": True,
    ...
    "GATEWAY_PROXY_KEY": {
        "description": "Bearer token for authenticating with the remote LAIA API server (proxy mode). Must match the API_SERVER_KEY on the remote host.",
    ...
    # LAIA_TOOL_PROGRESS and LAIA_TOOL_PROGRESS_MODE are deprecated —
...
def get_missing_skill_config_vars() -> List[Dict[str, Any]]:
    ...
    Scans all enabled skills for ``metadata.laia.config`` entries, then checks
...
def _normalize_custom_provider_entry(
    ...
    *,
    ...
    if isinstance(models, dict) and models:
        ...
        # Hand-edited configs (and older LAIA versions) write ``models`` as
...
def get_custom_provider_context_length(
    ...
    used by:
      ...
      * ``laia_cli.model_switch.resolve_display_context_length`` (``/model`` confirmation display)
...
def validate_config_structure(config: Optional[Dict[str, Any]] = None) -> List["ConfigIssue"]:
    ...
    if config is None:
        try:
            ...
            return [ConfigIssue("error", "Could not load config.yaml", "Run 'laia setup' to create a valid config")]
    ...
    if cp and not model_cfg:
        issues.append(ConfigIssue(
            "warning",
            "custom_providers defined but no 'model' section — LAIA won't know which provider to use",
            "Add a model section:\n"
            "  model:\n"
            "    provider: custom\n"
...
def print_config_warnings(config: Optional[Dict[str, Any]] = None) -> None:
    ...
    lines.append("  \033[2mRun 'laia doctor' for fix suggestions.\033[0m")
    sys.stderr.write("\n".join(lines) + "\n\n")
...
def warn_deprecated_cwd_env_vars(config: Optional[Dict[str, Any]] = None) -> None:
    ...
    if lines:
        hint_path = os.environ.get("LAIA_HOME", "~/.laia")
...
def migrate_config(interactive: bool = True, quiet: bool = False) -> Dict[str, Any]:
    ...
    if current_ver < 4:
        ...
        if "tool_progress" not in display:
            old_enabled = get_env_value("LAIA_TOOL_PROGRESS")
            old_mode = get_env_value("LAIA_TOOL_PROGRESS_MODE")
            if old_enabled and old_enabled.lower() in ("false", "0", "no"):
                display["tool_progress"] = "off"
                results["config_added"].append("display.tool_progress=off (from LAIA_TOOL_PROGRESS=false)")
            elif old_mode and old_mode.lower() in ("new", "all"):
                display["tool_progress"] = old_mode.lower()
                results["config_added"].append(f"display.tool_progress={old_mode.lower()} (from LAIA_TOOL_PROGRESS_MODE)")
            else:
    ...
    if current_ver < 5:
        ...
        if "timezone" not in config:
            old_tz = os.getenv("LAIA_TIMEZONE", "")
            if old_tz and old_tz.strip():
                config["timezone"] = old_tz.strip()
                results["config_added"].append(f"timezone={old_tz.strip()} (from LAIA_TIMEZONE)")
            else:
                config["timezone"] = ""
    ...
    if current_ver < 21:
        ...
        if "enabled" not in plugins_cfg:
            ...
            # Scan ``$LAIA_HOME/plugins/`` for currently installed user plugins.
            grandfathered: List[str] = []
            try:
                user_plugins_dir = get_laia_home() / "plugins"
            ...
            if not quiet:
                if grandfathered:
                    ...
                    print(
                        ...
                        "Use `laia plugins enable <name>` to activate."
                    )
    ...
    if new_var_names and interactive and not quiet:
        ...
        if new_and_unset:
            ...
            if answer in ("y", "yes"):
                ...
                print("  Set later with: laia config set <key> <value>")
    ...
    # metadata.laia.config in their SKILL.md frontmatter.
...
def cfg_get(cfg: Optional[Dict[str, Any]], *keys: str, default: Any = None) -> Any:
    ...
    ubiquitous ``cfg_path = _laia_home / "config.yaml"`` local variable
...
def read_raw_config() -> Dict[str, Any]:
    """Read ~/.laia/config.yaml as-is, without merging defaults or migrating.
...
def load_config() -> Dict[str, Any]:
    """Load configuration from ~/.laia/config.yaml.
    ...
    (which change ``LAIA_HOME`` and therefore ``get_config_path()``)
    don't collide.
    """
    ensure_laia_home()
    config_path = get_config_path()
...
#   openai-codex (OAuth — laia auth) — OpenAI Codex
#   nous         (OAuth — laia auth) — Nous Portal
...
def save_config(config: Dict[str, Any]):
    """Save configuration to ~/.laia/config.yaml."""
    if is_managed():
...
def load_env() -> Dict[str, str]:
    """Load environment variables from ~/.laia/.env.
...
def _sanitize_env_lines(lines: list) -> list:
    ...
    split on real LAIA env var names, avoiding false positives from values
...
def sanitize_env_file() -> int:
    """Read, sanitize, and rewrite ~/.laia/.env in place.
...
def save_env_value(key: str, value: str):
    """Save or update a value in ~/.laia/.env."""
    if is_managed():
...
def remove_env_value(key: str) -> bool:
    """Remove a key from ~/.laia/.env and os.environ.
...
def reload_env() -> int:
    """Re-read ~/.laia/.env into os.environ. Returns count of vars updated.

    Adds/updates vars that changed and removes vars that were deleted from
    the .env file (but only vars known to LAIA — OPTIONAL_ENV_VARS and
    ...
    for key, value in env_vars.items():
        if os.environ.get(key) != value:
            os.environ[key] = value
            count += 1
    # Remove known LAIA vars that are no longer in .env
...
def get_env_value(key: str) -> Optional[str]:
    """Get a value from ~/.laia/.env or environment."""
...
def show_config():
    ...
    print(color("│              ⚕ LAIA Configuration                    │", Colors.CYAN))
    print(color("└─────────────────────────────────────────────────────────┘", Colors.CYAN))
    ...
    print(color("  laia config edit     # Edit config file", Colors.DIM))
    print(color("  laia config set <key> <value>", Colors.DIM))
    print(color("  laia setup           # Run setup wizard", Colors.DIM))
    print()
...
def config_command(args):
    ...
    if subcmd is None or subcmd == "show":
        ...
        if not key or value is None:
            print("Usage: laia config set <key> <value>")
            print()
            print("Examples:")
            print("  laia config set model anthropic/claude-sonnet-4")
            print("  laia config set terminal.backend docker")
            print("  laia config set OPENROUTER_API_KEY sk-or-...")
            sys.exit(1)
        ...
        if missing_config:
            ...
            print("    Run 'laia config migrate' to add them")
        ...
        print("  laia config           Show current configuration")
        print("  laia config edit      Open config in editor")
        print("  laia config set <key> <value>   Set a config value")
        print("  laia config check     Check for missing/outdated config")
        print("  laia config migrate   Update config with new options")
        print("  laia config path      Show config file path")
        print("  laia config env-path  Show .env file path")
        sys.exit(1)
