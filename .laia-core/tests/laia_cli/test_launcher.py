"""Tests for the top-level `./laia` launcher script."""
...
def test_launcher_delegates_to_argparse_entrypoint(monkeypatch):
    """`./laia` should use `laia_cli.main`, not the legacy Fire wrapper."""
    launcher_path = Path(__file__).resolve().parents[2] / "laia"
    called = []

    fake_main_module = types.ModuleType("laia_cli.main")

    def fake_main():
        called.append("laia_cli.main")

    fake_main_module.main = fake_main
    monkeypatch.setitem(sys.modules, "laia_cli.main", fake_main_module)
