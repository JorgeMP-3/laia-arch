...
LAIA_HOME = Path(os.environ.get("LAIA_HOME", str(Path.home() / ".laia")))
LAIA_AGENT_ROOT = LAIA_HOME / "laia-agent"
LAIA_AGENT_PYTHON = LAIA_AGENT_ROOT / "venv" / "bin" / "python"
...
def _workspace_path(name: str) -> Path:
    ...
    path = LAIA_HOME / "workspaces" / clean
    if not path.exists():
...
def _active_workspace() -> str:
    ...
    workspaces = list_workspaces(LAIA_HOME)
...
def _configured_workspace_names() -> list[str]:
    ...
    if not names:
        names = [path.name for path in list_workspaces(LAIA_HOME)]
    seen: set[str] = set()
...
class LAIAWebSession:
    ...
    async def _maybe_idle_close(self) -> None:
        try:
            seconds = float(os.environ.get("LAIA_WEB_IDLE_CLOSE_SECONDS", "0"))
        except ValueError:
            seconds = 0
        if seconds <= 0:
            return
    ...
    async def start(self) -> None:
        async with self._start_lock:
            ...
            custom = os.environ.get("LAIA_WEB_GATEWAY_CMD", "").strip()
            if custom:
                self.proc = await asyncio.create_subprocess_shell(
                    ...
                    cwd=str(LAIA_AGENT_ROOT if LAIA_AGENT_ROOT.exists() else LAIA_HOME),
                )
            else:
                python = str(LAIA_AGENT_PYTHON if LAIA_AGENT_PYTHON.exists() else sys.executable)
...
class TerminalSession:
    def __init__(
        ...
        *,
        ...
        self.cwd = str(Path(cwd).expanduser()) if cwd else str(LAIA_AGENT_ROOT if LAIA_AGENT_ROOT.exists() else LAIA_HOME)
    ...
    async def _watch_exit(self) -> None:
        ...
        try:
            ...
            await self._notify_laia_exit()

    async def _notify_laia_exit(self) -> None:
...
def _workspace_db_paths() -> list[Path]:
    ...
    for name in _configured_workspace_names():
        db_path = LAIA_HOME / "workspaces" / name / "workspace.db"
        if db_path.exists():
...
def _agent_protocol_prompt(task: str, session: TerminalSession) -> str:
    return (
        ...
        "- No uses sqlite3/Python/curl para modificar la DB general; LAIA documenta y fusiona el conocimiento final.\n"
...
def _create_terminal_approval(
    *,
    ...
    approval = {
        ...
        "requested_by": requested_by or "laia",
        "status": "pending",
        "created_at": _now(),
    }
...
app = FastAPI(title="LAIA Workspace UI", lifespan=app_lifespan)
app.add_middleware(
    CORSMiddleware,
    allow_origins=["*"],
...
async def health() -> dict[str, Any]:
    return {"ok": True, "service": "laia-ui-server", "time": _now()}
...
async def get_workspaces() -> list[dict[str, Any]]:
    ...
    for path in list_workspaces(LAIA_HOME):
...
async def create_workspace(payload: CreateWorkspacePayload) -> dict[str, Any]:
    ...
    ws_path = LAIA_HOME / "workspaces" / name
...
async def context_engine_injected() -> dict[str, Any]:
    ...
    if inject_mode == "all-indexes":
        names = list(cfg["workspaces"]) or sorted(p.name for p in list_workspaces(LAIA_HOME))
    ...
    instruction = "Usa estos nodos como contexto DB-first de LAIA."
    return {
        **cfg,
...
async def context_engine_skills() -> dict[str, Any]:
    ...
    total = 0
    for root in (LAIA_HOME / "skills", LAIA_AGENT_ROOT / "skills"):
        if not root.exists():
            continue
...
async def agent_status() -> dict[str, Any]:
    state_path = LAIA_HOME / "gateway-runtime.json"
    state: dict[str, Any] = {}
...
async def index() -> Any:
    ...
    return JSONResponse({"ok": True, "service": "laia-ui-server"})
