...
def _redirect_cache(tmp_path, monkeypatch):
    """Point document cache to tmp_path so tests don't touch ~/.laia."""
    monkeypatch.setattr(
...
class TestAppMentionHandler:
    ...
    def test_app_mention_registered_on_connect(self):
        ...
        # covering every COMMAND_REGISTRY entry (e.g. /laia, /btw, /stop,
        ...
        for expected in ("/laia", "/btw", "/stop", "/model", "/help"):
...
class TestSlashCommands:
    ...
    # instead of as /laia subcommands. This is the Discord/Telegram parity
    # fix: the slash name itself becomes the command.
    # ------------------------------------------------------------------

    @pytest.mark.asyncio
    async def test_native_btw_slash(self, adapter):
        """/btw with args must dispatch to /background, not /laia btw."""
        command = {
            "command": "/btw",
    ...
    @pytest.mark.asyncio
    async def test_legacy_laia_prefix_still_works(self, adapter):
        """Backward compat: /laia btw foo must still route to /btw foo.

        Old workspace manifests only declared /laia as the single slash.
    ...
    @pytest.mark.asyncio
    async def test_legacy_laia_freeform_question(self, adapter):
        """/laia <free-form text> must stay as the raw text (non-command)."""
        command = {
            "command": "/laia",
