"""Tests for the LAIA plugin system (laia_cli.plugins)."""
...
def _make_plugin_dir(base: Path, name: str, *, register_body: str = "pass",
    ...
    ``<laia_home>/config.yaml`` under ``plugins.enabled``. Plugins are
    ...
    *base* is expected to be ``<laia_home>/plugins/``; we derive
    ``<laia_home>`` from it by walking one level up.
    """
    ...
    if auto_enable:
        # Write/merge plugins.enabled in <LAIA_HOME>/config.yaml.
        # Config is always read from LAIA_HOME (not from the project
        # dir for project plugins), so that's where we opt in.
        import os
        laia_home_str = os.environ.get("LAIA_HOME")
        if laia_home_str:
            laia_home = Path(laia_home_str)
        else:
            laia_home = base.parent
        laia_home.mkdir(parents=True, exist_ok=True)
        cfg_path = laia_home / "config.yaml"
        cfg: dict = {}
        if cfg_path.exists():
            try:
...
class TestPluginDiscovery:
    ...
    def test_discover_user_plugins(self, tmp_path, monkeypatch):
        """Plugins in ~/.laia/plugins/ are discovered."""
        plugins_dir = tmp_path / "laia_test" / "plugins"
        _make_plugin_dir(plugins_dir, "hello_plugin")
        monkeypatch.setenv("LAIA_HOME", str(tmp_path / "laia_test"))
    ...
    def test_discover_project_plugins(self, tmp_path, monkeypatch):
        ...
        monkeypatch.setenv("LAIA_ENABLE_PROJECT_PLUGINS", "true")
        plugins_dir = project_dir / ".laia" / "plugins"
...
class TestPluginLoading:
    ...
    def test_load_registers_namespace_module(self, tmp_path, monkeypatch):
        """Directory plugins are importable under laia_plugins.<name>."""
        plugins_dir = tmp_path / "laia_test" / "plugins"
        _make_plugin_dir(plugins_dir, "ns_plugin")
        monkeypatch.setenv("LAIA_HOME", str(tmp_path / "laia_test"))

        # Clean up any prior namespace module
        sys.modules.pop("laia_plugins.ns_plugin", None)

        mgr = PluginManager()
        mgr.discover_and_load()

        assert "laia_plugins.ns_plugin" in sys.modules
...
class TestPluginHooks:
    ...
    def test_invalid_hook_name_warns(self, tmp_path, monkeypatch, caplog):
        ...
        with caplog.at_level(logging.WARNING, logger="laia_cli.plugins"):
            mgr = PluginManager()
            mgr.discover_and_load()
...
class TestPluginToolVisibility:
    ...
    def test_plugin_tools_in_definitions(self, tmp_path, monkeypatch):
        ...
        import laia_cli.plugins as plugins_mod
...
class TestPluginCommands:
    ...
    def test_get_plugin_command_handler_found(self):
        ...
        with patch("laia_cli.plugins._plugin_manager", mgr):
    ...
    def test_commands_in_list_plugins_output(self, tmp_path, monkeypatch):
        ...
        plugins_dir = tmp_path / "laia_test" / "plugins"
        # Set LAIA_HOME BEFORE _make_plugin_dir so auto-enable targets
...
class TestPluginDispatchTool:
    ...
    def test_dispatch_tool_calls_registry(self):
        ...
        with patch("laia_cli.plugins.PluginContext.dispatch_tool.__module__", "laia_cli.plugins"):
