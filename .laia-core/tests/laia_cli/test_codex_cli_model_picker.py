...
Covers:
 - Normal path (tokens already in LAIA auth store)
 - Claude Code fallback (tokens only in ~/.claude/.credentials.json)
 - Negative case (no credentials anywhere)

Note: auto-import from ~/.codex/auth.json was removed in #12360 — LAIA
now owns its own openai-codex auth state, and users explicitly adopt
existing Codex CLI tokens via `laia auth openai-codex`. The old
...
@pytest.fixture()
def laia_auth_only_env(tmp_path, monkeypatch):
    """Tokens already in LAIA auth store (no Codex CLI needed)."""
...
def test_normal_path_still_works(laia_auth_only_env):
    """openai-codex appears when tokens are already in LAIA auth store."""
...
def claude_code_only_env(tmp_path, monkeypatch):
    ...
    ~/.claude/.credentials.json (Claude Code) — not in env vars or LAIA
    auth store."""
    laia_home = tmp_path / ".laia"
    laia_home.mkdir()
    ...
    (laia_home / "auth.json").write_text(
