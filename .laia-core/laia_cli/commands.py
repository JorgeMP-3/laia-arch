"""Slash command definitions and autocomplete for the LAIA CLI.
...
COMMAND_REGISTRY: list[CommandDef] = [
    ...
    CommandDef("rollback", "List or restore filesystem checkpoints", "Session",
               args_hint="[number]"),
    CommandDef("snapshot", "Create or restore state snapshots of LAIA config/state", "Session",
    ...
    CommandDef("busy", "Control what Enter does while LAIA is working", "Configuration",
    ...
    CommandDef("update", "Update LAIA Agent to the latest version", "Info",
               gateway_only=True),
...
def _iter_plugin_command_entries() -> list[tuple[str, str, str]]:
    ...
    :func:`laia_cli.plugins.PluginContext.register_command`. They behave
    like ``CommandDef`` entries for gateway surfacing: they appear in the
    Telegram command menu, in Slack's ``/laia`` subcommand mapping, and
    ...
    """
    try:
        from laia_cli.plugins import get_plugin_commands
    except Exception:
        return []
    try:
...
def telegram_menu_commands(max_commands: int = 100) -> tuple[list[tuple[str, str]], int]:
    ...
    Skills disabled for the ``"telegram"`` platform (via ``laia skills
...
def slack_native_slashes() -> list[tuple[str, str, str]]:
    ...
    first-class slash and not a ``/laia <verb>`` subcommand.
    ...
    avoidance. ``/laia`` is always reserved as the first entry so the
    legacy ``/laia <subcommand>`` form keeps working for anything that
    ...
    # Reserve /laia as the catch-all top-level command.
    entries.append(("laia", "Talk to LAIA or run a subcommand", "[subcommand] [args]"))
    seen.add("laia")
...
def slack_app_manifest(request_url: str = "https://laia-agent.local/slack/commands") -> dict[str, Any]:
...
def slack_subcommand_map() -> dict[str, str]:
    """Return subcommand -> /command mapping for Slack /laia handler.

    Maps both canonical names and aliases so /laia bg do stuff works
    the same as /laia background do stuff.

    Plugin-registered slash commands are included so ``/laia <plugin-cmd>``
...
class SlashCommandCompleter(Completer):
    ...
    def _skin_completions(sub_text: str, sub_lower: str):
        ...
        try:
            from laia_cli.skin_engine import list_skins
            for s in list_skins():
                name = s["name"]
