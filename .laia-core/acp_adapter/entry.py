"""CLI entry point for the laia-agent ACP adapter.

Loads environment variables from ``~/.laia/.env``, configures logging
...
def _load_env() -> None:
    """Load .env from LAIA_HOME (default ``~/.laia``)."""
    from laia_cli.env_loader import load_laia_dotenv

    laia_home = get_laia_home()
    loaded = load_laia_dotenv(laia_home=laia_home)
    if loaded:
        for env_file in loaded:
            logging.getLogger(__name__).info("Loaded env from %s", env_file)
    else:
        logging.getLogger(__name__).info(
            "No .env found at %s, using system env", laia_home / ".env"
        )
...
def main() -> None:
    ...
    logger.info("Starting laia-agent ACP adapter")
    ...
    import acp
    from .server import LAIAACPAgent
