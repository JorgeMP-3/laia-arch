...
Training tasks (3):
    1. Create ~/greeting.txt with "Hello from LAIA Agent"
...
TRAIN_TASKS = [
    {
        "prompt": "Create a file at ~/greeting.txt containing exactly the text: Hello from LAIA Agent",
        "verify_path": "~/greeting.txt",
        "expected_content": "Hello from LAIA Agent",
    },
    {
...
class TerminalTestEnv(LAIAAgentBaseEnv):
    ...
    def config_init(cls) -> Tuple[TerminalTestEnvConfig, List[APIServerConfig]]:
        ...
        Claude for inference. API keys loaded from ~/laia-agent/.env.
        """
