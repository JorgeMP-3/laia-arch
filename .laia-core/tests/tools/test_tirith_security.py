...
class TestEnsureInstalled:
    ...
    def test_not_found_returns_none(self, mock_cfg):
        ...
        with patch("tools.tirith_security.shutil.which", return_value=None), \
             patch("tools.tirith_security._laia_bin_dir", return_value="/nonexistent"), \
...
class TestDiskFailureMarker:
    ...
    def test_install_failed_recovers_from_laia_bin(self):
        """After _INSTALL_FAILED, manual install in LAIA_HOME/bin is picked up."""
        from tools.tirith_security import _resolve_tirith_path, _INSTALL_FAILED
        import tempfile
        tmpdir = tempfile.mkdtemp()
        laia_bin = os.path.join(tmpdir, "tirith")
        # Create a fake executable
        with open(laia_bin, "w") as f:
            f.write("#!/bin/sh\n")
        os.chmod(laia_bin, 0o755)

        _tirith_mod._resolved_path = _INSTALL_FAILED

        with patch("tools.tirith_security.shutil.which", return_value=None), \
             patch("tools.tirith_security._laia_bin_dir", return_value=tmpdir), \
             patch("tools.tirith_security._clear_install_failed") as mock_clear:
            result = _resolve_tirith_path("tirith")
            assert result == laia_bin
            assert _tirith_mod._resolved_path == laia_bin
            mock_clear.assert_called_once()
...
class TestLAIAHomeIsolation:
    def test_laia_bin_dir_respects_laia_home(self):
        """_laia_bin_dir must use LAIA_HOME, not hardcoded ~/.laia."""
        from tools.tirith_security import _laia_bin_dir
        import tempfile
        tmpdir = tempfile.mkdtemp()
        with patch.dict(os.environ, {"LAIA_HOME": tmpdir}):
            result = _laia_bin_dir()
        assert result == os.path.join(tmpdir, "bin")
        assert os.path.isdir(result)

    def test_failure_marker_respects_laia_home(self):
        """_failure_marker_path must use LAIA_HOME, not hardcoded ~/.laia."""
        from tools.tirith_security import _failure_marker_path
        with patch.dict(os.environ, {"LAIA_HOME": "/custom/laia"}):
            result = _failure_marker_path()
        assert result == "/custom/laia/.tirith-install-failed"

    def test_conftest_isolation_prevents_real_home_writes(self):
        """The conftest autouse fixture sets LAIA_HOME; verify it's active."""
        laia_home = os.getenv("LAIA_HOME")
        assert laia_home is not None, "LAIA_HOME should be set by conftest"
        assert "laia_test" in laia_home, "Should point to test temp dir"

    def test_get_laia_home_fallback(self):
        """Without LAIA_HOME set, falls back to ~/.laia."""
        from tools.tirith_security import _get_laia_home
        with patch.dict(os.environ, {}, clear=True):
            # Remove LAIA_HOME entirely
            os.environ.pop("LAIA_HOME", None)
            result = _get_laia_home()
        assert result == os.path.join(os.path.expanduser("~"), ".laia")
