"""
LAIAAgentBaseEnv -- Abstract Base Environment for LAIA-Agent + Atropos

Provides the Atropos integration plumbing that all laia-agent environments share:
- Two-mode operation (OpenAI server for Phase 1, VLLM ManagedServer for Phase 2)
- Per-group toolset/distribution resolution
- Agent loop orchestration via LAIAAgentLoop
...
# Ensure the laia-agent repo root is on sys.path so that imports like
...
# Load API keys from laia-agent/.env so all environments can access them
...
from environments.agent_loop import AgentResult, LAIAAgentLoop
...
# Import laia-agent toolset infrastructure
...
class LAIAAgentEnvConfig(BaseEnvConfig):
    """
    Configuration for laia-agent Atropos environments.
    ...
    enabled_toolsets: Optional[List[str]] = Field(
        default=None,
        description="Explicit list of laia toolsets to enable (e.g., ['terminal', 'file', 'web']). "
    ...
    tool_call_parser: str = Field(
        ...
        "Options: laia, mistral, llama3_json, qwen, deepseek_v3, etc.",
    )
    ...
    default_result_size_chars: int = Field(
        ...
        "to sandbox. Results exceeding this are written to /tmp/laia-results/ "
...
class LAIAAgentBaseEnv(BaseEnv):
    """
    Abstract base environment for laia-agent Atropos integration.
    ...
    name: Optional[str] = "laia-agent"
    env_config_cls = LAIAAgentEnvConfig

    def __init__(
        self,
        config: LAIAAgentEnvConfig,
        ...
        # Set terminal environment variables so laia tools pick them up.
    ...
    async def compute_reward(
        ...
        - ctx: ToolContext -- call ANY laia-agent tool (terminal, file, web,
