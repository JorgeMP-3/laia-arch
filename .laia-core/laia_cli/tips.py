...
TIPS = [
    ...
    "/title \"my project\" names your session — resume it later with /resume or laia -c.",
    ...
    "Ctrl+Z suspends LAIA to the background — run fg in your shell to resume.",
    ...
    # --- CLI Flags ---
    "laia -c resumes your most recent CLI session. laia -c \"project name\" resumes by title.",
    "laia -w creates an isolated git worktree — perfect for parallel agent workflows.",
    "laia -w -q \"Fix issue #42\" combines worktree isolation with a one-shot query.",
    "laia chat -t web,terminal enables only specific toolsets for a focused session.",
    "laia chat -s github-pr-workflow preloads a skill at launch.",
    "laia chat -q \"query\" runs a single non-interactive query and exits.",
    "laia chat --max-turns 200 overrides the default 90-iteration limit per turn.",
    "laia chat --checkpoints enables filesystem snapshots before every destructive file change.",
    "laia --yolo bypasses all dangerous command approval prompts for the entire session.",
    "laia chat --source telegram tags the session for filtering in laia sessions list.",
    "laia -p work chat runs under a specific profile without changing your default.",

    # --- CLI Subcommands ---
    "laia doctor --fix diagnoses and auto-repairs config and dependency issues.",
    "laia dump outputs a compact setup summary — great for bug reports.",
    "laia config set KEY VALUE auto-routes secrets to .env and everything else to config.yaml.",
    "laia config edit opens config.yaml in your default editor.",
    "laia config check scans for missing or stale configuration options.",
    "laia sessions browse opens an interactive session picker with search.",
    "laia sessions stats shows session counts by platform and database size.",
    "laia sessions prune --older-than 30 cleans up old sessions.",
    "laia skills search react --source skills-sh searches the skills.sh public directory.",
    "laia skills check scans installed hub skills for upstream updates.",
    "laia skills tap add myorg/skills-repo adds a custom GitHub skill source.",
    "laia skills snapshot export setup.json exports your skill configuration for backup or sharing.",
    "laia mcp add github --command npx adds MCP servers from the command line.",
    "laia mcp serve runs LAIA itself as an MCP server for other agents.",
    "laia auth add lets you add multiple API keys for credential pool rotation.",
    "laia completion bash >> ~/.bashrc enables tab completion for all commands and profiles.",
    "laia logs -f follows agent.log in real time. --level WARNING --since 1h filters output.",
    "laia backup creates a zip backup of your entire LAIA home directory.",
    "laia profile create coder creates an isolated profile that becomes its own command.",
    "laia profile create work --clone copies your current config and keys to a new profile.",
    "laia update syncs new bundled skills to ALL profiles automatically.",
    "laia gateway install sets up LAIA as a system service (systemd/launchd).",
    "laia memory setup lets you configure an external memory provider (Honcho, Mem0, etc.).",
    "laia webhook subscribe creates event-driven webhook routes with HMAC validation.",
    ...
    "Set worktree: true in config.yaml to always create a git worktree (same as laia -w).",
    ...
    "LAIA_TIMEZONE overrides the server timezone with any IANA timezone string.",
    ...
    # --- Tools & Capabilities ---
    "execute_code runs Python scripts that call LAIA tools programmatically — results stay out of context.",
    ...
    "Profile names become shell commands — 'laia profile create coder' creates the 'coder' command.",
    "laia profile export coder -o backup.tar.gz creates a portable profile archive.",
    ...
    "When exiting, LAIA prints a resume command with session ID and stats.",
    "laia sessions export backup.jsonl exports all sessions for backup or analysis.",
    "laia -r SESSION_ID resumes any specific past session by its ID.",
    ...
    "laia skills install official/security/1password installs optional skills from the repo.",
    ...
    "The plan skill saves markdown plans under .laia/plans/ in the active workspace.",

    # --- Cron & Scheduling ---
    "Cron jobs can attach skills: laia cron add --skill blogwatcher \"Check for new posts\".",
    ...
    # --- Gateway & Messaging ---
    "LAIA runs on 18 platforms: Telegram, Discord, Slack, WhatsApp, Signal, Matrix, email, and more.",
    "laia gateway install sets it up as a system service that starts on boot.",
    "DingTalk uses Stream Mode — no webhooks or public URL needed.",
    "BlueBubbles brings iMessage to LAIA via a local macOS server.",
    ...
    "Context files (.laia.md, AGENTS.md) are security-scanned for prompt injection before loading.",
    ...
    "SOUL.md at ~/.laia/SOUL.md is the agent's primary identity — customize it to shape behavior.",
    "LAIA loads project context from .laia.md, AGENTS.md, CLAUDE.md, or .cursorrules (first match).",
    ...
    "MCP servers auto-generate toolsets at runtime — laia tools can toggle them per platform.",
    ...
    "Checkpoints use shadow repos in ~/.laia/checkpoints/ — your project's .git is never touched.",

    # --- Batch & Data ---
    "batch_runner.py processes hundreds of prompts in parallel for training data generation.",
    "laia chat -Q enables quiet mode for programmatic use — suppresses banner and spinner.",
    ...
    "laia plugins install owner/repo installs plugins directly from GitHub.",
    ...
    "laia acp runs LAIA as an ACP server for VS Code, Zed, and JetBrains integration.",
    "Custom providers: save named endpoints in config.yaml under custom_providers.",
    "LAIA_EPHEMERAL_SYSTEM_PROMPT injects a system prompt that's never persisted to history.",
    "credential_pool_strategies supports fill_first, round_robin, least_used, and random rotation.",
    "laia login supports OAuth-based auth for Nous and OpenAI Codex providers.",
    ...
    "laia status --deep runs deeper diagnostic checks across all components.",
    ...
    "Cron scripts live in ~/.laia/scripts/ and run before the agent — perfect for data collection pipelines.",
    "prefill_messages_file in config.yaml injects few-shot examples into every API call, never saved to history.",
    "SOUL.md completely replaces the agent's default identity — rewrite it to make LAIA your own.",
    "SOUL.md is auto-seeded with a default personality on first run. Edit ~/.laia/SOUL.md to customize.",
    ...
    "Before a gateway session reset, LAIA auto-flushes important facts to memory in the background.",
    ...
    "The skills audit log at ~/.laia/skills/.hub/audit.log tracks every install and removal operation.",
    "Stale git worktrees are auto-cleaned: 24-72h old with no unpushed commits get pruned on startup.",
    "Each profile gets its own subprocess HOME at LAIA_HOME/home/ — isolated git, ssh, npm, gh configs.",
    "LAIA_HOME_MODE env var (octal, e.g. 0701) sets custom directory permissions for web server traversal.",
    "Container mode: place .container-mode in LAIA_HOME and the host CLI auto-execs into the container.",
    "Ctrl+C has 5 priority tiers: cancel recording → cancel prompts → cancel picker → interrupt agent → exit.",
    "Every interrupt during an agent run is logged to ~/.laia/interrupt_debug.log with timestamps.",
    ...
    "Profile names cannot collide with existing PATH binaries — 'laia profile create ls' would be rejected.",
    "laia profile create backup --clone-all copies everything (config, keys, SOUL.md, memories, skills, sessions).",
    ...
    "laia claw migrate --dry-run previews OpenClaw migration without writing anything.",
    ...
    "LAIA_DEV=1 bypasses container mode detection for local development.",
    "Each MCP server gets its own toolset (mcp-servername) that can be toggled independently via laia tools.",
    "MCP ${ENV_VAR} placeholders in config are resolved at server spawn — including vars from ~/.laia/.env.",
    "Skills from trusted repos (NousResearch) get a 'trusted' security level; community skills get extra scanning.",
    "The skills quarantine at ~/.laia/skills/.hub/quarantine/ holds skills pending security review.",
]
