...
def test_openrouter_base_url_applies_or_headers(mock_openai):
    ...
    assert headers["HTTP-Referer"] == "https://laia-agent.nousresearch.com"
    assert headers["X-OpenRouter-Title"] == "LAIA Agent"
...
def test_ai_gateway_base_url_applies_attribution_headers(mock_openai):
    ...
    headers = agent._client_kwargs["default_headers"]
    assert headers["HTTP-Referer"] == "https://laia-agent.nousresearch.com"
    assert headers["X-Title"] == "LAIA Agent"
    assert headers["User-Agent"].startswith("LAIAAgent/")
