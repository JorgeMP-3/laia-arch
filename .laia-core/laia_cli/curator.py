"""CLI subcommand: `laia curator <subcommand>`.
...
def _cmd_run(args) -> int:
    ...
    if not args.synchronous:
        print("llm pass running in background — check `laia curator status` later")
    return 0
...
# ---------------------------------------------------------------------------
# argparse wiring (called from laia_cli.main)
# ---------------------------------------------------------------------------
...
def cli_main(argv=None) -> int:
    """Standalone entry (also usable by laia_cli.main fallthrough)."""
    parser = argparse.ArgumentParser(prog="laia curator")
    register_cli(parser)
