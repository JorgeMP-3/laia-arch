"""Tests for the `laia memory reset` CLI command.
...
- Profile-scoped reset (uses LAIA_HOME)
"""
...
def memory_env(tmp_path, monkeypatch):
    """Set up a fake LAIA_HOME with memory files."""
    laia_home = tmp_path / ".laia"
    memories = laia_home / "memories"
    memories.mkdir(parents=True)
    monkeypatch.setenv("LAIA_HOME", str(laia_home))

    # Create sample memory files
    (memories / "MEMORY.md").write_text(
        "§\nLAIA repo is at ~/.laia/laia-agent\n§\nUser prefers dark themes",
        encoding="utf-8",
    )
...
def _run_memory_reset(target="all", yes=False, monkeypatch=None, confirm_input="no"):
    ...
    Simulates what happens when `laia memory reset` is run.
    """
...
class TestMemoryReset:
    ...
    def test_reset_no_files_exist(self, tmp_path, monkeypatch):
        ...
        laia_home = tmp_path / ".laia"
        (laia_home / "memories").mkdir(parents=True)
        monkeypatch.setenv("LAIA_HOME", str(laia_home))
    ...
    def test_reset_profile_scoped(self, tmp_path, monkeypatch):
        """Reset should work on the active profile's LAIA_HOME."""
        ...
        monkeypatch.setenv("LAIA_HOME", str(profile_home))
    ...
    def test_reset_empty_memories_dir(self, tmp_path, monkeypatch):
        ...
        # The memories dir won't exist; get_laia_home() / "memories" won't have files
        result = _run_memory_reset(target="all", yes=True)
        assert result == "nothing"
