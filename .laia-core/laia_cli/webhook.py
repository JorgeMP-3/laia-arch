"""laia webhook — manage dynamic webhook subscriptions from the CLI.

Usage:
    laia webhook subscribe <name> [options]
    laia webhook list
    laia webhook remove <name>
    laia webhook test <name> [--payload '{"key": "value"}']

Subscriptions persist to ~/.laia/webhook_subscriptions.json and are
...
def _subscriptions_path() -> Path:
    return _laia_home() / _SUBSCRIPTIONS_FILENAME
...
def _setup_hint() -> str:
  ...
  Then start the gateway: laia gateway run
"""
...
def webhook_command(args):
    """Entry point for 'laia webhook' subcommand."""
    sub = getattr(args, "webhook_action", None)

    if not sub:
        print("Usage: laia webhook {subscribe|list|remove|test}")
        print("Run 'laia webhook --help' for details.")
        return
...
def _cmd_subscribe(args):
    ...
    print(f"  The gateway must be running to receive events (laia gateway run).\n")
...
def _cmd_list(args):
    ...
    if not subs:
        ...
        print("  Create one with: laia webhook subscribe <name>")
        return
...
def _cmd_test(args):
    ...
    payload = args.payload or '{"test": true, "event_type": "test", "message": "Hello from laia webhook test"}'
    ...
    try:
        ...
        print(f"  Error: {e}")
        print("  Is the gateway running? (laia gateway run)")
