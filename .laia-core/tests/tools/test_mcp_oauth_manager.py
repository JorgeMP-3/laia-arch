...
def test_laia_provider_subclass_exists():
    """LAIAMCPOAuthProvider is defined and subclasses OAuthClientProvider."""
    from tools.mcp_oauth_manager import _LAIA_PROVIDER_CLS
    from mcp.client.auth.oauth2 import OAuthClientProvider

    assert _LAIA_PROVIDER_CLS is not None
    assert issubclass(_LAIA_PROVIDER_CLS, OAuthClientProvider)
...
def test_manager_builds_laia_provider_subclass(tmp_path, monkeypatch):
    """get_or_build_provider returns LAIAMCPOAuthProvider, not plain OAuthClientProvider."""
    from tools.mcp_oauth_manager import (
        MCPOAuthManager, _LAIA_PROVIDER_CLS, reset_manager_for_tests,
    )
    reset_manager_for_tests()
    monkeypatch.setenv("LAIA_HOME", str(tmp_path))
    ...
    assert _LAIA_PROVIDER_CLS is not None
    assert isinstance(provider, _LAIA_PROVIDER_CLS)
    assert provider._laia_server_name == "srv"
