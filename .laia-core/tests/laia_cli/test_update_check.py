"""Tests for the update check mechanism in laia_cli.banner."""
...
def test_check_for_updates_uses_cache(tmp_path, monkeypatch):
    ...
    with patch("laia_cli.banner.subprocess.run") as mock_run:
...
def test_check_for_updates_expired_cache(tmp_path, monkeypatch):
    ...
    monkeypatch.setenv("LAIA_HOME", str(tmp_path))
    with patch("laia_cli.banner.subprocess.run", return_value=mock_result) as mock_run:
        result = check_for_updates()
...
def test_check_for_updates_no_git_dir(tmp_path, monkeypatch):
    ...
    import laia_cli.banner as banner

    # Create a fake banner.py so the fallback path also has no .git
    fake_banner = tmp_path / "laia_cli" / "banner.py"
...
def test_check_for_updates_fallback_to_project_root(tmp_path, monkeypatch):
    """Dev install: falls back to Path(__file__).parent.parent when LAIA_HOME has no git repo."""
    import laia_cli.banner as banner
    ...
    # Point LAIA_HOME at a temp dir with no laia-agent/.git
    monkeypatch.setenv("LAIA_HOME", str(tmp_path))
    with patch("laia_cli.banner.subprocess.run") as mock_run:
...
def test_invalidate_update_cache_clears_all_profiles(tmp_path):
    ...
    from laia_cli.main import _invalidate_update_cache

    # Build a fake ~/.laia with default + two named profiles
    ...
    with patch.object(Path, "home", return_value=tmp_path), \
         patch.dict(os.environ, {"LAIA_HOME": str(default_home)}):
