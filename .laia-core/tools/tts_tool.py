...
Configuration is loaded from ~/.laia/config.yaml under the 'tts:' key.
...
from tools.xai_http import laia_xai_user_agent
...
def _get_default_output_dir() -> str:
    ...
    return str(get_laia_dir("cache/audio", "audio_cache"))
...
# ===========================================================================
# Config loader -- reads tts: section from ~/.laia/config.yaml
# ===========================================================================
def _load_tts_config() -> Dict[str, Any]:
    """
    Load TTS configuration from ~/.laia/config.yaml.
    ...
    try:
        ...
        logger.debug("laia_cli.config not available, using default TTS config")
        return {}
...
def _generate_xai_tts(text: str, output_path: str, tts_config: Dict[str, Any]) -> str:
    ...
    # output_format when LAIA actually needs a non-default format/override.
    ...
    response = requests.post(
        ...
        headers={
            ...
            "User-Agent": laia_xai_user_agent(),
        },
        json=payload,
        timeout=60,
    )
...
def text_to_speech_tool(
    ...
    Reads provider/voice config from ~/.laia/config.yaml (tts: section).
    ...
    platform = get_session_env("LAIA_SESSION_PLATFORM", "").lower()
            ...
            try:
                ...
                return json.dumps({
                    ...
                    "error": "Mistral provider selected but 'mistralai' package not installed. "
                             "Run: pip install 'laia-agent[mistral]'"
                }, ensure_ascii=False)
            ...
            if not _check_neutts_available():
                return json.dumps({
                    ...
                    "error": "NeuTTS provider selected but neutts is not installed. "
                             "Run laia setup and choose NeuTTS, or install espeak-ng and run python -m pip install -U neutts[all]."
                }, ensure_ascii=False)
            ...
            try:
                ...
                return json.dumps({
                    ...
                    "error": "KittenTTS provider selected but 'kittentts' package not installed. "
                             "Run 'laia setup tts' and choose KittenTTS, or install manually: "
...
TTS_SCHEMA = {
    ...
    "parameters": {
        ...
        "properties": {
            ...
            "output_path": {
                "type": "string",
                "description": f"Optional custom file path to save the audio. Defaults to {display_laia_home()}/audio_cache/<timestamp>.mp3"
            }
        },
        "required": ["text"]
    }
