...
from laia_constants import OPENROUTER_BASE_URL, get_laia_home
...
if tinker_atropos_dir.exists():
    ...
    os.environ['LAIA_QUIET'] = '1'  # Disable temp subdirectory creation
    print(f"📂 Terminal working directory: {tinker_atropos_dir}")
else:
    # Fall back to laia-agent directory if submodule not found
...
def main(
    ...
    Args:
        ...
        model: Model to use for the agent (reads from ~/.laia/config.yaml if not provided)
    ...
    """
    # Load config from ~/.laia/config.yaml
    config = load_laia_config()
