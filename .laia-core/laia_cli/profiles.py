"""
Profile management for multiple isolated LAIA instances.

Each profile is a fully independent LAIA_HOME directory with its own
config.yaml, .env, memory, sessions, skills, gateway, cron, and logs.
Profiles live under ``~/.laia/profiles/<name>/`` by default.

The "default" profile is ``~/.laia`` itself — backward compatible,
zero migration needed.

Usage::

    laia profile create coder          # fresh profile + bundled skills
    laia profile create coder --clone  # also copy config, .env, SOUL.md
    laia profile create coder --clone-all  # full copy of source profile
    coder chat                           # use via wrapper alias
    laia -p coder chat                 # or via flag
    laia profile use coder             # set as sticky default
    laia profile delete coder          # remove profile + alias + service
"""
...
_PROFILE_DIRS = [
    ...
    # See laia_constants.get_subprocess_home() and issue #4426.
    "home",
]
...
# Directories/files to exclude when exporting the default (~/.laia) profile.
# The default profile contains infrastructure (repo checkout, worktrees, DBs,
# caches, binaries) that named profiles don't have.  We exclude those so the
# export is a portable, reasonable-size archive of actual profile data.
_DEFAULT_EXPORT_EXCLUDE_ROOT = frozenset({
    # Infrastructure
    "laia-agent",         # repo checkout (multi-GB)
...
_RESERVED_NAMES = frozenset({
    "laia", "default", "test", "tmp", "root", "sudo",
})

# LAIA subcommands that cannot be used as profile names/aliases
_LAIA_SUBCOMMANDS = frozenset({
...
def _get_profiles_root() -> Path:
    ...
    Anchored to the laia root, NOT to the current LAIA_HOME
    (which may itself be a profile).  This ensures ``coder profile list``
    can see all profiles.

    In Docker/custom deployments where LAIA_HOME points outside
    ``~/.laia``, profiles live under ``LAIA_HOME/profiles/`` so
    they persist on the mounted volume.
    """
    return _get_default_laia_home() / "profiles"


def _get_default_laia_home() -> Path:
    """Return the default (pre-profile) LAIA_HOME path.

    In standard deployments this is ``~/.laia``.
    In Docker/custom deployments where LAIA_HOME is outside ``~/.laia``
    (e.g. ``/opt/data``), returns LAIA_HOME directly.
    """
    from laia_constants import get_default_laia_root
    return get_default_laia_root()


def _get_active_profile_path() -> Path:
    """Return the path to the sticky active_profile file."""
    return _get_default_laia_home() / "active_profile"
...
def get_profile_dir(name: str) -> Path:
    """Resolve a profile name to its LAIA_HOME directory."""
    if name == "default":
        return _get_default_laia_home()
...
def check_alias_collision(name: str) -> Optional[str]:
    ...
    Checks: reserved names, laia subcommands, existing binaries in PATH.
    """
    if name in _RESERVED_NAMES:
        return f"'{name}' is a reserved name"
    if name in _LAIA_SUBCOMMANDS:
        return f"'{name}' conflicts with a laia subcommand"
...
def list_profiles() -> List[ProfileInfo]:
    ...
    # Default profile
    default_home = _get_default_laia_home()
...
def create_profile(
    ...
    if name == "default":
        raise ValueError(
            "Cannot create a profile named 'default' — it is the built-in profile (~/.laia)."
        )
...
def seed_profile_skills(profile_dir: Path, quiet: bool = False) -> Optional[dict]:
    ...
    Uses subprocess because sync_skills() caches LAIA_HOME at module level.
    ...
    try:
        result = subprocess.run(
            ...
            env={**os.environ, "LAIA_HOME": str(profile_dir)},
...
def delete_profile(name: str, yes: bool = False) -> Path:
    ...
    if name == "default":
        raise ValueError(
            "Cannot delete the default profile (~/.laia).\n"
            "To remove everything, use: laia uninstall"
        )
...
def _cleanup_gateway_service(name: str, profile_dir: Path) -> None:
    ...
    # Temporarily set LAIA_HOME so _profile_suffix resolves correctly
    old_home = os.environ.get("LAIA_HOME")
    try:
        os.environ["LAIA_HOME"] = str(profile_dir)
        from laia_cli.gateway import get_service_name, get_launchd_plist_path
        ...
        if old_home is not None:
            os.environ["LAIA_HOME"] = old_home
        elif "LAIA_HOME" in os.environ:
            del os.environ["LAIA_HOME"]
...
def set_active_profile(name: str) -> None:
    ...
    Writes to ``~/.laia/active_profile``. Use ``"default"`` to clear.
    """
    validate_profile_name(name)
    if name != "default" and not profile_exists(name):
        raise FileNotFoundError(
            f"Profile '{name}' does not exist. "
            f"Create it with: laia profile create {name}"
        )
...
def get_active_profile_name() -> str:
    """Infer the current profile name from LAIA_HOME.

    Returns ``"default"`` if LAIA_HOME is not set or points to ``~/.laia``.
    Returns the profile name if LAIA_HOME points into ``~/.laia/profiles/<name>``.
    Returns ``"custom"`` if LAIA_HOME is set to an unrecognized path.
    """
    from laia_constants import get_laia_home
    laia_home = get_laia_home()
    resolved = laia_home.resolve()

    default_resolved = _get_default_laia_home().resolve()
...
def export_profile(name: str, output_path: str) -> Path:
    ...
    if name == "default":
        # The default profile IS ~/.laia itself — its parent is ~/ and its
        # directory name is ".laia", not "default".  We stage a clean copy
...
def import_profile(archive_path: str, name: Optional[str] = None) -> Path:
    ...
    if not inferred_name:
        raise ValueError(
            ...
            "Specify it explicitly: laia profile import <archive> --name <name>"
        )
    ...
    # dir.  Importing as "default" would target ~/.laia itself — disallow
    # that and guide the user toward a named profile.
    if inferred_name == "default":
        raise ValueError(
            "Cannot import as 'default' — that is the built-in root profile (~/.laia). "
            "Specify a different name: laia profile import <archive> --name <name>"
        )
    ...
    with tempfile.TemporaryDirectory(prefix="laia_profile_import_") as tmpdir:
...
def _migrate_honcho_profile_host(old_name: str, new_name: str, new_dir: Path) -> None:
    ...
    old_host = f"laia.{old_name}"
    new_host = f"laia.{new_name}"

    candidates = [
        new_dir / "honcho.json",
        _get_default_laia_home() / "honcho.json",
...
def generate_bash_completion() -> str:
    """Generate a bash completion script for laia profile names."""
    return '''# LAIA Agent profile completion
# Add to ~/.bashrc: eval "$(laia completion bash)"

_laia_profiles() {
    local profiles_dir="$HOME/.laia/profiles"
...
_laia_completion() {
    ...
    if [[ "$prev" == "-p" || "$prev" == "--profile" ]]; then
        COMPREPLY=($(compgen -W "$(_laia_profiles)" -- "$cur"))
        return
    fi
...
complete -F _laia_completion laia
'''


def generate_zsh_completion() -> str:
    """Generate a zsh completion script for laia profile names."""
    return '''#compdef laia
# LAIA Agent profile completion
# Add to ~/.zshrc: eval "$(laia completion zsh)"

_laia() {
    local -a profiles
    profiles=(default)
    if [[ -d "$HOME/.laia/profiles" ]]; then
        profiles+=("${(@f)$(ls $HOME/.laia/profiles 2>/dev/null)}")
    fi
...
def resolve_profile_env(profile_name: str) -> str:
    """Resolve a profile name to a LAIA_HOME path string.

    Called early in the CLI entry point, before any laia modules
    are imported, to set the LAIA_HOME environment variable.
    """
    ...
    if profile_name != "default" and not profile_dir.is_dir():
        raise FileNotFoundError(
            ...
            f"Create it with: laia profile create {profile_name}"
        )
