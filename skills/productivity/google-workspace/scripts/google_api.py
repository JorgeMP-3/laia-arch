#!/usr/bin/env python3
"""Google Workspace API CLI for LAIA Agent.

Uses the Google Workspace CLI (`gws`) when available, but preserves the
existing LAIA-facing JSON contract and falls back to the Python client
...
from _laia_home import get_laia_home
...
def _gws_binary() -> str | None:
    override = os.getenv("LAIA_GWS_BIN")
    if override:
        return override
...
def main():
    parser = argparse.ArgumentParser(description="Google Workspace API for LAIA Agent")
