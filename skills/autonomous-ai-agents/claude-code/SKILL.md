...
Cuando Claude Code se ejecuta **dentro del Command Center** (`/command-center`), usa las herramientas nativas de Hermes en lugar de tmux:

```python
# 1. Lanzar sesión PTY con Claude Code
command_center_spawn(agent_type="claude-code-planner", cwd="/home/proyecto", prompt="claude .")

# 2. Inyectar prompts de seguimiento
command_center_inject(terminal_id="<id>", text="Revisa el módulo auth")
...
El `agent_type` en el Command Center es `"claude-code-planner"` (no `"claude-code"`). El comando que se ejecuta internamente es `claude` (Jorge, host) o `sclaude` (Maribel, Docker).

> **Nota:** NO uses `terminal(background=true)` ni tmux en el Command Center — esas APIs no existen en esa interfaz. Usa siempre `command_center_spawn/inject/kill`.
