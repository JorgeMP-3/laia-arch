...
class TestToolsetConsistency:
    ...
    def test_laia_platforms_share_core_tools(self):
        """All laia-* platform toolsets share the same core tools.

        Platform-specific additions (e.g. ``discord`` / ``discord_admin``
        on laia-discord, gated on DISCORD_BOT_TOKEN) are allowed on top —
        the invariant is that the core set is identical across platforms.
        """
        platforms = ["laia-cli", "laia-telegram", "laia-discord", "laia-whatsapp", "laia-slack", "laia-signal", "laia-homeassistant"]
