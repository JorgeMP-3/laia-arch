...
`mcp_tool.py`, and `laia_cli/mcp_config.py`. This module is the ONLY place
...
# ---------------------------------------------------------------------------
# LAIAMCPOAuthProvider — OAuthClientProvider subclass with disk-watch
# ---------------------------------------------------------------------------


def _make_laia_provider_class() -> Optional[type]:
    ...
    class LAIAMCPOAuthProvider(OAuthClientProvider):
        ...
        def __init__(self, *args: Any, server_name: str = "", **kwargs: Any):
            ...
            self._laia_server_name = server_name
        ...
        async def _initialize(self) -> None:
            ...
            Paired with :class:`LAIATokenStorage` persisting an absolute
                ...
                for url in build_protected_resource_metadata_discovery_urls(
                    ...
                    try:
                        ...
                        logger.debug(
                            ...
                            self._laia_server_name, url, exc,
                        )
                        continue
                ...
                for url in build_oauth_authorization_server_metadata_discovery_urls(
                    ...
                    if asm:
                        ...
                        logger.debug(
                            ...
                            "token_endpoint=%s",
                            self._laia_server_name, asm.token_endpoint,
                        )
                        break
    ...
    return LAIAMCPOAuthProvider


# Cached at import time. Tested and used by :class:`MCPOAuthManager`.
_LAIA_PROVIDER_CLS: Optional[type] = _make_laia_provider_class()
...
class MCPOAuthManager:
    ...
    def _build_provider(
        ...
        Constructs :class:`LAIAMCPOAuthProvider` directly using the helpers
        ...
        storage = LAIATokenStorage(server_name)
    ...
    def remove(self, server_name: str) -> None:
        ...
        Called by ``laia mcp remove <name>`` and (indirectly) by
        ``laia mcp login <name>`` during forced re-auth.
        """
        with self._entries_lock:
