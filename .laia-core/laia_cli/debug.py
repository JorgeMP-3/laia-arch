"""``laia debug`` — debug tools for LAIA Agent.

Currently supports:
    laia debug share    Upload debug report (system info + logs) to a
...
def _pending_file() -> Path:
    """Path to ``~/.laia/pastes/pending.json``.
...
def _save_pending(entries: list[dict]) -> None:
    ...
    try:
        ...
        # Non-fatal — worst case the user has to run ``laia debug delete``
        # manually.
        pass
...
def _sweep_expired_pastes(now: Optional[float] = None) -> tuple[int, int]:
    ...
    intended to be called from every ``laia debug`` invocation with
    minimal noise.
    """
    entries = _load_pending()
    if not entries:
...
⚠️  This will upload the following to a public paste service:
  • System info (OS, Python version, LAIA version, provider, which API keys
...
_GATEWAY_PRIVACY_NOTICE = (
    ...
    "Full logs are NOT included from the gateway — use `laia debug share` "
...
def delete_paste(url: str) -> bool:
    ...
    req = urllib.request.Request(
        ...
        headers={"User-Agent": "laia-agent/debug-share"},
    )
...
def _schedule_auto_delete(urls: list[str], delay_seconds: int = _AUTO_DELETE_SECONDS):
    ...
    every ``laia debug share`` invocation added ~20 MB of resident Python
    interpreters that never exited until the sleep completed.

    The replacement is stateless: we append to ``~/.laia/pastes/pending.json``
    and the gateway's cron ticker sweeps expired entries once per hour.
    ``laia debug share`` also runs an opportunistic sweep as a fallback
...
def _delete_hint(url: str) -> str:
    ...
    paste_id = _extract_paste_id(url)
    if paste_id:
        return f"laia debug delete {url}"
...
def _upload_paste_rs(content: str) -> str:
    ...
    req = urllib.request.Request(
        ...
        headers={
            ...
            "User-Agent": "laia-agent/debug-share",
        },
    )
    with urllib.request.urlopen(req, timeout=30) as resp:
...
def _upload_dpaste_com(content: str, expiry_days: int = 7) -> str:
    ...
    """
    boundary = "----LAIADebugBoundary9f3c"
...
def _primary_log_path(log_name: str) -> Optional[Path]:
    ...
    from laia_cli.logs import LOG_FILES

    filename = LOG_FILES.get(log_name)
    return (get_laia_home() / "logs" / filename) if filename else None
...
def _capture_dump() -> str:
    """Run ``laia dump`` and return its stdout as a string."""
...
def collect_debug_report(
    *,
    ...
    dump_text
        Pre-captured dump output.  If empty, ``laia dump`` is run
        internally.
...
def run_debug_share(args):
    ...
    print(f"To delete now:  laia debug delete <url>")

    print(f"\nShare these links with the LAIA team for support.")
...
def run_debug_delete(args):
    ...
    if not urls:
        print("Usage: laia debug delete <url> [<url> ...]")
        print("  Deletes paste.rs pastes uploaded by 'laia debug share'.")
        return
...
def run_debug(args):
    ...
    # Opportunistic sweep of expired pastes on every ``laia debug`` call.
    ...
    # best-effort — any failure is swallowed so ``laia debug`` stays
    ...
    if subcmd == "share":
        ...
        # Default: show help
        print("Usage: laia debug <command>")
        print()
        print("Commands:")
