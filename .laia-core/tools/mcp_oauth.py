...
This module provides the glue:
    - ``LAIATokenStorage``: persists tokens/client-info to disk so they
    ...
    mcp_servers:
      my_server:
        ...
        oauth:                                  # all fields optional
          ...
          client_name: "My Custom Client"       # default: "LAIA Agent"
"""
...
def _get_token_dir() -> Path:
    ...
    Uses LAIA_HOME so each profile gets its own OAuth tokens.
    Layout: ``LAIA_HOME/mcp-tokens/``
    """
    try:
        from laia_constants import get_laia_home
        base = Path(get_laia_home())
    except ImportError:
        base = Path(os.environ.get("LAIA_HOME", str(Path.home() / ".laia")))
...
# ---------------------------------------------------------------------------
# LAIATokenStorage -- persistent token/client-info on disk
# ---------------------------------------------------------------------------


class LAIATokenStorage:
    """Persist OAuth tokens and client registration to JSON files.

    File layout::

        LAIA_HOME/mcp-tokens/<server_name>.json         -- tokens
        LAIA_HOME/mcp-tokens/<server_name>.client.json   -- client info
    """
    ...
    async def get_tokens(self) -> "OAuthToken | None":
        ...
        if data is None:
            return None
        # LAIA records an absolute wall-clock ``expires_at`` alongside the
    ...
    class _Handler(BaseHTTPRequestHandler):
        def do_GET(self) -> None:  # noqa: N802
            ...
            body = (
                ...
                "<p>You can close this tab and return to LAIA.</p></body></html>"
            ) if code else (
...
def _build_client_metadata(cfg: dict) -> "OAuthClientMetadata":
    ...
    client_name = cfg.get("client_name", "LAIA Agent")
    scope = cfg.get("scope")
