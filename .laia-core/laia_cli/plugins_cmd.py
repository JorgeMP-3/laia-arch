"""``laia plugins`` CLI subcommand — install, update, remove, and list plugins.

Plugins are installed from Git repositories into ``~/.laia/plugins/``.
...
def _prompt_plugin_env_vars(manifest: dict, console) -> None:
    ...
    from laia_cli.config import get_env_value, save_env_value  # noqa: F811
    ...
    for spec in missing:
        ...
        try:
            ...
            console.print(f"\n[dim]  Skipped (you can set these later in {display_laia_home()}/.env)[/dim]")
            return

        if value:
            save_env_value(name, value)
            os.environ[name] = value
            console.print(f"  [green]✓[/green] Saved to {display_laia_home()}/.env")
        else:
            console.print(f"  [dim]  Skipped (set {name} in {display_laia_home()}/.env later)[/dim]")
...
def cmd_install(
        ...
        if target.exists():
            if not force:
                console.print(
                    ...
                    f"[bold]laia plugins update {plugin_name}[/bold] to pull latest."
                )
                sys.exit(1)
    ...
    if not (target / "plugin.yaml").exists() and not (target / "__init__.py").exists():
        console.print(
            ...
            f"or __init__.py. It may not be a valid LAIA plugin."
        )
    ...
    if should_enable:
        ...
        console.print(
            ...
            f"Run `laia plugins enable {installed_name}` to activate.[/dim]"
        )

    console.print("[dim]Restart the gateway for the plugin to take effect:[/dim]")
    console.print("[dim]  laia gateway restart[/dim]")
    console.print()
...
def _plugin_exists(name: str) -> bool:
    ...
    import laia_cli
    repo_plugins = _P(laia_cli.__file__).resolve().parent.parent / "plugins"
...
def _discover_all_plugins() -> list:
    ...
    import laia_cli
    repo_plugins = Path(laia_cli.__file__).resolve().parent.parent / "plugins"
...
def cmd_list() -> None:
    ...
    if not entries:
        ...
        console.print("[dim]Install with:[/dim] laia plugins install owner/repo")
        return
    ...
    console.print(table)
    console.print()
    console.print("[dim]Interactive toggle:[/dim] laia plugins")
    console.print("[dim]Enable/disable:[/dim] laia plugins enable/disable <name>")
...
def plugins_command(args) -> None:
    """Dispatch laia plugins subcommands."""
