"""
LAIA Agent Uninstaller.

Provides options for:
- Full uninstall: Remove everything including configs and data
- Keep data: Remove code but keep ~/.laia/ (configs, sessions, logs)
"""
...
def remove_path_from_shell_configs():
    """Remove LAIA PATH entries from shell configuration files."""
    ...
    for config_path in configs:
        try:
            ...
            # Remove lines containing laia-agent or laia PATH entries
            new_lines = []
            skip_next = False
            
            for line in content.split('\n'):
                # Skip the "# LAIA Agent" comment and following line
                if '# LAIA Agent' in line or '# laia-agent' in line:
                    skip_next = True
                    continue
                if skip_next and ('laia' in line.lower() and 'PATH' in line):
                    skip_next = False
                    continue
                skip_next = False
                
                # Remove any PATH line containing laia
                if 'laia' in line.lower() and ('PATH=' in line or 'path=' in line.lower()):
                    continue
...
def remove_wrapper_script():
    """Remove the laia wrapper script if it exists."""
    wrapper_paths = [
        Path.home() / ".local" / "bin" / "laia",
        Path("/usr/local/bin/laia"),
    ]
    ...
    for wrapper in wrapper_paths:
        if wrapper.exists():
            try:
                # Check if it's our wrapper (contains laia_cli reference)
                content = wrapper.read_text()
                if 'laia_cli' in content or 'laia-agent' in content:
                    wrapper.unlink()
...
def uninstall_gateway_service():
    ...
    - All platforms: standalone ``laia gateway run`` processes
    ...
    try:
        from laia_cli.gateway import kill_gateway_processes, find_gateway_pids
    ...
    # 3. macOS: uninstall launchd plist
        ...
        try:
            from laia_cli.gateway import get_launchd_plist_path
...
def _is_default_laia_home(laia_home: Path) -> bool:
    """Return True when ``laia_home`` points at the default (non-profile) root."""
    try:
        from laia_constants import get_default_laia_root
        return laia_home.resolve() == get_default_laia_root().resolve()
    except Exception:
        return False
...
def _discover_named_profiles():
    ...
    default root."""
    try:
        from laia_cli.profiles import list_profiles
    except Exception:
        return []
    try:
...
def _uninstall_profile(profile) -> None:
    ...
    remove its alias wrapper, and wipe its LAIA_HOME directory.

    We shell out to ``laia -p <name> gateway stop|uninstall`` because
    service names, unit paths, and plist paths are all derived from the
    current LAIA_HOME and can't be easily switched in-process.
    """
    import sys as _sys
    name = profile.name
    ...
    #    Use `python -m laia_cli.main` so we don't depend on a `laia`
    #    wrapper that may be half-removed mid-uninstall.
    laia_invocation = [_sys.executable, "-m", "laia_cli.main", "--profile", name]
    for subcmd in ("stop", "uninstall"):
        try:
            subprocess.run(
                laia_invocation + ["gateway", subcmd],
                capture_output=True,
                text=True,
                timeout=60,
                check=False,
    ...
    # 3. Wipe the profile's LAIA_HOME directory.
    try:
...
def run_uninstall(args):
    ...
    Options:
    - Full uninstall: removes code + ~/.laia/ (configs, data, logs)
    - Keep data: removes code but keeps ~/.laia/ for future reinstall
    """
    project_root = get_project_root()
    laia_home = get_laia_home()

    # Detect named profiles when uninstalling from the default root —
    # offer to clean them up too instead of leaving zombie LAIA_HOMEs
    # and systemd units behind.
    is_default_profile = _is_default_laia_home(laia_home)
    named_profiles = _discover_named_profiles() if is_default_profile else []

    print()
    print(color("┌─────────────────────────────────────────────────────────┐", Colors.MAGENTA, Colors.BOLD))
    print(color("│            ⚕ LAIA Agent Uninstaller                  │", Colors.MAGENTA, Colors.BOLD))
    print(color("└─────────────────────────────────────────────────────────┘", Colors.MAGENTA, Colors.BOLD))
    print()
    ...
    print(f"  Config:  {laia_home / 'config.yaml'}")
    print(f"  Secrets: {laia_home / '.env'}")
    print(f"  Data:    {laia_home / 'cron/'}, {laia_home / 'sessions/'}, {laia_home / 'logs/'}")
    print()
    ...
    # their alias wrappers, and wiping their LAIA_HOME dirs. Otherwise
    ...
    print()
    if full_uninstall:
        print(color("⚠️  WARNING: This will permanently delete ALL LAIA data!", Colors.RED, Colors.BOLD))
        ...
        print("This will remove the LAIA code but keep your configuration and data.")
    ...
    log_info("Removing laia command...")
    ...
    try:
        if project_root.exists():
            # If the install is inside ~/.laia/, just remove the laia-agent subdir
            if laia_home in project_root.parents or project_root.parent == laia_home:
    ...
    # 5. Optionally remove ~/.laia/ data directory (and named profiles)
    if full_uninstall:
        # 5a. Stop and remove each named profile's gateway service and
        #     alias wrapper. The profile LAIA_HOME dirs live under
        ...
        try:
            if laia_home.exists():
                shutil.rmtree(laia_home)
                log_success(f"Removed {laia_home}")
        except Exception as e:
            log_warn(f"Could not fully remove {laia_home}: {e}")
            log_info("You may need to manually remove it")
    else:
        log_info(f"Keeping configuration and data in {laia_home}")
    ...
    if not full_uninstall:
        ...
        print(color("  curl -fsSL https://raw.githubusercontent.com/NousResearch/laia-agent/main/scripts/install.sh | bash", Colors.DIM))
        print()
    
    print(color("Reload your shell to complete the process:", Colors.YELLOW))
    print("  source ~/.bashrc  # or ~/.zshrc")
    print()
    print("Thank you for using LAIA Agent! ⚕")
    print()
