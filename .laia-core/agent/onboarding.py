...
def openclaw_residue_hint_cli() -> str:
    """Banner shown the first time LAIA starts and finds ``~/.openclaw/``.

    Points users at ``laia claw migrate`` (non-destructive port of config,
    memory, and skills) first. ``laia claw cleanup`` is mentioned as the
    ...
    return (
        ...
        "To port your config, memory, and skills over to LAIA, run "
        "`laia claw migrate`.\n"
        "If you've already migrated and want to archive the old directory, "
        "run `laia claw cleanup` (renames it to ~/.openclaw.pre-migration — "
