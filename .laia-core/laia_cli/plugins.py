...
1. **Bundled plugins** – ``<repo>/plugins/<name>/`` (shipped with laia-agent;
   ``memory/`` and ``context_engine/`` subdirs are excluded — they have their
   own discovery paths)
2. **User plugins**   – ``~/.laia/plugins/<name>/``
3. **Project plugins** – ``./.laia/plugins/<name>/`` (opt-in via
   ``LAIA_ENABLE_PROJECT_PLUGINS``)
4. **Pip plugins**     – packages that expose the ``laia_agent.plugins``
   entry-point group.
...
ENTRY_POINTS_GROUP = "laia_agent.plugins"

_NS_PARENT = "laia_plugins"
...
class PluginManifest:
    ...
    # lookups and by ``laia plugins list``. For a flat plugin at
...
class PluginContext:
    ...
    def register_cli_command(
        ...
        """Register a CLI subcommand (e.g. ``laia honcho ...``).
    ...
    def register_command(
        ...
        Unlike ``register_cli_command()`` (which creates ``laia <subcommand>``
    ...
    def register_skill(
        ...
        ``~/.laia/skills/`` tree and is **not** listed in the system
...
class PluginManager:
    ...
    def discover_and_load(self, force: bool = False) -> None:
        ...
        #
        # Repo-shipped plugins live next to laia_cli/. Two layouts are
        ...
        # 2. User plugins (~/.laia/plugins/)
        user_dir = get_laia_home() / "plugins"
        manifests.extend(self._scan_directory(user_dir, source="user"))

        # 3. Project plugins (./.laia/plugins/)
        if _env_enabled("LAIA_ENABLE_PROJECT_PLUGINS"):
        ...
        for manifest in winners.values():
            ...
            # Built-in backends auto-load — they ship with laia and must
            ...
            if not is_enabled:
                loaded = LoadedPlugin(manifest=manifest, enabled=False)
                loaded.error = (
                    "not enabled in config (run `laia plugins enable {}` to activate)"
                    .format(lookup_key)
                )
                self._plugins[lookup_key] = loaded
                logger.debug(
    ...
    def _load_directory_module(self, manifest: PluginManifest) -> types.ModuleType:
        """Import a directory-based plugin as ``laia_plugins.<slug>``.

        The module slug is derived from ``manifest.key`` so category-namespaced
        plugins (``image_gen/openai``) import as
        ``laia_plugins.image_gen__openai`` without colliding with any
        future ``tts/openai``.
        """
...
def get_plugin_toolsets() -> List[tuple]:
    ...
    Used by the ``laia tools`` TUI so plugin-provided toolsets appear
