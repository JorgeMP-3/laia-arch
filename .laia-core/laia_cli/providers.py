"""
Single source of truth for provider identity in LAIA Agent.
...
2. **LAIA overlays** — transport type, auth patterns, aggregator flags,
...
# -- LAIA overlay ----------------------------------------------------------
# LAIA-specific metadata that models.dev doesn't provide.

@dataclass(frozen=True)
class LAIAOverlay:
    """LAIA-specific provider metadata layered on top of models.dev."""
...
LAIA_OVERLAYS: Dict[str, LAIAOverlay] = {
    "openrouter": LAIAOverlay(
    ...
    "openai-codex": LAIAOverlay(
        transport="codex_responses",
        auth_type="oauth_external",
        base_url_override="https://chatgpt.com/backend-api/codex",
    ),
    "qwen-oauth": LAIAOverlay(
        transport="openai_chat",
        auth_type="oauth_external",
        base_url_override="https://portal.qwen.ai/v1",
        base_url_env_var="LAIA_QWEN_BASE_URL",
    ),
    "google-gemini-cli": LAIAOverlay(
        transport="openai_chat",
        auth_type="oauth_external",
        base_url_override="cloudcode-pa://google",
    ),
    "lmstudio": LAIAOverlay(
        transport="openai_chat",
        auth_type="api_key",
    ...
    "copilot-acp": LAIAOverlay(
        transport="codex_responses",
        auth_type="external_process",
        base_url_override="acp://copilot",
        base_url_env_var="COPILOT_ACP_BASE_URL",
    ),
    "github-copilot": LAIAOverlay(
        transport="openai_chat",
        extra_env_vars=("COPILOT_GITHUB_TOKEN", "GH_TOKEN"),
    ),
    "anthropic": LAIAOverlay(
    ...
    "minimax-oauth": LAIAOverlay(
        transport="anthropic_messages",
        auth_type="oauth_external",
        base_url_override="https://api.minimax.io/anthropic",
    ),
    "minimax-cn": LAIAOverlay(
        transport="anthropic_messages",
        base_url_env_var="MINIMAX_CN_BASE_URL",
    ),
    "deepseek": LAIAOverlay(
        transport="openai_chat",
    ...
    "alibaba-coding-plan": LAIAOverlay(
        transport="openai_chat",
    ...
    "opencode": LAIAOverlay(
        transport="openai_chat",
        is_aggregator=True,
        base_url_env_var="OPENCODE_ZEN_BASE_URL",
    ),
    "opencode-go": LAIAOverlay(
        transport="openai_chat",
        is_aggregator=True,
...
class ProviderDef:
    ...
    doc: str = ""
    source: str = ""                      # "models.dev", "laia", "user-config"
...
def get_provider(name: str) -> Optional[ProviderDef]:
    ...
    Resolution order:
      1. LAIA overlays (for providers not in models.dev: nous, openai-codex, etc.)
      2. models.dev catalog + LAIA overlay
    ...
    overlay = LAIA_OVERLAYS.get(canonical)
    ...
    if mdev_info is not None:
        ...
        # Combine env vars: models.dev env + laia extra
    ...
    if overlay is not None:
        # LAIA-only provider (not in models.dev)
        return ProviderDef(
            id=canonical,
...
def determine_api_mode(provider: str, base_url: str = "") -> str:
    ...
    # Direct provider checks for providers not in LAIA_OVERLAYS
    if provider == "bedrock":
