...
def get_current_session_key(default: str = "default") -> str:
    ...
    return get_session_env("LAIA_SESSION_KEY", default)

# Sensitive write targets that should trigger approval even when referenced
# via shell expansions like $HOME or $LAIA_HOME.
_SSH_SENSITIVE_PATH = r'(?:~|\$home|\$\{home\})/\.ssh(?:/|$)'
_LAIA_ENV_PATH = (
    r'(?:~\/\.laia/|'
    r'(?:\$home|\$\{home\})/\.laia/|'
    r'(?:\$laia_home|\$\{laia_home\})/)'
    r'\.env\b'
)
...
DANGEROUS_PATTERNS = [
    ...
    (r'\blaia\s+gateway\s+(stop|restart)\b', "stop/restart laia gateway (kills running agents)"),
    (r'\blaia\s+update\b', "laia update (restarts gateway, kills running agents)"),
    # Gateway protection: never start gateway outside systemd management
    (r'gateway\s+run\b.*(&\s*$|&\s*;|\bdisown\b|\bsetsid\b)', "start gateway outside systemd (use 'systemctl --user restart laia-gateway')"),
    (r'\bnohup\b.*gateway\s+run\b', "start gateway outside systemd (use 'systemctl --user restart laia-gateway')"),
    # Self-termination protection: prevent agent from killing its own process
    (r'\b(pkill|killall)\b.*\b(laia|gateway|cli\.py)\b', "kill laia/gateway process (self-termination)"),
    # Self-termination via kill + command substitution (pgrep/pidof).
    # The name-based pattern above catches `pkill laia` but not
    # `kill -9 $(pgrep -f laia)` because the substitution is opaque
...
def check_dangerous_command(command: str, env_type: str,
    ...
    if os.getenv("LAIA_YOLO_MODE") or is_current_session_yolo_enabled():
        return {"approved": True, "message": None}
    ...
    is_cli = os.getenv("LAIA_INTERACTIVE")
    is_gateway = os.getenv("LAIA_GATEWAY_SESSION")

    if not is_cli and not is_gateway:
        # Cron sessions: respect cron_mode config
        if os.getenv("LAIA_CRON_SESSION"):
    ...
    if is_gateway or os.getenv("LAIA_EXEC_ASK"):
...
def check_all_command_guards(command: str, env_type: str,
    ...
    if os.getenv("LAIA_YOLO_MODE") or is_current_session_yolo_enabled() or approval_mode == "off":
        return {"approved": True, "message": None}

    is_cli = os.getenv("LAIA_INTERACTIVE")
    is_gateway = os.getenv("LAIA_GATEWAY_SESSION")
    is_ask = os.getenv("LAIA_EXEC_ASK")
