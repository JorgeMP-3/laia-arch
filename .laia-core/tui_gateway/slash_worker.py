"""Persistent slash-command worker — one LAIACLI per TUI session.
...
def _run(cli: LAIACLI, command: str) -> str:
...
def main():
    ...
    os.environ["LAIA_SESSION_KEY"] = args.session_key
    os.environ["LAIA_INTERACTIVE"] = "1"

    with contextlib.redirect_stdout(io.StringIO()), contextlib.redirect_stderr(io.StringIO()):
        cli = LAIACLI(model=args.model or None, compact=True, resume=args.session_key, verbose=False)
