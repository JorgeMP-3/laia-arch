...
metadata:
  laia:
    ...
    related_skills: [native-mcp, ascii-video, manim-video, laia-video]
...
```
LAIA Agent -> MCP (Streamable HTTP) -> twozero.tox (port 40404) -> TD Python
```
...
```bash
bash "${LAIA_HOME:-$HOME/.laia}/skills/creative/touchdesigner-mcp/scripts/setup.sh"
```

The script will:
1. Check if TD is running
2. Download twozero.tox if not already cached
3. Add `twozero_td` MCP server to LAIA config (if missing)
...
3. **Restart LAIA session** to pick up the new MCP server
