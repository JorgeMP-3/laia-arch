...
## 5. LAIA Configuration

### Config location

`$LAIA_HOME/config.yaml` (defaults to `~/.laia/config.yaml` when `LAIA_HOME` is unset)
...
Restart the LAIA session for changes to take effect. The MCP connection is
