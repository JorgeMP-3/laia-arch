...
def test_launch_tui_exports_model_and_provider(monkeypatch, main_mod):
    ...
    def fake_call(argv, cwd=None, env=None):
        ...
        active_path_during_call = Path(env["LAIA_TUI_ACTIVE_SESSION_FILE"])
    ...
    with pytest.raises(SystemExit):
        main_mod._launch_tui(model="nous/laia-test", provider="nous")

    env = captured["env"]
    assert env["LAIA_MODEL"] == "nous/laia-test"
    assert env["LAIA_INFERENCE_MODEL"] == "nous/laia-test"
    assert env["LAIA_TUI_PROVIDER"] == "nous"
    assert env["LAIA_INFERENCE_PROVIDER"] == "nous"
    active_path = Path(env["LAIA_TUI_ACTIVE_SESSION_FILE"])
    assert active_path.name.startswith("laia-tui-active-session-")
...
def test_print_tui_exit_summary_includes_resume_and_token_totals(monkeypatch, capsys):
    import laia_cli.main as main_mod
    ...
    monkeypatch.setitem(
        sys.modules, "laia_state", types.SimpleNamespace(SessionDB=lambda: _FakeDB())
    )
    ...
    assert "laia --tui --resume 20260409_000001_abc123" in out
    assert 'laia --tui -c "demo title"' in out
...
def test_print_tui_exit_summary_prefers_actual_active_session_file(
    ...
    assert "laia --tui --resume actual_session" in out
