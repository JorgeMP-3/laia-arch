...
# 5. Symlinks the 'laia' CLI command into a user-facing bin dir
...
echo ""
echo -e "${CYAN}⚕ LAIA Agent Setup${NC}"
echo ""
...
# ============================================================================
# PATH setup — symlink laia into a user-facing bin dir
# ============================================================================

echo -e "${CYAN}→${NC} Setting up laia command..."

LAIA_BIN="$SCRIPT_DIR/venv/bin/laia"
COMMAND_LINK_DIR="$(get_command_link_dir)"
COMMAND_LINK_DISPLAY_DIR="$(get_command_link_display_dir)"
mkdir -p "$COMMAND_LINK_DIR"
ln -sf "$LAIA_BIN" "$COMMAND_LINK_DIR/laia"
echo -e "${GREEN}✓${NC} Symlinked laia → $COMMAND_LINK_DISPLAY_DIR/laia"
    ...
    if [ -n "$SHELL_CONFIG" ]; then
        ...
        if ! echo "$PATH" | tr ':' '\n' | grep -q "^$HOME/.local/bin$"; then
            if ! grep -q '\.local/bin' "$SHELL_CONFIG" 2>/dev/null; then
                echo "" >> "$SHELL_CONFIG"
                echo "# LAIA Agent — ensure ~/.local/bin is on PATH" >> "$SHELL_CONFIG"
...
# ============================================================================
# Seed bundled skills into ~/.laia/skills/
# ============================================================================

LAIA_SKILLS_DIR="${LAIA_HOME:-$HOME/.laia}/skills"
mkdir -p "$LAIA_SKILLS_DIR"

echo ""
echo "Syncing bundled skills to ~/.laia/skills/ ..."
if "$SCRIPT_DIR/venv/bin/python" "$SCRIPT_DIR/tools/skills_sync.py" 2>/dev/null; then
    echo -e "${GREEN}✓${NC} Skills synced"
else
    # Fallback: copy if sync script fails (missing deps, etc.)
    if [ -d "$SCRIPT_DIR/skills" ]; then
        cp -rn "$SCRIPT_DIR/skills/"* "$LAIA_SKILLS_DIR/" 2>/dev/null || true
...
echo "Other commands:"
echo "  laia status        # Check configuration"
if is_termux; then
    echo "  laia gateway       # Run gateway in foreground"
else
    echo "  laia gateway install # Install gateway service (messaging + cron)"
fi
echo "  laia cron list     # View scheduled jobs"
echo "  laia doctor        # Diagnose issues"
echo ""
...
if [[ $REPLY =~ ^[Yy]$ ]] || [[ -z $REPLY ]]; then
    ...
    "$SCRIPT_DIR/venv/bin/python" -m laia_cli.main setup
fi
