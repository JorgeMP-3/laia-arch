...
# `export LAIA_REDACT_SECRETS=true`) cannot enable/disable redaction
# mid-session.  OFF by default — user must opt in via
# `security.redact_secrets: true` in config.yaml (bridged to this env var
# in laia_cli/main.py and gateway/run.py) or `LAIA_REDACT_SECRETS=true`
# in ~/.laia/.env.
_REDACT_ENABLED = os.getenv("LAIA_REDACT_SECRETS", "").lower() in ("1", "true", "yes", "on")
...
def mask_secret(
    ...
    *,
    ...
    Canonical helper for display-time redaction across LAIA — used by
    ``laia config``, ``laia status``, ``laia dump``, and anywhere
