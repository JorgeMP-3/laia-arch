...
TTS provider tools must resolve API keys from ``~/.laia/.env`` (via
``laia_cli.config.get_env_value``) and not only from ``os.environ`` —
...
already addressed for ``agent/credential_pool`` and ``laia_cli/auth``.
"""
...
class TestDotenvFallbackPerProvider:
    """For each affected provider, when only ``~/.laia/.env`` carries the
    ...
    ``laia_cli.config.load_env`` integration. Before the fix, ``os.getenv``
...
class TestRegressionGuard:
    ...
    ``laia_cli.config.load_env`` to simulate ``~/.laia/.env`` carrying the
    ...
    def test_check_tts_requirements_sees_dotenv_minimax(self, monkeypatch):
        ...
        in ``~/.laia/.env``, even though the dispatcher would later succeed.
        """
        from tools import tts_tool
