...
assert "~/.laia/ swept to live LAIA_HOME"     "$(grep -q '~/.laia/' "$cfg" && echo 1 || echo 0)"
...
assert "state_db points to live LAIA_HOME"    "$(grep -Fq "  state_db: \${LAIA_HOME:-$LAIA_HOME_OVERRIDE}/state.db" "$cfg" && echo 0 || echo 1)"
