...
One ``CDPSupervisor`` runs per LAIA ``task_id`` that has a reachable CDP
...
DIALOG_BRIDGE_HOST = "laia-dialog-bridge.invalid"
...
(() => {
  if (window.__laiaDialogBridgeInstalled) return;
  window.__laiaDialogBridgeInstalled = true;
  const ENDPOINT = "http://laia-dialog-bridge.invalid/";
