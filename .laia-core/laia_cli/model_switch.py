...
- ``laia_cli.providers``        -- canonical provider identity + overlays
- ``laia_cli.model_normalize``  -- per-provider name formatting
...
from laia_cli.providers import (
...
_LAIA_MODEL_WARNING = (
    "Nous Research LAIA 3 & 4 models are NOT agentic and are not designed "
    "for use with LAIA Agent. They lack the tool-calling capabilities "
    "required for agent workflows. Consider using an agentic model instead "
    "(Claude, GPT, Gemini, DeepSeek, etc.)."
)

# Match only the real Nous Research LAIA 3 / LAIA 4 chat families.
# The previous substring check (`"laia" in name.lower()`) false-positived on
# unrelated local Modelfiles like ``laia-brain:qwen3-14b-ctx16k`` that just
# happen to carry "laia" in their tag but are fully tool-capable.
#
# Positive examples the regex must match:
#   NousResearch/LAIA-3-Llama-3.1-70B, laia-4-405b, openrouter/laia3:70b
# Negative examples it must NOT match:
#   laia-brain:qwen3-14b-ctx16k, qwen3:14b, claude-opus-4-6
_NOUS_LAIA_NON_AGENTIC_RE = re.compile(
    r"(?:^|[/:])laia[-_ ]?[34](?:[-_.:]|$)",
    re.IGNORECASE,
)


def is_nous_laia_non_agentic(model_name: str) -> bool:
    """Return True if *model_name* is a real Nous LAIA 3/4 chat model.
    ...
    if not model_name:
        return False
    return bool(_NOUS_LAIA_NON_AGENTIC_RE.search(model_name))


def _check_laia_model_warning(model_name: str) -> str:
    """Return a warning string if *model_name* is a Nous LAIA 3/4 chat model."""
    if is_nous_laia_non_agentic(model_name):
        return _LAIA_MODEL_WARNING
    return ""
...
def _ensure_direct_aliases() -> None:
    ...
    the module attribute. This keeps `from laia_cli.model_switch import
...
def switch_model(
    ...
    if explicit_provider:
        ...
        if pdef is None:
            _switch_err = (
                ...
                f"Check 'laia model' for available providers, or define it "
            ...
            try:
                ...
                if _cfg_issues:
                    _switch_err += "\n\nRun 'laia doctor' — config issues detected:"
    ...
    # Mirrors the same logic in laia_cli.runtime_provider.resolve_runtime_provider;
    ...
    laia_warn = _check_laia_model_warning(new_model)
    if laia_warn:
        warnings.append(laia_warn)
...
def list_authenticated_providers(
    ...
    Uses the curated model lists from laia_cli/models.py (OPENROUTER_MODELS,
    ...
    def _record_builtin_endpoint(slug: str) -> None:
        ...
        try:
            from laia_cli.auth import PROVIDER_REGISTRY as _reg
        except Exception:
            return
        pcfg = _reg.get(slug)
        if not pcfg:
    ...
    # Build curated model lists keyed by laia provider ID
    ...
    # --- 1. Check LAIA-mapped providers ---
    for laia_id, mdev_id in PROVIDER_TO_MODELS_DEV.items():
        ...
        pconfig = PROVIDER_REGISTRY.get(laia_id)
        # Skip non-API-key auth providers here — they are handled in
        # section 2 (LAIA_OVERLAYS) with proper auth store checking.
        ...
        if not has_creds:
            try:
                from laia_cli.auth import _load_auth_store
                store = _load_auth_store()
                if store and laia_id in store.get("credential_pool", {}):
                    has_creds = True
            except Exception:
                pass
        if not has_creds:
        ...
        # show up in the picker without requiring a LAIA release.
        model_ids = curated.get(laia_id, [])
        if laia_id in _MODELS_DEV_PREFERRED:
            model_ids = _merge_with_models_dev(laia_id, model_ids)
        total = len(model_ids)
    ...
    # --- 2. Check LAIA-only providers (nous, openai-codex, copilot, opencode-go) ---
    from laia_cli.providers import LAIA_OVERLAYS
    from laia_cli.auth import PROVIDER_REGISTRY as _auth_registry

    # Build reverse mapping: models.dev ID → LAIA provider ID.
    # LAIA_OVERLAYS keys may be models.dev IDs (e.g. "github-copilot")
    # while _PROVIDER_MODELS and config.yaml use LAIA IDs ("copilot").
    _mdev_to_laia = {v: k for k, v in PROVIDER_TO_MODELS_DEV.items()}

    for pid, overlay in LAIA_OVERLAYS.items():
        if pid.lower() in seen_slugs:
            continue

        # Resolve LAIA slug — e.g. "github-copilot" → "copilot"
        laia_slug = _mdev_to_laia.get(pid, pid)
        if laia_slug.lower() in seen_slugs:
            continue
        ...
        if not has_creds:
            try:
                ...
                if store and (
                    pid in providers_store or laia_slug in providers_store
                    or pid in pool_store or laia_slug in pool_store
                ):
                    has_creds = True
        ...
        if not has_creds:
            try:
                ...
                pool = load_pool(laia_slug)
                if pool.has_credentials():
                    has_creds = True
            except Exception as exc:
                logger.debug("Credential pool check failed for %s: %s", laia_slug, exc)
        ...
        # configured.
        if not has_creds and laia_slug == "anthropic":
            try:
                ...
                if (laia_creds and laia_creds.get("accessToken")) or \
        ...
        if laia_slug in {"copilot", "copilot-acp"}:
            model_ids = provider_model_ids(laia_slug)
        ...
        # reflects the active region (eu.*, ap.*) not the static us.* list.
            ...
            try:
                ...
                model_ids = _ids if _ids is not None else (curated.get(laia_slug, []) or curated.get(pid, []))
            except Exception:
                model_ids = curated.get(laia_slug, []) or curated.get(pid, [])
        else:
            # Use curated list — look up by LAIA slug, fall back to overlay key
            model_ids = curated.get(laia_slug, []) or curated.get(pid, [])
            # Merge with models.dev for preferred providers (same rationale as above).
            if laia_slug in _MODELS_DEV_PREFERRED:
                model_ids = _merge_with_models_dev(laia_slug, model_ids)
        total = len(model_ids)
        top = model_ids[:max_models]

        results.append({
            "slug": laia_slug,
            "name": get_label(laia_slug),
            "is_current": laia_slug == current_provider or pid == current_provider,
            "is_user_defined": False,
            "models": top,
            "total_models": total,
            "source": "laia",
        ...
        seen_slugs.add(laia_slug.lower())
        _record_builtin_endpoint(laia_slug)

    # --- 2b. Cross-check canonical provider list ---
    # Catches providers that are in CANONICAL_PROVIDERS but weren't found
    # in PROVIDER_TO_MODELS_DEV or LAIA_OVERLAYS (keeps /model in sync
    # with `laia model`).
    try:
        from laia_cli.models import CANONICAL_PROVIDERS as _canon_provs
    except ImportError:
        _canon_provs = []
    ...
    if user_providers and isinstance(user_providers, dict):
        for ep_name, ep_cfg in user_providers.items():
            ...
            # ``base_url`` is LAIA's canonical write key (matches
            ...
            # LAIA writes ``models:`` as a dict keyed by model id
            # (see laia_cli/main.py::_save_custom_provider); older
    ...
    if custom_providers and isinstance(custom_providers, list):
        ...
        for entry in custom_providers:
            ...
            if group_key not in groups:
                ...
                # LAIA's own writer uses; a hyphen variant is accepted
            ...
            # active model. LAIA's own writer (main.py::_save_custom_provider)
            # stores every configured model as a dict under ``models:``;
            # downstream readers (agent/models_dev.py, gateway/run.py,
            # run_agent.py, laia_cli/config.py) already consume that dict.
