...
GATEWAY_SECRET_CAPTURE_UNSUPPORTED_MESSAGE = (
    ...
    "Load this skill in the local CLI to be prompted, or add the key to ~/.laia/.env manually."
)
...
# Default location: {LAIA_HOME}/cache/images/ (legacy: image_cache/)
IMAGE_CACHE_DIR = get_laia_dir("cache/images", "image_cache")
...
AUDIO_CACHE_DIR = get_laia_dir("cache/audio", "audio_cache")
...
VIDEO_CACHE_DIR = get_laia_dir("cache/videos", "video_cache")
...
DOCUMENT_CACHE_DIR = get_laia_dir("cache/documents", "document_cache")
...
_PLAINTEXT_GATEWAY_RESTART_PATTERNS: tuple[re.Pattern[str], ...] = (
    ...
    re.compile(r"^(?:please\s+)?restart\s+(?:the\s+)?laia\s+gateway[.!?\s]*$", re.IGNORECASE),
    re.compile(r"^(?:please\s+)?restart\s+laia[.!?\s]*$", re.IGNORECASE),
)
...
class BasePlatformAdapter(ABC):
    ...
    async def handle_message(self, event: MessageEvent) -> None:
        ...
        if session_key in self._active_sessions:
            ...
            from laia_cli.commands import should_bypass_active_session
    ...
    def _get_human_delay() -> float:
        ...
        Reads from env vars:
          LAIA_HUMAN_DELAY_MODE: "off" (default) | "natural" | "custom"
          LAIA_HUMAN_DELAY_MIN_MS: minimum delay in ms (default 800, custom mode)
          LAIA_HUMAN_DELAY_MAX_MS: maximum delay in ms (default 2500, custom mode)
        """
        mode = os.getenv("LAIA_HUMAN_DELAY_MODE", "off").lower()
        if mode == "off":
            return 0.0
        min_ms = int(os.getenv("LAIA_HUMAN_DELAY_MIN_MS", "800"))
        max_ms = int(os.getenv("LAIA_HUMAN_DELAY_MAX_MS", "2500"))
        if mode == "natural":
    ...
    async def _process_message_background(self, event: MessageEvent, session_key: str) -> None:
        ...
        callback_generation = getattr(interrupt_event, "_laia_run_generation", None)
