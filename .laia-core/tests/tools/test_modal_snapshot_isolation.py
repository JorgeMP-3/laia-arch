...
def _restore_tool_modules():
    original_laia_home = os.environ.get("LAIA_HOME")
    original_modules = {
        name: module
    ...
    try:
        ...
        if original_laia_home is None:
            os.environ.pop("LAIA_HOME", None)
        else:
            os.environ["LAIA_HOME"] = original_laia_home
        _reset_modules(("tools", "laia_cli", "modal"))
...
def _install_modal_test_modules(
    ...
    *,
    ...
    laia_cli = types.ModuleType("laia_cli")
    laia_cli.__path__ = []  # type: ignore[attr-defined]
    sys.modules["laia_cli"] = laia_cli
    laia_home = tmp_path / "laia-home"
    os.environ["LAIA_HOME"] = str(laia_home)
    sys.modules["laia_cli.config"] = types.SimpleNamespace(
        get_laia_home=lambda: laia_home,
    )
    ...
    async def _lookup_aio(_name: str, create_if_missing: bool = False):
        return types.SimpleNamespace(name="laia-agent", create_if_missing=create_if_missing)
    ...
    return {
        "snapshot_store": laia_home / "modal_snapshots.json",
