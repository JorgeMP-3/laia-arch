...
## 5. GODMODE FAST — LAIA 4 405B (Zero Refusal)

**Model:** `nousresearch/laia-4-405b`
**Strategy:** LAIA models are already uncensored, so this combo is about speed — instant token streaming with zero refusal checking needed. Uses the same Pliny Love divider format as a formality.
...
## Using with LAIA Agent

### As ephemeral system prompt (config.yaml)

Pick a template above and set it in `~/.laia/config.yaml`:
