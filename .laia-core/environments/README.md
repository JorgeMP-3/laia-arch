# LAIA-Agent Atropos Environments

This directory contains the integration layer between **laia-agent's** tool-calling capabilities and the **Atropos** RL training framework. It provides everything needed to run agentic LLMs through multi-turn tool-calling loops, score their output with arbitrary reward functions, and feed results into Atropos for training or evaluation.
...
```
                    ...
                    └───────────┬───────────┘
                                │ inherits
                    ┌───────────┴───────────┐
                    │  LAIAAgentBaseEnv    │  laia_base_env.py
                    │  - Terminal backend    │
                    │  - Tool resolution     │
                    │  - Agent loop          │
                    │  - ToolContext          │
              ...
              ┌─────────────────┼─────────────────┐
              │                 │                  │
     TerminalTestEnv     LAIASweEnv    TerminalBench2EvalEnv
...
**LAIAAgentBaseEnv** (`laia_base_env.py`) extends BaseEnv with laia-agent specifics:
- Sets `os.environ["TERMINAL_ENV"]` to configure the terminal backend (local, docker, modal, daytona, ssh, singularity)
- Resolves laia-agent toolsets via `_resolve_tools_for_group()` (calls `get_tool_definitions()` which queries `tools/registry.py`)
- Implements `collect_trajectory()` which runs the full agent loop and computes rewards
- Supports two-phase operation (Phase 1: OpenAI server, Phase 2: VLLM ManagedServer)
- Applies monkey patches for async-safe tool operation at import time

Concrete environments inherit from `LAIAAgentBaseEnv` and implement:
...
`LAIAAgentLoop` is the reusable multi-turn agent engine. It runs the same pattern as laia-agent's `run_agent.py`:
...
`ToolContext` is a per-rollout handle that gives reward/verification functions direct access to **all** laia-agent tools, scoped to the rollout's `task_id`. The same `task_id` means the terminal/browser session is the SAME one the model used during its rollout -- all state (files, processes, browser tabs) is preserved.
...
- **Generic**: `call_tool(name, args)` -- call any laia-agent tool by name
- **Cleanup**: `cleanup()` -- release all resources (called automatically after `compute_reward`)

### Patches (`patches.py`)

**Problem**: Some laia-agent tools use `asyncio.run()` internally (e.g., the Modal backend). This crashes when called from inside Atropos's event loop because `asyncio.run()` cannot be nested.
...
Available parsers:
- `laia` -- LAIA/ChatML `<tool_call>` XML format
...
├── laia_base_env.py            # Abstract base (LAIAAgentBaseEnv)
├── agent_loop.py                 # Multi-turn agent engine (LAIAAgentLoop)
...
│
├── laia_swe_env/               # SWE-bench style training environment
│   └── laia_swe_env.py
│
...
### LAIASweEnv (`laia_swe_env/`)

SWE-bench style training environment. The model gets a coding task, uses terminal + file + web tools to solve it, and the reward function runs tests in the same Modal sandbox.

```bash
python environments/laia_swe_env/laia_swe_env.py serve \
...
2. Create your env file inheriting from `LAIAAgentBaseEnv`
3. Implement the four abstract methods + `evaluate()`

```python
from environments.laia_base_env import LAIAAgentBaseEnv, LAIAAgentEnvConfig

class MyEnvConfig(LAIAAgentEnvConfig):
    pass  # Add custom fields as needed

class MyEnv(LAIAAgentBaseEnv):
    name = "my-env"
...
2. Inherit from `LAIAAgentBaseEnv`
...
|-------|-------------|---------|
| `enabled_toolsets` | Which laia toolsets to enable | `None` (all) |
...
| `tool_call_parser` | Parser name for Phase 2 | `laia` |
