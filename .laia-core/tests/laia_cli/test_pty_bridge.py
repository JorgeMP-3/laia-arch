"""Unit tests for laia_cli.pty_bridge — PTY spawning + byte forwarding.
...
class TestPtyBridgeIO:
    def test_reads_child_stdout(self):
        bridge = PtyBridge.spawn(["/bin/sh", "-c", "printf laia-ok"])
        try:
            output = _read_until(bridge, b"laia-ok")
            assert b"laia-ok" in output
        finally:
            bridge.close()
...
class TestPtyBridgeEnv:
    ...
    def test_env_is_forwarded(self):
        bridge = PtyBridge.spawn(
            ["/bin/sh", "-c", "printf %s \"$LAIA_PTY_TEST\""],
            env={**os.environ, "LAIA_PTY_TEST": "pty-env-works"},
        )
        try:
