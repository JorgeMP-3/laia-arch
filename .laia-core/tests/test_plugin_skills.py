...
- laia_cli/plugins register_skill API + registry
...
class TestPluginSkillRegistry:
    ...
    def pm(self, monkeypatch):
        from laia_cli import plugins as plugins_mod
        from laia_cli.plugins import PluginManager
...
class TestPluginContextRegisterSkill:
    ...
    def ctx(self, tmp_path, monkeypatch):
        from laia_cli import plugins as plugins_mod
        from laia_cli.plugins import PluginContext, PluginManager, PluginManifest
...
class TestSkillViewPluginGuards:
    ...
    def test_disabled_plugin(self, tmp_path, monkeypatch):
        ...
        self._reg(tmp_path, "---\nname: foo\n---\nBody.\n")
        monkeypatch.setattr("laia_cli.plugins._get_disabled_plugins", lambda: {"myplugin"})
