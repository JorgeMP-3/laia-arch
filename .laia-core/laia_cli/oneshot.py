...
Toolsets = whatever the user has configured for "cli" in `laia tools`.
Rules / memory / AGENTS.md / preloaded skills = same as a normal chat turn.
Approvals = auto-bypassed (LAIA_YOLO_MODE=1 is set for the call).
Working directory = the user's CWD (AGENTS.md etc. resolve from there as usual).

Model / provider selection mirrors `laia chat`:
...
Env var fallbacks (used when the corresponding arg is not passed):
    - LAIA_INFERENCE_MODEL
    - LAIA_INFERENCE_PROVIDER  (already read by resolve_runtime_provider)
"""
...
def run_oneshot(
    ...
    Args:
        ...
        model: Optional model override. Falls back to LAIA_INFERENCE_MODEL
            env var, then config.yaml's model.default / model.model.
        provider: Optional provider override. Falls back to
            LAIA_INFERENCE_PROVIDER env var, then config.yaml's model.provider,
            then "auto".
    ...
    env_model_early = os.getenv("LAIA_INFERENCE_MODEL", "").strip()
    if provider and not ((model or "").strip() or env_model_early):
        sys.stderr.write(
            "laia -z: --provider requires --model (or LAIA_INFERENCE_MODEL). "
    ...
    os.environ["LAIA_ACCEPT_HOOKS"] = "1"
...
def _run_agent(
    ...
    # Imports are local so they don't run when laia is invoked for
    # other commands (keeps top-level CLI startup cheap).
    from laia_cli.config import load_config
    from laia_cli.models import detect_provider_for_model
    ...
    env_model = os.getenv("LAIA_INFERENCE_MODEL", "").strip()
    ...
    if effective_provider is None and (model or env_model):
        ...
        if explicit_model:
            ...
            try:
                from laia_cli import model_switch as _ms
            ...
            if direct is not None:
                ...
                current_provider = (
                    cfg_provider
                    or os.getenv("LAIA_INFERENCE_PROVIDER", "").strip().lower()
                    or "auto"
                )
    ...
    agent = AIAgent(
        ...
        #                LAIA_INTERACTIVE which we never set
        #   - shell-hook approval → auto-approved via LAIA_ACCEPT_HOOKS=1
        #                (set above); also falls back to deny on non-tty
        #   - dangerous-command approval → bypassed via LAIA_YOLO_MODE=1
