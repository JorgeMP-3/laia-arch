...
def _isolate(tmp_path, monkeypatch):
    """Isolate LAIA_HOME so tests don't touch real config."""
    home = tmp_path / ".laia"
    home.mkdir()
...
def cli_obj(_isolate):
    """Create a minimal LAIACLI instance for banner testing."""
    with patch("cli.load_cli_config", return_value={
        ...
        obj = LAIACLI.__new__(LAIACLI)
        obj.model = "test-model"
        obj.enabled_toolsets = ["laia-core"]
        obj.compact = False
