# LAIA Agent Has Had "Routines" Since March
...
LAIA equivalent — works today:
```bash
laia cron create "0 2 * * *" \
...
LAIA equivalent — works today:
```bash
laia webhook subscribe auth-watch \
  --events "pull_request" \
...
LAIA equivalent — works today:
```bash
laia webhook subscribe alert-triage \
...
Every use case in their blog post — backlog triage, docs drift, deploy verification, alert correlation, library porting, bespoke PR review — has a working LAIA implementation. No new features needed. It's been shipping since March 2026.

---

## What's Different

| | Claude Code Routines | LAIA Agent |
|---|---|---|
...
## Things LAIA Does That Routines Can't
...
LAIA has no daily limit. Run 500 automations a day if you want. The only constraint is your API budget, and you choose which models to use for which tasks.
...
LAIA Agent is open source and free. The automation infrastructure — cron scheduler, webhook platform, skill system, multi-platform delivery — is built in.

```bash
pip install laia-agent
laia setup
```

Set up a scheduled task in 30 seconds:
```bash
laia cron create "0 9 * * 1" \
...
```bash
laia gateway setup    # enable webhooks
laia webhook subscribe pr-review \
  --events "pull_request" \
...
Full automation templates gallery: [laia-agent.nousresearch.com/docs/guides/automation-templates](https://laia-agent.nousresearch.com/docs/guides/automation-templates)

Documentation: [laia-agent.nousresearch.com](https://laia-agent.nousresearch.com)

GitHub: [github.com/NousResearch/laia-agent](https://github.com/NousResearch/laia-agent)

---

*LAIA Agent is built by [Nous Research](https://nousresearch.com). Open source, model-agnostic, runs on your infrastructure.*
