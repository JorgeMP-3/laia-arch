...
class TestWorktreeCreation:
    ...
    def test_creates_worktree(self, git_repo):
        ...
        assert info["branch"].startswith("laia/laia-")
...
class TestOrphanedBranchPruning:
    """Test cleanup of orphaned laia/* and pr-* branches."""

    def test_prunes_orphaned_laia_branch(self, git_repo):
        """laia/laia-* branches with no worktree should be deleted."""
        # Create a branch that looks like a worktree branch but has no worktree
        subprocess.run(
            ["git", "branch", "laia/laia-deadbeef", "HEAD"],
            cwd=str(git_repo), capture_output=True,
        )

        # Verify it exists
        result = subprocess.run(
            ["git", "branch", "--list", "laia/laia-deadbeef"],
            capture_output=True, text=True, cwd=str(git_repo),
        )
        assert "laia/laia-deadbeef" in result.stdout
        ...
        assert "laia/laia-deadbeef" in orphaned
        ...
        assert "laia/laia-deadbeef" not in result.stdout
