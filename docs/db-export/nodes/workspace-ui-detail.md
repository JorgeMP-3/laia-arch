# Workspace UI (Detailed)

## Metadata

- ID: `89`
- Slug: `workspace-ui-detail`
- Kind: `doc`
- Status: `active`
- Filename: `workspace-ui-detail.md`
- Parent: `workspace-ui-area`
- Source kind: `manual`
- Created at: `2026-05-08T08:34:00.218963+00:00`
- Updated at: `2026-05-19T11:33:14.566183+00:00`
- Aliases: `workspace-ui-detail`

## Summary

**FastAPI backend + React frontend** for browsing, editing, and visualizing the Hermes knowledge gra
...
FastAPI backend  (port 8077)
    ...
    │  WorkspaceStore API
    ▼
SQLite database  (one .db file per workspace at ~/.laia/workspaces/{ws}/workspace.db)
```
...
5. FastAPI receives the request, validates the workspace name, creates a `WorkspaceStore` instance pointing at `~/.laia/workspaces/{ws}/`
...
```bash
cd ~/.laia/workspace-ui/frontend
npm install
npm run build
```

The output goes to `~/.laia/workspace-ui/frontend/dist/`. The backend serves it from there.
...
| 404 on `/api/workspaces/{ws}/nodes` | The workspace directory must exist at `~/.laia/workspaces/{ws}/` |
