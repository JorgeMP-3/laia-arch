"""
LAIA MCP Server — expose messaging conversations as MCP tools.
...
Plus: channels_list (LAIA-specific extra)

Usage:
    laia mcp serve
    laia mcp serve --verbose
...
logger = logging.getLogger("laia.mcp_serve")
...
def _get_sessions_dir() -> Path:
    """Return the sessions directory using LAIA_HOME."""
    try:
        from laia_constants import get_laia_home
        return get_laia_home() / "sessions"
    except ImportError:
        return Path(os.environ.get("LAIA_HOME", Path.home() / ".laia")) / "sessions"
...
def _load_channel_directory() -> dict:
    ...
    try:
        from laia_constants import get_laia_home
        directory_file = get_laia_home() / "channel_directory.json"
    except ImportError:
        directory_file = Path(
            os.environ.get("LAIA_HOME", Path.home() / ".laia")
...
class EventBridge:
    ...
    This is the LAIA equivalent of OpenClaw's WebSocket gateway bridge.
    ...
    def _poll_once(self, db):
        ...
        try:
            from laia_constants import get_laia_home
            db_file = get_laia_home() / "state.db"
        except ImportError:
            db_file = Path(os.environ.get("LAIA_HOME", Path.home() / ".laia")) / "state.db"
...
def create_mcp_server(event_bridge: Optional[EventBridge] = None) -> "FastMCP":
    """Create and return the LAIA MCP server with all tools registered."""
    ...
    mcp = FastMCP(
        "laia",
        instructions=(
            "LAIA Agent messaging bridge. Use these tools to interact with "
...
def run_mcp_server(verbose: bool = False) -> None:
    """Start the LAIA MCP server on stdio."""
    if not _MCP_SERVER_AVAILABLE:
        print(
