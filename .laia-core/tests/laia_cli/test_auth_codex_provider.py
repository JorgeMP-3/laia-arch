"""Tests for Codex auth — tokens stored in LAIA auth store (~/.laia/auth.json)."""
...
def _setup_laia_auth(laia_home: Path, *, access_token: str = "access", refresh_token: str = "refresh"):
    """Write Codex tokens into the LAIA auth store."""
...
def test_read_codex_tokens_missing(tmp_path, monkeypatch):
    ...
    laia_home.mkdir(parents=True, exist_ok=True)
    # Empty auth store
    (laia_home / "auth.json").write_text(json.dumps({"version": 1, "providers": {}}))
    monkeypatch.setenv("LAIA_HOME", str(laia_home))
...
def test_resolve_codex_runtime_credentials_missing_access_token(tmp_path, monkeypatch):
    laia_home = tmp_path / "laia"
    _setup_laia_auth(laia_home, access_token="")
    monkeypatch.setenv("LAIA_HOME", str(laia_home))
...
def test_resolve_codex_runtime_credentials_refreshes_expiring_token(tmp_path, monkeypatch):
    ...
    _setup_laia_auth(laia_home, access_token=expiring_token, refresh_token="refresh-old")
    monkeypatch.setenv("LAIA_HOME", str(laia_home))
    ...
    monkeypatch.setattr("laia_cli.auth._refresh_codex_auth_tokens", _fake_refresh)
...
def test_resolve_codex_runtime_credentials_force_refresh(tmp_path, monkeypatch):
    laia_home = tmp_path / "laia"
    _setup_laia_auth(laia_home, access_token="access-current", refresh_token="refresh-old")
    monkeypatch.setenv("LAIA_HOME", str(laia_home))
...
def test_codex_tokens_not_written_to_shared_file(tmp_path, monkeypatch):
    """Verify _save_codex_tokens writes only to LAIA auth store, not ~/.codex/."""
    laia_home = tmp_path / "laia"
    codex_home = tmp_path / "codex-cli"
    laia_home.mkdir(parents=True, exist_ok=True)
    codex_home.mkdir(parents=True, exist_ok=True)
    ...
    _save_codex_tokens({"access_token": "laia-at", "refresh_token": "laia-rt"})

    # ~/.codex/auth.json should NOT exist — _save_codex_tokens only touches LAIA store
    assert not (codex_home / "auth.json").exists()

    # LAIA auth store should have the tokens
    data = _read_codex_tokens()
    assert data["tokens"]["access_token"] == "laia-at"


def test_resolve_returns_laia_auth_store_source(tmp_path, monkeypatch):
    laia_home = tmp_path / "laia"
    _setup_laia_auth(laia_home)
    monkeypatch.setenv("LAIA_HOME", str(laia_home))

    creds = resolve_codex_runtime_credentials()
    assert creds["source"] == "laia-auth-store"
...
def _patch_httpx(monkeypatch, response):
    ...
    monkeypatch.setattr("laia_cli.auth.httpx.Client", _factory)
...
def test_login_openai_codex_force_new_login_skips_existing_reuse_prompt(monkeypatch):
    ...
    monkeypatch.setattr(
        "laia_cli.auth.resolve_codex_runtime_credentials",
        lambda: {"base_url": DEFAULT_CODEX_BASE_URL},
    )
    monkeypatch.setattr(
        "laia_cli.auth._import_codex_cli_tokens",
    ...
    monkeypatch.setattr("laia_cli.auth._save_codex_tokens", _fake_save)
    monkeypatch.setattr("laia_cli.auth._update_config_for_provider", lambda *args, **kwargs: "/tmp/config.yaml")
    monkeypatch.setattr(
        "builtins.input",
