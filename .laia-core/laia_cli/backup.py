"""
Backup and import commands for laia CLI.

`laia backup` creates a zip archive of the entire ~/.laia/ directory
(excluding the laia-agent repo and transient files).

`laia import` restores from a backup zip, overlaying onto the current
LAIA_HOME root.
"""
...
from laia_constants import get_default_laia_root, get_laia_home, display_laia_home
...
_EXCLUDED_DIRS = {
    "laia-agent",     # the codebase repo — re-clone instead
...
def _should_exclude(rel_path: Path) -> bool:
    """Return True if *rel_path* (relative to laia root) should be skipped."""
...
def run_backup(args) -> None:
    """Create a zip backup of the LAIA home directory."""
    laia_root = get_default_laia_root()

    if not laia_root.is_dir():
        print(f"Error: LAIA home directory not found at {laia_root}")
        sys.exit(1)
    ...
    if args.output:
        ...
        if out_path.is_dir():
            ...
            out_path = out_path / f"laia-backup-{stamp}.zip"
    else:
        stamp = datetime.now().strftime("%Y-%m-%d-%H%M%S")
        out_path = Path.home() / f"laia-backup-{stamp}.zip"
    ...
    # Collect files
    print(f"Scanning {display_laia_home()} ...")
    files_to_add: list[tuple[Path, Path]] = []  # (absolute, relative)
    skipped_dirs = set()

    for dirpath, dirnames, filenames in os.walk(laia_root, followlinks=False):
        dp = Path(dirpath)
        rel_dir = dp.relative_to(laia_root)
        ...
        for fname in filenames:
            fpath = dp / fname
            rel = fpath.relative_to(laia_root)

            if _should_exclude(rel):
                continue

            # Skip the output zip itself if it happens to be inside laia root
            try:
    ...
    print(f"\nRestore with: laia import {out_path.name}")
...
def _validate_backup_zip(zf: zipfile.ZipFile) -> tuple[bool, str]:
    """Check that a zip looks like a LAIA backup.
    ...
    # Look for telltale files that a laia home would have
    ...
    if not found:
        return False, (
            "zip does not appear to be a LAIA backup "
...
def _detect_prefix(zf: zipfile.ZipFile) -> str:
    ...
    Some tools zip as `.laia/config.yaml` instead of `config.yaml`.
    ...
    if len(first_parts) == 1:
        ...
        # Only strip if it looks like a laia dir name
        if prefix in (".laia", "laia"):
            return prefix + "/"
...
def run_import(args) -> None:
    ...
    with zipfile.ZipFile(zip_path, "r") as zf:
        ...
        print(f"Target: {display_laia_home()}")

        if prefix:
            print(f"Detected archive prefix: {prefix!r} (will be stripped)")

        # Check for existing installation
        has_config = (laia_root / "config.yaml").exists()
        has_env = (laia_root / ".env").exists()

        if (has_config or has_env) and not args.force:
            print()
            print("Warning: Target directory already has LAIA configuration.")
        ...
        laia_root.mkdir(parents=True, exist_ok=True)
        ...
        for member in members:
            ...
            target = laia_root / rel

            # Security: reject absolute paths and traversals
            try:
                target.resolve().relative_to(laia_root.resolve())
            except ValueError:
        ...
        profiles_dir = laia_root / "profiles"
        restored_profiles = []
        if profiles_dir.is_dir():
            try:
                ...
                # laia_cli.profiles might not be available (fresh install)
                if any(profiles_dir.iterdir()):
                    print(f"\n  Profiles detected but aliases could not be created.")
                    print(f"  Run: laia profile list  (after installing laia)")

        # Guidance
        print()
        if not (laia_root / "laia-agent").is_dir():
            print("Note: The laia-agent codebase was not included in the backup.")
            print("  If this is a fresh install, run: laia update")

        if restored_profiles:
            gw_profiles = [n for n, _ in restored_profiles]
            print("\nTo re-enable gateway services for profiles:")
            for pname in gw_profiles:
                print(f"  laia -p {pname} gateway install")

        print("Done. Your LAIA configuration has been restored.")


# ---------------------------------------------------------------------------
# Quick state snapshots (used by /snapshot slash command and laia backup --quick)
# ---------------------------------------------------------------------------

# Critical state files to include in quick snapshots (relative to LAIA_HOME).
...
# — `laia update` snapshots this set before pulling so approved-user lists
...
def _quick_snapshot_root(laia_home: Optional[Path] = None) -> Path:
    home = laia_home or get_laia_home()
    return home / _QUICK_SNAPSHOTS_DIR


def create_quick_snapshot(
    label: Optional[str] = None,
    laia_home: Optional[Path] = None,
) -> Optional[str]:
...
def list_quick_snapshots(
    ...
    root = _quick_snapshot_root(laia_home)
    if not root.exists():
        return []
...
def prune_quick_snapshots(
    ...
    return _prune_quick_snapshots(_quick_snapshot_root(laia_home), keep=keep)


def run_quick_backup(args) -> None:
    """CLI entry point for laia backup --quick."""
    label = getattr(args, "label", None)
    snap_id = create_quick_snapshot(label=label)
    if snap_id:
        print(f"State snapshot created: {snap_id}")
        snaps = list_quick_snapshots()
        print(f"  {len(snaps)} snapshot(s) stored in {display_laia_home()}/state-snapshots/")
...
def _write_full_zip_backup(out_path: Path, laia_root: Path) -> Optional[Path]:
    """Write a full zip snapshot of ``laia_root`` to ``out_path``.
...
def _pre_update_backup_dir(laia_home: Optional[Path] = None) -> Path:
    home = laia_home or get_laia_home()
...
def create_pre_update_backup(
    ...
    """Create a full zip backup of LAIA_HOME under ``backups/``.

    Mirrors :func:`run_backup` (same exclusion rules, same SQLite safe-copy)
    but writes to ``<LAIA_HOME>/backups/pre-update-<timestamp>.zip`` and
    auto-prunes old pre-update backups.

    Returns the path to the created zip, or ``None`` if no files were
    found or the backup could not be created.  Never raises — the caller
    (``laia update``) should continue even if the backup fails.
    """
    laia_root = laia_home or get_default_laia_root()
    if not laia_root.is_dir():
        return None

    backup_dir = _pre_update_backup_dir(laia_root)
    try:
    ...
    result = _write_full_zip_backup(out_path, laia_root)
    if result is None:
        return None
...
# ---------------------------------------------------------------------------
# Pre-migration auto-backup (used by `laia claw migrate`)
# ---------------------------------------------------------------------------
...
def create_pre_migration_backup(
    ...
    """Create a full zip backup of LAIA_HOME under ``backups/`` before a
    ``laia claw migrate`` apply.

    Shares implementation with :func:`create_pre_update_backup` via
    ``_write_full_zip_backup`` — same exclusions, same SQLite safe-copy,
    restorable with ``laia import <archive>``.  Writes to
    ``<LAIA_HOME>/backups/pre-migration-<timestamp>.zip`` and auto-prunes
    ...
    # Reuses the shared backups/ directory so `laia import` and the
