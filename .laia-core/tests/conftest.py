"""Shared fixtures for the laia-agent test suite.
...
2. **Isolated LAIA_HOME.** LAIA_HOME points to a per-test tempdir so
   code reading ``~/.laia/*`` via ``get_laia_home()`` can't see the
   real one. (We do NOT also redirect HOME — that broke subprocesses in
   CI. Code using ``Path.home() / ".laia"`` instead of the canonical
   ``get_laia_home()`` is a bug to fix at the callsite.)
3. **Deterministic runtime.** TZ=UTC, LANG=C.UTF-8, PYTHONHASHSEED=0.
4. **No LAIA_SESSION_* inheritance** — the agent's current gateway
...
# LAIA_* vars that change test behavior by being set. Unset all of these
# unconditionally — individual tests that need them set do so explicitly.
_LAIA_BEHAVIORAL_VARS = frozenset({
    "LAIA_YOLO_MODE",
    "LAIA_INTERACTIVE",
    "LAIA_QUIET",
    "LAIA_TOOL_PROGRESS",
    ...
    "LAIA_MAX_ITERATIONS",
    "LAIA_SESSION_PLATFORM",
    "LAIA_SESSION_CHAT_ID",
    "LAIA_SESSION_CHAT_NAME",
    "LAIA_SESSION_THREAD_ID",
    "LAIA_SESSION_SOURCE",
    "LAIA_SESSION_KEY",
    "LAIA_GATEWAY_SESSION",
    "LAIA_PLATFORM",
    "LAIA_INFERENCE_PROVIDER",
    "LAIA_MANAGED",
    "LAIA_DEV",
    "LAIA_CONTAINER",
...
def _hermetic_environment(tmp_path, monkeypatch):
    ...
    Also redirects HOME and LAIA_HOME to per-test tempdirs so code that
    reads ``~/.laia/*`` can't touch the real one, and pins TZ/LANG so
    ...
    # 2. Blank behavioral LAIA_* vars that could change test semantics.
    for name in _LAIA_BEHAVIORAL_VARS:
        monkeypatch.delenv(name, raising=False)

    # 3. Redirect LAIA_HOME to a per-test tempdir. Code that reads
    #    ``~/.laia/*`` via ``get_laia_home()`` now gets the tempdir.
    #
    #    NOTE: We do NOT also redirect HOME. Doing so broke CI because
    #    some tests (and their transitive deps) spawn subprocesses that
    #    inherit HOME and expect it to be stable. If a test genuinely
    #    needs HOME isolated, it should set it explicitly in its own
    #    fixture. Any code in the codebase reading ``~/.laia/*`` via
    #    ``Path.home() / ".laia"`` instead of ``get_laia_home()``
    #    is a bug to fix at the callsite.
    fake_laia_home = tmp_path / "laia_test"
    fake_laia_home.mkdir()
    (fake_laia_home / "sessions").mkdir()
    (fake_laia_home / "cron").mkdir()
    (fake_laia_home / "memories").mkdir()
    (fake_laia_home / "skills").mkdir()
    monkeypatch.setenv("LAIA_HOME", str(fake_laia_home))
    ...
    #    ~/.laia/plugins/ (which, per step 3, is now empty — but the
    #    singleton might still be cached from a previous test).
    try:
        import laia_cli.plugins as _plugins_mod
...
@pytest.fixture(autouse=True)
def _isolate_laia_home(_hermetic_environment):
...
@pytest.fixture()
def mock_config():
    """Return a minimal laia config dict suitable for unit tests."""
    return {
