...
LAIA_HOME_DIR="${LAIA_HOME:-$HOME/.laia}"
LAIA_CFG="${LAIA_HOME_DIR}/config.yaml"
MCP_PORT=40404
...
# ── 3. Ensure LAIA config has twozero_td MCP entry ──
if [[ ! -f "$LAIA_CFG" ]]; then
    echo -e " ${FAIL} LAIA config not found at ${LAIA_CFG}"
    manual_steps+=("Create ${LAIA_CFG} with twozero_td MCP server entry")
elif grep -q 'twozero_td' "$LAIA_CFG" 2>/dev/null; then
    echo -e " ${OK} twozero_td MCP entry exists in LAIA config"
else
    echo -e " ${WARN} Adding twozero_td MCP entry to LAIA config..."
    python3 -c "
...
" 2>/dev/null && echo -e " ${OK} twozero_td MCP entry added to config" \
              || { echo -e " ${FAIL} Could not update config (is PyYAML installed?)"; \
                   manual_steps+=("Add twozero_td MCP entry to ${LAIA_CFG} manually"); }
    manual_steps+=("Restart LAIA session to pick up config change")
fi
