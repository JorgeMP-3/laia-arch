...
def _state_file() -> Path:
    return get_laia_home() / "skills" / ".curator_state"
...
def _load_config() -> Dict[str, Any]:
    """Read curator.* config from ~/.laia/config.yaml. Tolerates missing file."""
    try:
...
CURATOR_REVIEW_PROMPT = (
    "You are running as LAIA' background skill CURATOR. This is an "
    ...
    "into ~/.laia/skills/.archive/) is the maximum destructive action. "
    ...
    "to find: laia-config-*, laia-dashboard-*, gateway-*, codex-*, "
    ...
    "~/.laia/skills/<umbrella>/references/ && mv ... <umbrella>/"
...
def _reports_root() -> Path:
    ...
    Lives under the profile-aware logs dir (``~/.laia/logs/curator/``)
    ...
    authored skill data in ``~/.laia/skills/``.
    """
    return get_laia_home() / "logs" / "curator"
...
def _render_report_markdown(p: Dict[str, Any]) -> str:
    ...
    if archived:
        ...
        lines.append("_Archived skills are at `~/.laia/skills/.archive/`. "
                     "Restore any via `laia curator restore <name>`._\n")
    ...
    lines.append("- Restore an archived skill: `laia curator restore <name>`")
    lines.append("- All archives live under `~/.laia/skills/.archive/` and are recoverable by `mv`")
...
def run_curator_review(
    ...
    def _llm_pass():
        ...
        # recorded in state so `laia curator status` can point at it.
        try:
