"""ACP session manager — maps ACP sessions to LAIA AIAgent instances.

Sessions are persisted to the shared SessionDB (``~/.laia/state.db``) so they
...
def _expand_acp_enabled_toolsets(
    ...
    expanded: List[str] = []
    for name in list(toolsets or ["laia-acp"]):
...
@dataclass
class SessionState:
    """Tracks per-session state for an ACP-managed LAIA agent."""
...
class SessionManager:
    """Thread-safe manager for ACP sessions backed by LAIA AIAgent instances.
    ...
    def __init__(self, agent_factory=None, db=None):
        ...
        Args:
            agent_factory: Optional callable that creates an AIAgent-like object.
                           ...
                           using the current LAIA runtime provider configuration.
            db:            Optional SessionDB instance. When omitted, the default
                           SessionDB (``~/.laia/state.db``) is lazily created.
        """
    ...
    def _get_db(self):
        ...
        Note: we resolve ``LAIA_HOME`` dynamically rather than relying on
        ...
        later (e.g. by the test fixture ``_isolate_laia_home``).
        """
        ...
        try:
            ...
            self._db_instance = SessionDB(db_path=laia_home / "state.db")
            return self._db_instance
        except Exception:
