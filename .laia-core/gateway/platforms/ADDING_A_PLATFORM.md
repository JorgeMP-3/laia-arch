...
Checklist for integrating a new messaging platform into the LAIA gateway.
...
And add it to the `laia-gateway` composite:

```python
"laia-gateway": {
    "includes": [..., "laia-your-platform"]
}
```
...
## 12. Status Display (`laia_cli/status.py`)
...
## 13. Gateway Setup Wizard (`laia_cli/gateway.py`)
...
grep -r "telegram\|discord\|whatsapp\|slack" gateway/ tools/ agent/ cron/ laia_cli/ toolsets.py \
  --include="*.py" -l | sort -u
