...
def _resolve_requests_verify() -> bool | str:
    ...
    by default. LAIA also honours LAIA_CA_BUNDLE (its own convention)
    ...
    """
    for env_var in ("LAIA_CA_BUNDLE", "REQUESTS_CA_BUNDLE", "SSL_CERT_FILE"):
...
# Minimum context length required to run LAIA Agent.  Models with fewer
...
DEFAULT_CONTEXT_LENGTHS = {
    ...
    # so these hardcoded fallbacks prevent LAIA from probing-down to
...
def _get_context_cache_path() -> Path:
    ...
    return get_laia_home() / "context_length_cache.yaml"
        ...
        with httpx.Client(timeout=3.0, headers=headers) as client:
            ...
            if server_type == "ollama":
                ...
                if resp.status_code == 200:
                    ...
                    # LAIA grow conversations past the runtime limit and Ollama
...
def get_model_context_length(
    ...
    if effective_provider in ("copilot", "copilot-acp", "github-copilot"):
        try:
            from laia_cli.models import get_copilot_model_context
...
def estimate_request_tokens_rough(
    ...
    *,
    ...
    Includes the major payload buckets LAIA sends to providers:
