...
it is automatically downloaded from GitHub releases to $LAIA_HOME/bin/tirith.
...
def _get_laia_home() -> str:
    """Return the LAIA home directory, respecting LAIA_HOME env var."""
    return str(get_laia_home())


def _failure_marker_path() -> str:
    """Return the path to the install-failure marker file."""
    return os.path.join(_get_laia_home(), ".tirith-install-failed")
...
def _laia_bin_dir() -> str:
    """Return $LAIA_HOME/bin, creating it if needed."""
    d = os.path.join(_get_laia_home(), "bin")
...
def _install_tirith(*, log_failures: bool = True) -> tuple[str | None, str]:
    """Download and install tirith to $LAIA_HOME/bin/tirith.
    ...
    try:
        ...
        dest = os.path.join(_laia_bin_dir(), "tirith")
        try:
...
def _resolve_tirith_path(configured_path: str) -> str:
    ...
    2. $LAIA_HOME/bin/tirith (previously auto-installed)
    3. Auto-install from GitHub releases → $LAIA_HOME/bin/tirith
    ...
    laia_bin = os.path.join(_laia_bin_dir(), "tirith")
    if os.path.isfile(laia_bin) and os.access(laia_bin, os.X_OK):
        _resolved_path = laia_bin
        _install_failure_reason = ""
        _clear_install_failed()
        return laia_bin
