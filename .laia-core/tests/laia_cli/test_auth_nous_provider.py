...
from laia_cli.auth import AuthError, get_provider_auth_state, resolve_nous_runtime_credentials
...
class TestResolveVerifyFallback:
    ...
    def test_missing_ca_bundle_in_auth_state_falls_back(self):
        from laia_cli.auth import _resolve_verify
    ...
    def test_missing_ssl_cert_file_env_falls_back(self, monkeypatch):
        ...
        monkeypatch.delenv("LAIA_CA_BUNDLE", raising=False)
        result = _resolve_verify(auth_state={"tls": {}})
        assert result is True

    def test_missing_laia_ca_bundle_env_falls_back(self, monkeypatch):
        from laia_cli.auth import _resolve_verify

        monkeypatch.setenv("LAIA_CA_BUNDLE", "/nonexistent/laia-ca.pem")
...
def _setup_nous_auth(
    ...
    *,
    ...
    (laia_home / "auth.json").write_text(json.dumps(auth_store, indent=2))
...
def test_get_nous_auth_status_auth_store_fallback(tmp_path, monkeypatch):
    ...
    laia_home = tmp_path / "laia"
    _setup_nous_auth(laia_home, access_token="at-123")
    monkeypatch.setenv("LAIA_HOME", str(laia_home))
    monkeypatch.setattr(
...
def test_get_nous_auth_status_prefers_runtime_auth_store_over_stale_pool(tmp_path, monkeypatch):
    ...
    laia_home = tmp_path / "laia"
    _setup_nous_auth(laia_home, access_token="at-fresh")
    monkeypatch.setenv("LAIA_HOME", str(laia_home))
...
def test_get_nous_auth_status_reports_revoked_refresh_session(tmp_path, monkeypatch):
    ...
    monkeypatch.setattr("laia_cli.auth.resolve_nous_runtime_credentials", _boom)
...
def test_refresh_token_persisted_when_mint_returns_insufficient_credits(tmp_path, monkeypatch):
    laia_home = tmp_path / "laia"
    _setup_nous_auth(laia_home, refresh_token="refresh-old")
    monkeypatch.setenv("LAIA_HOME", str(laia_home))
    ...
    monkeypatch.setattr("laia_cli.auth._refresh_access_token", _fake_refresh_access_token)
    monkeypatch.setattr("laia_cli.auth._mint_agent_key", _fake_mint_agent_key)
...
class TestLoginNousSkipKeepsCurrent:
    """When a user runs `laia model` → Nous Portal → Skip (keep current) after
    ...
    def _setup_home_with_openrouter(self, tmp_path, monkeypatch):
        ...
        auth_path = laia_home / "auth.json"
        auth_path.write_text(json.dumps({
            "version": 1,
            "active_provider": "openrouter",
            "providers": {"openrouter": {"api_key": "sk-or-fake"}},
        }))
        return laia_home, config_path, auth_path
    ...
    def _patch_login_internals(self, monkeypatch, *, prompt_returns):
        ...
        import laia_cli.auth as auth_mod
        import laia_cli.models as models_mod
        import laia_cli.nous_subscription as ns
    ...
    def test_skip_keep_current_preserves_provider_and_model(self, tmp_path, monkeypatch):
        ...
        import argparse
        import yaml
        from laia_cli.auth import PROVIDER_REGISTRY, _login_nous

        laia_home, config_path, auth_path = self._setup_home_with_openrouter(
...
def test_persist_nous_credentials_writes_both_pool_and_providers(tmp_path, monkeypatch):
    ...
    Regression guard: before this helper existed, `laia auth add nous`
    ...
    """
    from laia_cli.auth import persist_nous_credentials, NOUS_DEVICE_CODE_SOURCE
...
def test_persist_nous_credentials_allows_recovery_from_401(tmp_path, monkeypatch):
    ...
    must succeed (not raise "LAIA is not logged into Nous Portal").
    ...
    """
    from laia_cli.auth import persist_nous_credentials, resolve_nous_runtime_credentials
    ...
    # doesn't raise "LAIA is not logged into Nous Portal".
    def _fake_refresh_access_token(*, client, portal_base_url, client_id, refresh_token):
        return {
...
def test_persist_nous_credentials_embeds_custom_label(tmp_path, monkeypatch):
    ...
    Previously `laia auth add nous --type oauth --label <name>` silently
...
def test_refresh_token_reuse_detection_surfaces_actionable_message():
    ...
    ``error_description`` containing "reuse detected", LAIA must surface an
    ...
    re-authenticate" string led users to report this as a LAIA persistence
    bug when the true cause is external RT consumption (monitoring scripts,
    custom self-heal hooks).
    """
    from laia_cli.auth import _refresh_access_token
    ...
    assert "laia auth add nous" in message.lower()
