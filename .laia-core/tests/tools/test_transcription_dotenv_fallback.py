...
``~/.laia/.env`` entries. These tests confirm each STT provider now
...
class TestProviderSelectionGate:
    ...
    ``os.environ`` a user with keys in ``~/.laia/.env`` would be told
    ...
    def test_explicit_groq_sees_dotenv(self):
        ...
        with patch.object(tt, "_HAS_FASTER_WHISPER", False), \
             ...
             patch("laia_cli.config.load_env",
...
class TestEndToEndRegressionGuard:
    """End-to-end probe: patch ``laia_cli.config.load_env`` to simulate
    ``~/.laia/.env`` carrying the key while ``os.environ`` does not.
    ...
    def test_xai_key_only_in_dotenv_before_fix(self, monkeypatch):
        ...
        with patch("laia_cli.config.load_env",
                   return_value={"XAI_API_KEY": "dotenv-secret"}):
