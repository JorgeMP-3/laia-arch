"""
LAIAAgentLoop -- Reusable Multi-Turn Agent Engine

Runs the laia-agent tool-calling loop using standard OpenAI-spec tool calling.
...
identical to laia-agent's run_agent.py. Tool execution is dispatched via
...
# Resized at runtime by LAIAAgentBaseEnv.__init__ via resize_tool_pool().
...
def resize_tool_pool(max_workers: int):
    ...
    Called by LAIAAgentBaseEnv.__init__ based on config.tool_pool_size.
...
class LAIAAgentLoop:
    """
    Runs laia-agent's tool-calling loop using standard OpenAI-spec tool calling.
    ...
    async def run(self, messages: List[Dict[str, Any]]) -> AgentResult:
        ...
        for turn in range(self.max_turns):
            ...
            # laia-agent's standalone parsers. This handles the case where
            ...
            if (
                ...
                try:
                    ...
                    fallback_parser = get_parser("laia")
            ...
            if assistant_msg.tool_calls:
                ...
                # Execute each tool call via laia-agent's dispatch
