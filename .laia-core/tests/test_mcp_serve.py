"""
Tests for mcp_serve — LAIA MCP server.
...
def _isolate_laia_home(tmp_path, monkeypatch):
    """Redirect LAIA_HOME to a temp directory."""
    monkeypatch.setenv("LAIA_HOME", str(tmp_path))
    try:
        import laia_constants
        monkeypatch.setattr(laia_constants, "get_laia_home", lambda: tmp_path)
...
def _create_test_db(db_path, session_id, messages):
    """Create a minimal SQLite DB mimicking laia_state schema."""
