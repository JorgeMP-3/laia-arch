...
import laia_cli.gateway as gateway_cli
...
class TestSystemdServiceRefresh:
    def test_systemd_install_repairs_outdated_unit_without_force(self, tmp_path, monkeypatch):
        unit_path = tmp_path / "laia-gateway.service"
...
class TestLaunchdServiceRecovery:
    def test_get_restart_drain_timeout_prefers_env_then_config_then_default(self, monkeypatch):
        monkeypatch.delenv("LAIA_RESTART_DRAIN_TIMEOUT", raising=False)
        ...
        monkeypatch.setenv("LAIA_RESTART_DRAIN_TIMEOUT", "9")
        assert gateway_cli._get_restart_drain_timeout() == 9.0

        monkeypatch.setenv("LAIA_RESTART_DRAIN_TIMEOUT", "invalid")
        assert (
            gateway_cli._get_restart_drain_timeout()
            == DEFAULT_GATEWAY_RESTART_DRAIN_TIMEOUT
        )
    ...
    def test_launchd_install_repairs_outdated_plist_without_force(self, tmp_path, monkeypatch):
        plist_path = tmp_path / "ai.laia.gateway.plist"
...
class TestGatewaySystemServiceRouting:
    ...
    def test_systemd_status_surfaces_planned_restart_failure(self, monkeypatch, capsys):
        ...
        monkeypatch.setattr(gateway_cli, "has_legacy_laia_units", lambda: False)
    ...
    def test_gateway_install_reports_termux_manual_mode(self, monkeypatch, capsys):
        ...
        assert "Run manually: laia gateway" in out
    ...
    def test_gateway_status_on_termux_shows_manual_guidance(self, monkeypatch, capsys):
        ...
        assert "nohup laia gateway" in out
    ...
    def test_gateway_restart_does_not_fallback_to_foreground_when_launchd_restart_fails(self, tmp_path, monkeypatch):
        ...
        monkeypatch.setattr(
            ...
            lambda: (_ for _ in ()).throw(
                gateway_cli.subprocess.CalledProcessError(5, ["launchctl", "kickstart", "-k", "gui/501/ai.laia.gateway"])
            ),
        )
...
class TestSystemUnitLAIAHome:
    """LAIA_HOME in system units must reference the target user, not root."""

    def test_system_unit_uses_target_user_home_not_calling_user(self, monkeypatch):
        # Simulate sudo: Path.home() returns /root, target user is alice
        monkeypatch.setattr(Path, "home", staticmethod(lambda: Path("/root")))
        monkeypatch.delenv("LAIA_HOME", raising=False)
        monkeypatch.setattr(
        ...
        assert 'LAIA_HOME=/home/alice/.laia' in unit
        assert '/root/.laia' not in unit

    def test_system_unit_remaps_profile_to_target_user(self, monkeypatch):
        # Simulate sudo with a profile: LAIA_HOME was resolved under root
        monkeypatch.setattr(Path, "home", staticmethod(lambda: Path("/root")))
        monkeypatch.setenv("LAIA_HOME", "/root/.laia/profiles/coder")
        monkeypatch.setattr(
            gateway_cli, "_system_service_identity",
            lambda run_as_user=None: ("alice", "alice", "/home/alice"),
        )
        ...
        assert 'LAIA_HOME=/home/alice/.laia/profiles/coder' in unit
        assert '/root/' not in unit

    def test_system_unit_preserves_custom_laia_home(self, monkeypatch):
        # Custom LAIA_HOME not under any user's home — keep as-is
        monkeypatch.setattr(Path, "home", staticmethod(lambda: Path("/root")))
        monkeypatch.setenv("LAIA_HOME", "/opt/laia-shared")
        monkeypatch.setattr(
            gateway_cli, "_system_service_identity",
            lambda run_as_user=None: ("alice", "alice", "/home/alice"),
        )
        ...
        assert 'LAIA_HOME=/opt/laia-shared' in unit

    def test_user_unit_unaffected_by_change(self):
        # User-scope units should still use the calling user's LAIA_HOME
        unit = gateway_cli.generate_systemd_unit(system=False)

        laia_home = str(gateway_cli.get_laia_home().resolve())
        assert f'LAIA_HOME={laia_home}' in unit


class TestLAIAHomeForTargetUser:
    """Unit tests for _laia_home_for_target_user()."""

    def test_remaps_default_home(self, monkeypatch):
        monkeypatch.setattr(Path, "home", staticmethod(lambda: Path("/root")))
        monkeypatch.delenv("LAIA_HOME", raising=False)

        result = gateway_cli._laia_home_for_target_user("/home/alice")
        assert result == "/home/alice/.laia"
    ...
    def test_remaps_profile_path(self, monkeypatch):
        ...
        result = gateway_cli._laia_home_for_target_user("/home/alice")
        assert result == "/home/alice/.laia/profiles/coder"

    def test_keeps_custom_path(self, monkeypatch):
        monkeypatch.setattr(Path, "home", staticmethod(lambda: Path("/root")))
        monkeypatch.setenv("LAIA_HOME", "/opt/laia")

        result = gateway_cli._laia_home_for_target_user("/home/alice")
        assert result == "/opt/laia"
...
class TestPreflightUserSystemd:
    ...
    def test_raises_when_linger_disabled_and_loginctl_denied(self, monkeypatch):
        ...
        assert "laia gateway run" in msg  # foreground fallback mentioned
...
class TestProfileArg:
    ...
    def test_default_laia_home_returns_empty(self, tmp_path, monkeypatch):
        """Default ~/.laia should not produce a --profile flag."""
        laia_home = tmp_path / ".laia"
        laia_home.mkdir()
        monkeypatch.setattr(Path, "home", lambda: tmp_path)
        monkeypatch.setenv("LAIA_HOME", str(laia_home))
        result = gateway_cli._profile_arg(str(laia_home))
        assert result == ""

    def test_named_profile_returns_flag(self, tmp_path, monkeypatch):
        """~/.laia/profiles/mybot should return '--profile mybot'."""
        profile_dir = tmp_path / ".laia" / "profiles" / "mybot"
        profile_dir.mkdir(parents=True)
        monkeypatch.setattr(Path, "home", lambda: tmp_path)
        monkeypatch.setenv("LAIA_HOME", str(tmp_path / ".laia"))
    ...
    def test_hash_path_returns_empty(self, tmp_path, monkeypatch):
        """Arbitrary non-profile LAIA_HOME should return empty string."""
        custom_home = tmp_path / "custom" / "laia"
    ...
    def test_nested_profile_path_returns_empty(self, tmp_path, monkeypatch):
        """~/.laia/profiles/mybot/subdir should NOT match — too deep."""
        nested = tmp_path / ".laia" / "profiles" / "mybot" / "subdir"
    ...
    def test_invalid_profile_name_returns_empty(self, tmp_path, monkeypatch):
        ...
        bad_profile = tmp_path / ".laia" / "profiles" / "My Bot!"
    ...
    def test_systemd_unit_includes_profile(self, tmp_path, monkeypatch):
        ...
        profile_dir.mkdir(parents=True)
        monkeypatch.setattr(Path, "home", lambda: tmp_path)
        monkeypatch.setenv("LAIA_HOME", str(profile_dir))
        monkeypatch.setattr(gateway_cli, "get_laia_home", lambda: profile_dir)
        unit = gateway_cli.generate_systemd_unit(system=False)
    ...
    def test_launchd_plist_path_uses_real_user_home_not_profile_home(self, tmp_path, monkeypatch):
        profile_dir = tmp_path / ".laia" / "profiles" / "orcha"
        profile_dir.mkdir(parents=True)
        ...
        assert plist_path == machine_home / "Library" / "LaunchAgents" / "ai.laia.gateway-orcha.plist"
...
class TestRemapPathForUser:
    ...
    def test_remaps_path_under_current_home(self, monkeypatch, tmp_path):
        ...
        result = gateway_cli._remap_path_for_user(
            str(tmp_path / "root" / ".laia" / "laia-agent"),
            str(tmp_path / "alice"),
        )
        assert result == str(tmp_path / "alice" / ".laia" / "laia-agent")

    def test_keeps_system_path_unchanged(self, monkeypatch, tmp_path):
        monkeypatch.setattr(Path, "home", lambda: tmp_path / "root")
        (tmp_path / "root").mkdir()
        result = gateway_cli._remap_path_for_user("/opt/laia", str(tmp_path / "alice"))
        assert result == "/opt/laia"

    def test_noop_when_same_user(self, monkeypatch, tmp_path):
        monkeypatch.setattr(Path, "home", lambda: tmp_path / "alice")
        (tmp_path / "alice").mkdir()
        original = str(tmp_path / "alice" / ".laia" / "laia-agent")
...
class TestSystemUnitPathRemapping:
    ...
    def test_system_unit_has_no_root_paths(self, monkeypatch, tmp_path):
        ...
        root_home.mkdir()
        project = root_home / ".laia" / "laia-agent"
        ...
        monkeypatch.setenv("LAIA_HOME", str(root_home / ".laia"))
        monkeypatch.setattr(gateway_cli, "get_laia_home", lambda: root_home / ".laia")
        ...
        assert "WorkingDirectory=/home/alice/.laia/laia-agent" in unit
...
class TestDockerAwareGateway:
    ...
    def test_run_systemctl_raises_runtimeerror_when_missing(self, monkeypatch):
        ...
        with pytest.raises(RuntimeError, match="systemctl is not available"):
            gateway_cli._run_systemctl(["start", "laia-gateway"])
    ...
    def test_run_systemctl_passes_through_on_success(self, monkeypatch):
        ...
        result = gateway_cli._run_systemctl(["status", "laia-gateway"])
        assert result.returncode == 0
        assert len(calls) == 1
        assert "status" in calls[0]

    def test_install_in_container_prints_docker_guidance(self, monkeypatch, capsys):
        """'laia gateway install' inside Docker exits 0 with container guidance."""
        import pytest
    ...
    def test_uninstall_in_container_prints_docker_guidance(self, monkeypatch, capsys):
        """'laia gateway uninstall' inside Docker exits 0 with container guidance."""
        import pytest
    ...
    def test_start_in_container_prints_docker_guidance(self, monkeypatch, capsys):
        """'laia gateway start' inside Docker exits 0 with container guidance."""
        import pytest
        ...
        out = capsys.readouterr().out
        assert "docker" in out.lower()
        assert "laia gateway run" in out


class TestLegacyLAIAUnitDetection:
    """Tests for _find_legacy_laia_units / has_legacy_laia_units.

    These guard against the scenario that tripped Luis in April 2026: an
    older install left a ``laia.service`` unit behind when the service was
    renamed to ``laia-gateway.service``. After PR #5646 (signal recovery
    via systemd), the two services began SIGTERM-flapping over the same
    Telegram bot token in a 30-second cycle.

    The detector must flag ``laia.service`` ONLY when it actually runs our
    gateway, and must NEVER flag profile units
    (``laia-gateway-<profile>.service``) or unrelated third-party services.
    """

    # Minimal ExecStart that looks like our gateway
    _OUR_UNIT_TEXT = (
        "[Unit]\nDescription=LAIA Gateway\n[Service]\n"
        "ExecStart=/usr/bin/python -m laia_cli.main gateway run --replace\n"
    )
    ...
    def test_detects_legacy_laia_service_in_user_scope(self, tmp_path, monkeypatch):
        user_dir, _ = self._setup_search_paths(tmp_path, monkeypatch)
        legacy = user_dir / "laia.service"
        legacy.write_text(self._OUR_UNIT_TEXT, encoding="utf-8")

        results = gateway_cli._find_legacy_laia_units()

        assert len(results) == 1
        name, path, is_system = results[0]
        assert name == "laia.service"
        assert path == legacy
        assert is_system is False
        assert gateway_cli.has_legacy_laia_units() is True

    def test_detects_legacy_laia_service_in_system_scope(self, tmp_path, monkeypatch):
        _, system_dir = self._setup_search_paths(tmp_path, monkeypatch)
        legacy = system_dir / "laia.service"
        legacy.write_text(self._OUR_UNIT_TEXT, encoding="utf-8")
    ...
    def test_ignores_profile_unit_laia_gateway_coder(self, tmp_path, monkeypatch):
        """CRITICAL: profile units must NOT be flagged as legacy.

        Teknium's concern — ``laia-gateway-coder.service`` is our standard
        ...
        for base in (user_dir, system_dir):
            (base / "laia-gateway-coder.service").write_text(
                self._OUR_UNIT_TEXT, encoding="utf-8"
            )
            (base / "laia-gateway-orcha.service").write_text(
                self._OUR_UNIT_TEXT, encoding="utf-8"
            )
            (base / "laia-gateway.service").write_text(
                self._OUR_UNIT_TEXT, encoding="utf-8"
            )
        ...
        assert results == []
        assert gateway_cli.has_legacy_laia_units() is False

    def test_ignores_unrelated_laia_service(self, tmp_path, monkeypatch):
        """Third-party ``laia.service`` that isn't ours stays untouched.

        If a user has some other package named ``laia`` installed as a
        service, we must not flag it.
        """
        user_dir, _ = self._setup_search_paths(tmp_path, monkeypatch)
        (user_dir / "laia.service").write_text(
            "[Unit]\nDescription=Some Other LAIA\n[Service]\n"
            "ExecStart=/opt/other-laia/bin/daemon --foreground\n",
            encoding="utf-8",
        )
    ...
    def test_returns_empty_when_no_legacy_files_exist(self, tmp_path, monkeypatch):
        ...
        assert gateway_cli._find_legacy_laia_units() == []
        assert gateway_cli.has_legacy_laia_units() is False
    ...
    def test_detects_both_scopes_simultaneously(self, tmp_path, monkeypatch):
        ...
        user_dir, system_dir = self._setup_search_paths(tmp_path, monkeypatch)
        (user_dir / "laia.service").write_text(self._OUR_UNIT_TEXT, encoding="utf-8")
        (system_dir / "laia.service").write_text(self._OUR_UNIT_TEXT, encoding="utf-8")
    ...
    def test_accepts_alternate_execstart_formats(self, tmp_path, monkeypatch):
        ...
        ExecStart variants we've seen in the wild:
          - python -m laia_cli.main gateway run
          - python path/to/laia_cli/main.py gateway run
          - laia gateway run   (direct binary)
          - python path/to/gateway/run.py
        """
        user_dir, _ = self._setup_search_paths(tmp_path, monkeypatch)
        variants = [
            "ExecStart=/venv/bin/python -m laia_cli.main gateway run --replace",
            "ExecStart=/venv/bin/python /opt/laia/laia_cli/main.py gateway run",
            "ExecStart=/usr/local/bin/laia gateway run --replace",
            "ExecStart=/venv/bin/python /opt/laia/gateway/run.py",
        ]
        for i, execstart in enumerate(variants):
            name = f"laia.service" if i == 0 else f"laia.service"  # same name
            # Test each variant fresh
            (user_dir / "laia.service").write_text(
                f"[Unit]\nDescription=Old LAIA\n[Service]\n{execstart}\n",
                encoding="utf-8",
            )
            results = gateway_cli._find_legacy_laia_units()
    ...
    def test_print_legacy_unit_warning_shows_migration_hint(self, tmp_path, monkeypatch, capsys):
        ...
        assert "laia.service" in out
        assert "laia gateway migrate-legacy" in out
    ...
    def test_handles_unreadable_unit_file_gracefully(self, tmp_path, monkeypatch):
        ...
        user_dir, _ = self._setup_search_paths(tmp_path, monkeypatch)
        unreadable = user_dir / "laia.service"
...
class TestRemoveLegacyLAIAUnits:
    """Tests for remove_legacy_laia_units (the migration action)."""
    ...
    def test_returns_zero_when_no_legacy_units(self, tmp_path, monkeypatch, capsys):
        ...
        removed, remaining = gateway_cli.remove_legacy_laia_units(interactive=False)
    ...
    def test_dry_run_lists_without_removing(self, tmp_path, monkeypatch, capsys):
        ...
        removed, remaining = gateway_cli.remove_legacy_laia_units(
    ...
    def test_removes_user_scope_legacy_unit(self, tmp_path, monkeypatch, capsys):
        ...
        assert any("--user stop laia.service" in c for c in cmds_joined)
        assert any("--user disable laia.service" in c for c in cmds_joined)
    ...
    def test_system_scope_without_root_defers_removal(self, tmp_path, monkeypatch, capsys):
        ...
        out = capsys.readouterr().out
        assert "sudo laia gateway migrate-legacy" in out
    ...
    def test_system_scope_with_root_removes(self, tmp_path, monkeypatch, capsys):
        ...
        assert any(
            c.startswith("systemctl stop laia.service") for c in cmds_joined
        )
        assert any(
            c.startswith("systemctl disable laia.service") for c in cmds_joined
        )
    ...
    def test_removes_both_scopes_with_root(self, tmp_path, monkeypatch, capsys):
        ...
        user_legacy = user_dir / "laia.service"
        system_legacy = system_dir / "laia.service"
    ...
    def test_does_not_touch_profile_units_during_migration(
        ...
        """Teknium's constraint: profile units (laia-gateway-coder.service)
        ...
        profile_unit = user_dir / "laia-gateway-coder.service"
        profile_unit.write_text(self._OUR_UNIT_TEXT, encoding="utf-8")
        default_unit = user_dir / "laia-gateway.service"
    ...
    def test_interactive_prompt_no_skips_removal(self, tmp_path, monkeypatch, capsys):
        ...
        removed, remaining = gateway_cli.remove_legacy_laia_units(interactive=True)
...
class TestMigrateLegacyCommand:
    """Tests for the `laia gateway migrate-legacy` subcommand dispatch."""
    ...
    def test_migrate_legacy_subparser_accepts_dry_run_and_yes(self):
        ...
        import laia_cli.main as cli_main
        ...
        # public, just confirm that `laia gateway --help` shows it.
        import subprocess
        import sys

        project_root = cli_main.PROJECT_ROOT if hasattr(cli_main, "PROJECT_ROOT") else None
        if project_root is None:
            import laia_cli.gateway as gw
            project_root = gw.PROJECT_ROOT

        result = subprocess.run(
            [sys.executable, "-m", "laia_cli.main", "gateway", "--help"],
    ...
    def test_gateway_command_migrate_legacy_dispatches(
        ...
        monkeypatch.setattr(gateway_cli, "remove_legacy_laia_units", fake_remove)
        monkeypatch.setattr(gateway_cli, "supports_systemd_services", lambda: True)
        monkeypatch.setattr(gateway_cli, "is_macos", lambda: False)
...
class TestGatewayStatusParser:
    def test_gateway_status_subparser_accepts_full_flag(self):
        ...
        result = subprocess.run(
            [sys.executable, "-m", "laia_cli.main", "gateway", "status", "-l", "--help"],
...
class TestSystemdInstallOffersLegacyRemoval:
    ...
    def test_install_offers_removal_when_legacy_detected(
        ...
        # has_legacy_laia_units must return True
        monkeypatch.setattr(gateway_cli, "has_legacy_laia_units", lambda: True)
        monkeypatch.setattr(gateway_cli, "remove_legacy_laia_units", fake_remove)
