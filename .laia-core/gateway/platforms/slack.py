...
class SlackAdapter(BasePlatformAdapter):
    ...
    async def connect(self) -> bool:
        ...
        tokens_file = get_laia_home() / "slack_tokens.json"
        ...
        try:
            ...
            # /model work directly without /laia prefix). A single regex
            ...
            # manifest (see `laia slack manifest`). In Socket Mode, Slack
            ...
            from laia_cli.commands import slack_native_slashes
            import re as _re
            ...
            if _slash_names:
                ...
                _slash_pattern = _re.compile(r"^/laia$")

            @self._app.command(_slash_pattern)
            async def handle_laia_command(ack, command):
                await ack()
    ...
    def _dm_top_level_threads_as_sessions(self) -> bool:
        ...
        own LAIA session — matching the per-thread behavior channels already
                    ...
                    "elements": [
                        {
                            ...
                            "style": "primary",
                            "action_id": "laia_approve_once",
                            "value": session_key,
                        },
                        {
                            "type": "button",
                            "text": {"type": "plain_text", "text": "Allow Session"},
                            "action_id": "laia_approve_session",
                            "value": session_key,
                        },
                        {
                            "type": "button",
                            "text": {"type": "plain_text", "text": "Always Allow"},
                            "action_id": "laia_approve_always",
                            "value": session_key,
                        },
                        {
                            "type": "button",
    ...
    async def _handle_approval_action(self, ack, body, action) -> None:
        ...
        choice_map = {
            "laia_approve_once": "once",
            "laia_approve_session": "session",
            "laia_approve_always": "always",
            "laia_deny": "deny",
        }
    ...
    async def _handle_slash_command(self, command: dict) -> None:
        ...
        The legacy ``/laia <subcommand> [args]`` form is preserved for
        backward compatibility with older workspace manifests and for users
        who want a single entry point for free-form questions (``/laia
        ...
        if slash_name in ("laia", ""):
            # Legacy /laia <subcommand> [args] routing + free-form questions.
            # Empty slash_name falls into this branch for backward compat
            # with any caller that didn't populate command["command"].
            from laia_cli.commands import slack_subcommand_map
