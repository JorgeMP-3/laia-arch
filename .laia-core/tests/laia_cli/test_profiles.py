"""Comprehensive tests for laia_cli.profiles module.
...
from laia_cli.profiles import (
    ...
    _get_profiles_root,
    _get_default_laia_home,
)


# ---------------------------------------------------------------------------
# Shared fixture: redirect Path.home() and LAIA_HOME for profile tests
# ---------------------------------------------------------------------------
...
def profile_env(tmp_path, monkeypatch):
    ...
    * Path.home() -> tmp_path  (so _get_profiles_root() = tmp_path/.laia/profiles)
    * LAIA_HOME  -> tmp_path/.laia  (so get_laia_home() agrees)
    * Creates the bare-minimum ~/.laia directory.
    """
    monkeypatch.setattr(Path, "home", lambda: tmp_path)
    default_home = tmp_path / ".laia"
    default_home.mkdir(exist_ok=True)
    monkeypatch.setenv("LAIA_HOME", str(default_home))
    return tmp_path
...
class TestDeleteProfile:
    ...
    def test_removes_directory(self, profile_env):
        ...
        with patch("laia_cli.profiles._cleanup_gateway_service"):
...
class TestActiveProfile:
    ...
    def test_empty_file_returns_default(self, profile_env):
        tmp_path = profile_env
        active_path = tmp_path / ".laia" / "active_profile"
...
class TestGetActiveProfileName:
    ...
    def test_default_laia_home_returns_default(self, profile_env):
        # LAIA_HOME points to tmp_path/.laia which is the default
        assert get_active_profile_name() == "default"

    def test_profile_path_returns_profile_name(self, profile_env, monkeypatch):
        tmp_path = profile_env
        create_profile("coder", no_alias=True)
        profile_dir = tmp_path / ".laia" / "profiles" / "coder"
    ...
    def test_custom_path_returns_default(self, profile_env, monkeypatch):
        """A custom LAIA_HOME (Docker, etc.) IS the default root."""
        tmp_path = profile_env
        custom = tmp_path / "some" / "other" / "path"
        custom.mkdir(parents=True)
        monkeypatch.setenv("LAIA_HOME", str(custom))
        # With Docker-aware roots, a custom LAIA_HOME is the default —
...
class TestAliasCollision:
    ...
    def test_reserved_name_returns_message(self, profile_env):
        result = check_alias_collision("laia")
...
class TestRenameProfile:
    ...
    def test_renames_root_honcho_host_without_changing_ai_peer(self, profile_env):
        ...
        assert "laia.ssi_health" not in cfg["hosts"]
        assert cfg["hosts"]["laia.heimdall"]["aiPeer"] == "ssi_health"
        assert cfg["hosts"]["laia.heimdall"]["peerName"] == "user-peer"
    ...
    def test_pins_ai_peer_when_absent_on_honcho_host_rename(self, profile_env):
        ...
        honcho_path.write_text(json.dumps({
            "hosts": {
                "laia.ssi_health": {"workspace": "laia", "enabled": True}
            }
        }))
        ...
        assert "laia.ssi_health" not in cfg["hosts"]
        assert cfg["hosts"]["laia.heimdall"]["aiPeer"] == "ssi_health"
        assert cfg["hosts"]["laia.heimdall"]["workspace"] == "laia"
    ...
    def test_does_not_overwrite_existing_honcho_host_on_rename(self, profile_env):
        ...
        honcho_path.write_text(json.dumps({
            "hosts": {
                "laia.ssi_health": {"aiPeer": "ssi_health"},
                "laia.heimdall": {"aiPeer": "heimdall"},
            }
        }))
        ...
        cfg = json.loads(honcho_path.read_text())
        assert cfg["hosts"]["laia.ssi_health"]["aiPeer"] == "ssi_health"
        assert cfg["hosts"]["laia.heimdall"]["aiPeer"] == "heimdall"
...
class TestExportImport:
    ...
    def test_export_default_excludes_infrastructure(self, profile_env, tmp_path):
        ...
        for d in ("laia-agent", ".worktrees", "profiles", "bin",
        ...
        for f in ("state.db", "gateway.pid", "gateway_state.json",
                  "processes.json", "errors.log", ".laia_history",
        ...
        excluded_prefixes = [
            "default/laia-agent", "default/.worktrees", "default/profiles",
        ...
        excluded_files = [
            ...
            "default/errors.log", "default/.laia_history",
...
class TestCompletion:
    ...
    def test_bash_completion_has_laia_profiles_function(self):
        script = generate_bash_completion()
        assert "_laia_profiles" in script

    def test_zsh_completion_has_laia_function(self):
        script = generate_zsh_completion()
        assert "_laia" in script


# ===================================================================
# TestGetProfilesRoot / TestGetDefaultLAIAHome (internal helpers)
# ===================================================================

class TestInternalHelpers:
    """Tests for _get_profiles_root() and _get_default_laia_home()."""

    def test_profiles_root_under_home(self, profile_env):
        tmp_path = profile_env
        root = _get_profiles_root()
        assert root == tmp_path / ".laia" / "profiles"

    def test_default_laia_home(self, profile_env):
        tmp_path = profile_env
        home = _get_default_laia_home()
        assert home == tmp_path / ".laia"

    def test_profiles_root_docker_deployment(self, tmp_path, monkeypatch):
        """In Docker (LAIA_HOME outside ~/.laia), profiles go under LAIA_HOME."""
        docker_home = tmp_path / "opt" / "data"
        docker_home.mkdir(parents=True)
        monkeypatch.setattr(Path, "home", lambda: tmp_path)
        monkeypatch.setenv("LAIA_HOME", str(docker_home))
        root = _get_profiles_root()
        assert root == docker_home / "profiles"

    def test_default_laia_home_docker(self, tmp_path, monkeypatch):
        """In Docker, _get_default_laia_home() returns LAIA_HOME itself."""
        docker_home = tmp_path / "opt" / "data"
        docker_home.mkdir(parents=True)
        monkeypatch.setattr(Path, "home", lambda: tmp_path)
        monkeypatch.setenv("LAIA_HOME", str(docker_home))
    ...
    def test_profiles_root_profile_mode(self, tmp_path, monkeypatch):
        """In profile mode (LAIA_HOME under ~/.laia), profiles root is still ~/.laia/profiles."""
        native = tmp_path / ".laia"
    ...
    def test_active_profile_path_docker(self, tmp_path, monkeypatch):
        """In Docker, active_profile file lives under LAIA_HOME."""
        from laia_cli.profiles import _get_active_profile_path
        docker_home = tmp_path / "opt" / "data"
        docker_home.mkdir(parents=True)
        monkeypatch.setattr(Path, "home", lambda: tmp_path)
        monkeypatch.setenv("LAIA_HOME", str(docker_home))
    ...
    def test_create_profile_docker(self, tmp_path, monkeypatch):
        """Profile created in Docker lands under LAIA_HOME/profiles/."""
        docker_home = tmp_path / "opt" / "data"
        docker_home.mkdir(parents=True)
        monkeypatch.setattr(Path, "home", lambda: tmp_path)
        monkeypatch.setenv("LAIA_HOME", str(docker_home))
    ...
    def test_active_profile_name_docker_profile(self, tmp_path, monkeypatch):
        ...
        monkeypatch.setattr(Path, "home", lambda: tmp_path)
        monkeypatch.setenv("LAIA_HOME", str(profile))
...
class TestEdgeCases:
    ...
    def test_create_profile_returns_correct_path(self, profile_env):
        ...
        expected = tmp_path / ".laia" / "profiles" / "mybot"
        assert result == expected
    ...
    def test_gateway_running_check_with_pid_file(self, profile_env):
        ...
        from laia_cli.profiles import _check_gateway_running
        tmp_path = profile_env
        default_home = tmp_path / ".laia"
