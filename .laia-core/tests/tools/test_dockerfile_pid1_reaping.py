...
- Signal forwarding runs through the init so ``docker stop`` triggers
  laia's own graceful-shutdown path.
"""
...
def test_dockerfile_installs_an_init_for_zombie_reaping(dockerfile_text):
    ...
    Without a PID-1 init that handles SIGCHLD, laia accumulates zombie
    ...
    assert installed, (
        ...
        "orphaned subprocesses, laia accumulates zombies in Docker "
...
def test_dockerfile_entrypoint_routes_through_the_init(dockerfile_text):
    ...
    directly, the shell becomes PID 1 and will ``exec`` into laia,
    ...
    assert routes_through_init, (
        ...
        "If tini is only installed but not wired into ENTRYPOINT, laia "
...
def test_dockerfile_installs_tui_dependencies(dockerfile_text):
    ...
    assert "ui-tui/packages/laia-ink/package-lock.json" in dockerfile_text
    assert any(
...
def test_dockerfile_materializes_local_tui_ink_package(dockerfile_text):
    assert any(
        "ui-tui" in step
        and "node_modules/@laia/ink" in step
        and "packages/laia-ink" in step
        and "rm -rf packages/laia-ink/node_modules" in step
        and "npm install --omit=dev" in step
        and "--prefix node_modules/@laia/ink" in step
        and "rm -rf node_modules/@laia/ink/node_modules/react" in step
        and "await import('@laia/ink')" in step
        for step in _run_steps(dockerfile_text)
    )
