...
export default function CommandCenter() {
  ...
  const [laiaWidth, setLAIAWidth] = useState(340)
  ...
  function onDragHandleMouseDown(e: React.MouseEvent) {
    ...
    dragStartW.current = laiaWidth
    function onMove(ev: MouseEvent) {
      if (!dragging.current) return
      const delta = ev.clientX - dragStartX.current
      setLAIAWidth(Math.max(220, Math.min(600, dragStartW.current + delta)))
    }
    function onUp() {
  ...
  // Poll terminal list every 2s so terminals spawned by LAIA via its tool appear automatically
  useEffect(() => {
    let cancelled = false
    const sync = () => {
      if (cancelled) return
    ...
    >
      <style>{`
        ...
        @media (max-width: 1180px){
          ...
          .cc-laia-pane{width:min(330px,38vw)!important}
        }
        @media (max-width: 900px){
          .cc-body{position:relative}
          .cc-laia-pane{width:min(360px,46vw)!important}
        ...
        @media (max-width: 680px){
          .cc-laia-pane{display:none!important}
      ...
      <div className="cc-topbar" style={{
        ...
        <button
          ...
          title={promptApprovalRequired ? 'Prompts de LAIA requieren aprobación' : 'Prompts de LAIA se inyectan sin aprobación'}
          style={{
      ...
      <div className="cc-body" style={{ display: 'flex', flex: 1, minHeight: 0 }}>
        ...
        {/* LAIA panel — resizable left */}
        <div className="cc-laia-pane" style={{
          width: laiaWidth, flexShrink: 0, display: 'flex', flexDirection: 'column',
          overflow: 'hidden',
        }}>
          <div style={{
            padding: '8px 12px 0',
...
function WorkspaceSynapsePanel({
  ...
  const absorbPrompt = () => {
    ...
    window.dispatchEvent(new CustomEvent('cc:laia-draft', {
      detail: {
        text: [
      ...
      <div style={{
        ...
        <div style={{ display: 'flex', alignItems: 'center', gap: 7, marginBottom: 6 }}>
          ...
          <button
            ...
            title="Preparar cierre para LAIA"
            style={{
              marginLeft: 'auto',
              width: 24,
              height: 22,
