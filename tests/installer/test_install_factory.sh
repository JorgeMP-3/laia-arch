...
assert "auth.json created" "$([[ -f "$LAIA_HOME_OVERRIDE/auth.json" ]] && echo 0 || echo 1)"
assert "cli-config.yaml created" "$([[ -f "$LAIA_HOME_OVERRIDE/cli-config.yaml" ]] && echo 0 || echo 1)"
