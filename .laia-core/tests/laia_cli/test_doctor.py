...
import laia_cli.doctor as doctor
import laia_cli.gateway as gateway_cli
from laia_cli import doctor as doctor_mod
from laia_cli.doctor import _has_provider_env_config
...
def test_run_doctor_sets_interactive_env_for_tool_checks(monkeypatch, tmp_path):
    ...
    monkeypatch.setattr(doctor_mod, "LAIA_HOME", laia_home)
    monkeypatch.delenv("LAIA_INTERACTIVE", raising=False)

    seen = {}

    def fake_check_tool_availability(*args, **kwargs):
        seen["interactive"] = os.getenv("LAIA_INTERACTIVE")
        raise SystemExit(0)
...
class TestDoctorMemoryProviderSection:
    ...
    def _make_laia_home(self, tmp_path, provider=""):
        """Create a minimal LAIA_HOME with config.yaml."""
        home = tmp_path / ".laia"
    ...
    def _run_doctor_and_capture(self, monkeypatch, tmp_path, provider=""):
        ...
        home = self._make_laia_home(tmp_path, provider)
...
def test_run_doctor_accepts_laia_provider_ids_that_catalog_aliases(
