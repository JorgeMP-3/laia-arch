...
def update_config_fallback_text(workspace_name: str, activate: bool) -> None:
    if not CONFIG_PATH.exists():
...
def update_config(workspace_name: str, activate: bool) -> None:
    if not CONFIG_PATH.exists():
        print(f"  AVISO: config.yaml no encontrado en {CONFIG_PATH}")
        return

    if yaml is None:
        update_config_fallback_text(workspace_name, activate)
        if activate:
