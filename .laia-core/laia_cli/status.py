"""
Status command for laia CLI.

Shows the status of all LAIA Agent components.
"""
...
from laia_cli.auth import AuthError, resolve_provider
from laia_cli.colors import Colors, color
from laia_cli.config import get_env_path, get_env_value, get_laia_home, load_config
from laia_cli.models import provider_label
from laia_cli.nous_subscription import get_nous_subscription_features
from laia_cli.runtime_provider import resolve_requested_provider
...
def redact_key(key: str) -> str:
    ...
    "(not set)" placeholder in dim color to match ``laia config``'s
...
def show_status(args):
    """Show status of all LAIA Agent components."""
    show_all = getattr(args, 'all', False)
    deep = getattr(args, 'deep', False)
    
    print()
    print(color("┌─────────────────────────────────────────────────────────┐", Colors.CYAN))
    print(color("│                 ⚕ LAIA Agent Status                  │", Colors.CYAN))
    print(color("└─────────────────────────────────────────────────────────┘", Colors.CYAN))
    ...
    nous_label = "logged in" if nous_logged_in else "not logged in (run: laia auth add nous --type oauth)"
    print(
    ...
    print(
        ...
        f"{'logged in' if codex_logged_in else 'not logged in (run: laia model)'}"
    )
    ...
    print(
        ...
        f"{'logged in' if minimax_logged_in else 'not logged in (run: laia auth add minimax-oauth)'}"
    )
    ...
    for pname, env_vars in apikey_providers.items():
        ...
        label = "configured" if configured else "not configured (run: laia model)"
    ...
    if _effective_provider_label() == "LM Studio":
        from laia_cli.models import probe_lmstudio_models
        model_cfg = config.get("model")
    ...
    if terminal_env == "ssh":
        ...
        sdk_label = "installed" if sdk_ok else "missing (install: pip install 'laia-agent[vercel]')"
    ...
    try:
        from laia_cli.gateway import get_gateway_runtime_snapshot, _format_gateway_pids
    ...
    jobs_file = get_laia_home() / "cron" / "jobs.json"
    if jobs_file.exists():
        import json
        try:
    ...
    sessions_file = get_laia_home() / "sessions" / "sessions.json"
    ...
    print(color("  Run 'laia doctor' for detailed diagnostics", Colors.DIM))
    print(color("  Run 'laia setup' to configure", Colors.DIM))
    print()
