"""Shell completion script generation for laia CLI.
...
def generate_bash(parser: argparse.ArgumentParser) -> str:
    ...
    for cmd in sorted(tree["subcommands"]):
        ...
        if cmd == "profile" and info["subcommands"]:
            ...
            cases.append(
                ...
                f"                    COMPREPLY=($(compgen -W \"$(_laia_profiles)\" -- \"$cur\"))\n"
                f"                    return\n"
                f"                    ;;\n"
                f"            esac\n"
                f"            ;;"
    ...
    return f"""# LAIA Agent bash completion
# Add to ~/.bashrc:
#   eval "$(laia completion bash)"
...
def generate_zsh(parser: argparse.ArgumentParser) -> str:
    ...
    return f"""#compdef laia
# LAIA Agent zsh completion
# Add to ~/.zshrc:
#   eval "$(laia completion zsh)"
...
_laia() {{
    ...
    _arguments -C \\
        ...
        '(-p --profile){{-p,--profile}}[Profile name]:profile:_laia_profiles' \\
        '1:command:->commands' \\
        '*::arg:->args'
...
{top_cmds_str}
            ...
            _describe 'laia command' subcmds
            ;;
        args)
            case ${{line[1]}} in
{sub_cases_str}
...
def generate_fish(parser: argparse.ArgumentParser) -> str:
    ...
    lines: list[str] = [
        "# LAIA Agent fish completion",
        "# Add to your config:",
        "#   laia completion fish | source",
        "",
        "# Helper: list available profiles",
        "function __laia_profiles",
        "    echo default",
        "    if test -d $HOME/.laia/profiles",
        "        ls $HOME/.laia/profiles 2>/dev/null",
        "    end",
        "end",
        "",
        ...
        "",
        "# Complete profile names after -p / --profile",
        "complete -c laia -f -s p -l profile"
        " -d 'Profile name' -xa '(__laia_profiles)'",
        "",
        ...
        if cmd == "profile":
            for action in sorted(profile_name_actions):
                lines.append(
                    ...
                    f"-a '(__laia_profiles)' -d 'Profile name'"
                )
