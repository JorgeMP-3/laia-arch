...
The evaluate flow:
    ...
    2. evaluate()  -- Iterates over all runs sequentially through:
        a. rollout_and_score_eval()  -- Per-run agent loop
            ...
            - Runs LAIAAgentLoop with terminal tool only
...
Requires: pip install laia-agent[yc-bench]
"""
...
from environments.agent_loop import LAIAAgentLoop
...
class YCBenchEvalConfig(LAIAAgentEnvConfig):
    """
    Configuration for the YC-Bench evaluation environment.

    Extends LAIAAgentEnvConfig with YC-Bench-specific settings for
...
class YCBenchEvalEnv(LAIAAgentBaseEnv):
    """
    ...
    a competing built-in agent loop). The LAIAAgentLoop then drives the
    ...
    async def setup(self):
        ...
        try:
            ...
            raise RuntimeError(
                ...
                '  pip install "laia-agent[yc-bench]"\n'
    ...
    async def rollout_and_score_eval(self, eval_item: Dict[str, Any]) -> Dict:
        ...
        3. Runs LAIAAgentLoop with terminal tool
        ...
        try:
            ...
            # LiteLLM), which would compete with our LAIAAgentLoop.
