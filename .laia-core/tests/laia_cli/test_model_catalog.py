"""Tests for laia_cli.model_catalog — remote manifest fetch + cache + fallback."""
...
def isolated_home(tmp_path, monkeypatch):
    """Isolate LAIA_HOME + reset any module-level catalog cache per test."""
    home = tmp_path / ".laia"
    home.mkdir()
    ...
    import importlib
    from laia_cli import model_catalog
...
class TestValidation:
    def test_accepts_well_formed_manifest(self, isolated_home):
        from laia_cli.model_catalog import _validate_manifest
...
class TestIntegrationWithModelsModule:
    """Exercise the fallback paths via the real callers in laia_cli.models."""

    def test_curated_nous_ids_falls_back_to_hardcoded_on_empty_catalog(
        self, isolated_home
    ):
        from laia_cli import model_catalog
        from laia_cli.models import get_curated_nous_model_ids, _PROVIDER_MODELS
    ...
    def test_curated_nous_ids_prefers_manifest(self, isolated_home):
        from laia_cli import model_catalog
        from laia_cli.models import get_curated_nous_model_ids
