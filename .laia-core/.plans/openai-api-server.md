# OpenAI-Compatible API Server for LAIA Agent
...
SSE streaming. By exposing this endpoint, laia-agent becomes instantly
...
│  AnythingLLM     │  │    {"messages": [...]}             │  laia-agent   │
│  NextChat        │  │                                    │  gateway        │
...
A user would:
1. Set `API_SERVER_ENABLED=true` in `~/.laia/.env`
2. Run `laia gateway` (API server starts alongside Telegram/Discord/etc.)
3. Point Open WebUI (or any frontend) at `http://localhost:8642/v1`
4. Chat with laia-agent through any OpenAI-compatible UI
...
| GET | `/v1/models` | List available "models" (returns laia-agent as a model) |
...
Authorization: Bearer laia-api-key-here
...
But laia-agent sessions have persistent state (memory, skills, tool context).
...
Frontends send `"model": "laia-agent"` (or whatever). The actual LLM model
used is configured server-side in config.yaml. The API server maps any
requested model name to the configured laia-agent model.
...
1. `gateway/platforms/api_server.py` — new adapter
   ...
   - Frontend system prompt layered on top of laia-agent's core prompt
...
Once implemented, laia-agent works as a drop-in backend for:
