"""laia hooks — inspect and manage shell-script hooks.

Usage::

    laia hooks list
    laia hooks test <event> [--for-tool X] [--payload-file F]
    laia hooks revoke <command>
    laia hooks doctor

Consent records live under ``~/.laia/shell-hooks-allowlist.json`` and
hook definitions come from the ``hooks:`` block in ``~/.laia/config.yaml``
...
def hooks_command(args) -> None:
    """Entry point for ``laia hooks`` — dispatches to the requested action."""
    sub = getattr(args, "hooks_action", None)

    if not sub:
        print("Usage: laia hooks {list|test|revoke|doctor}")
        print("Run 'laia hooks --help' for details.")
        return
...
def _cmd_list(_args) -> None:
    ...
    if not specs:
        print("No shell hooks configured in ~/.laia/config.yaml.")
        print("See `laia hooks --help` or")
                ...
                if entry and entry.get("approved_at"):
                    ...
                    if mtime_now and mtime_at and mtime_now > mtime_at:
                        print(
                            ...
                            f"run `laia hooks doctor` to re-validate"
                        )
        print()
...
# stdin a script sees under `laia hooks test` and `laia hooks doctor`
...
def _print_run_result(result: Dict[str, Any]) -> None:
    ...
    if parsed:
        print(f"      parsed (LAIA wire shape): {json.dumps(parsed)}")
    else:
...
def _doctor_one(spec, shell_hooks) -> int:
    ...
    if entry and entry.get("script_mtime_at_approval"):
        ...
        if mtime_now and mtime_at and mtime_now > mtime_at:
            ...
            print(f"      ⚠ script modified since approval "
                  ...
                  f"then `laia hooks revoke` + re-approve to refresh")
    ...
    # is already allowlisted.  Otherwise `laia hooks doctor` would execute
