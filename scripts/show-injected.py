...
def build_instruction(
    workspace: str,
    mode: str,
    all_workspace_names: list[str] | None = None,
    ...
    editable = ", ".join(active_workspaces or [workspace])
    loaded_text = (
    ...
    return (
        f"[WORKSPACE ACTIVO: {workspace} | MODO: {mode} | EDITABLES: {editable}]\n"
        f"{loaded_text}"
...
def main() -> None:
    ...
    active_workspaces = ws_cfg.get("active_workspaces") or []
    if isinstance(active_workspaces, str):
        active_workspaces = [name.strip() for name in active_workspaces.split(",") if name.strip()]
    active_workspaces = [str(name).strip() for name in active_workspaces if str(name).strip()] or [workspace]
    ...
    if inject_mode == "all-indexes":
        ...
        all_names = [workspace]
    instruction = build_instruction(workspace, inject_mode, all_names, active_workspaces)
